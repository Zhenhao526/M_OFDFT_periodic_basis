#!/usr/bin/env python3
"""Validate follow-up R3 preregistration or replay committed evidence."""
...
from analyze_s1_g1_three_layer_al_followup_r3 import build_analysis, write_analysis
...
def validate_preregistered(project_root: Path, config: dict, rows: list[dict[str, str]]) -> dict:
    ...
    require(config["external_state_root"] == "/home/shenwei01/.local/state/m_ofdft/s1_g1_three_layer_al_domain_followup_r3_20260810", "external state path differs")
    require(not Path(config["external_state_root"]).exists(), "fresh R3 external state must not exist at preregistration")
...
def validate_final(project_root: Path, config: dict, rows: list[dict[str, str]], require_committed: bool) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="g1_al_followup_r3_replay_") as temporary:
...
def main() -> int:
    ...
    if args.preregistered_only or args.require_binding_smoke_committed:
        ...
        if args.require_binding_smoke_committed:
            smoke = verify_binding_smoke(config, project_root, payload["head"])
