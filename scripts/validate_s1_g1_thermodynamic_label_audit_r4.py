#!/usr/bin/env python3
"""Validate the fail-closed S1-G1 thermodynamic-label audit R4.

R4 is a clean rerun, not a reinterpretation of R1.  Historical R1/R2/R3 runs stay
immutable but contribute zero to R4 acceptance.  Forty new experiment IDs
...
an R4 experiment-number offset.
"""
...
import generate_s1_g1_thermodynamic_label_audit_r4 as registration
...
from parse_s1_g1_thermodynamic_labels_r4 import (
...
R4_AUDIT_IDS = tuple(registration.R4_AUDIT_IDS)
...
EVIDENCE_NAME = "g1_thermodynamic_label_audit_r4.json"
STATUS_NAME = "thermodynamic_label_status_r4.json"
FAILURE_CLASS_NAME = "thermodynamic_label_failure_classification_r4.json"
FAILURE_INVENTORY_NAME = "thermodynamic_label_failure_artifact_inventory_r4.json"
LABEL_NAME = r1.LABEL_NAME
ANALYSIS_ROOT = Path("analysis/s1/g1_thermodynamic_label_audit_r4_20260807")
...
EXPECTED_NEW_TO_LOGICAL = {
    ...
    for new, logical in enumerate(_EXPECTED_LOGICAL_SUFFIXES, 41)
}
...
R4_PILOT_IDS = tuple(registration.PILOT_IDS)
R4_K_GATE_COMPLETION_LOGICAL_ID = EXPECTED_NEW_TO_LOGICAL[R4_PILOT_IDS[-1]]
ATTEMPT_LEDGER_ROOT = Path(
    "orchestration/s1/g1_thermodynamic_label_audit_r4_20260807/attempts"
)
...
SUPERVISOR_STATE_DIRECTORY = (
    ...
    "g1_thermodynamic_label_audit_r4_20260807"
)
...
def _assert_static_namespace() -> None:
    if R1_REUSED_AUDIT_IDS != EXPECTED_REUSED_LOGICAL_IDS:
        raise RuntimeError("R4 must not reuse any historical R1 run")
    if NEW_TO_LOGICAL != EXPECTED_NEW_TO_LOGICAL:
        raise RuntimeError("generator changed the frozen R4-to-logical mapping")
    if R4_AUDIT_IDS != tuple(EXPECTED_NEW_TO_LOGICAL):
        raise RuntimeError("R4 physical ID order differs from 001--040")
    expected_effective = {
    ...
    if EXECUTION_ORDER != R4_AUDIT_IDS:
        raise RuntimeError("R4 execution order must be the exact 001--040 sequence")
...
def _expected_registered_files(project_root: Path) -> dict[str, object]:
    return {
        ...
        "runner_path": "scripts/run_s1_g1_thermodynamic_label_audit_r4.sh",
        "runner_sha256": sha256(
            project_root / "scripts/run_s1_g1_thermodynamic_label_audit_r4.sh"
        ),
    }
...
def _expected_sealed_execution_inputs(
    ...
    relative_paths = {
        "runner": Path("scripts/run_s1_g1_thermodynamic_label_audit_r4.sh"),
...
def logical_id(experiment_id: str) -> str:
    ...
    try:
        ...
        raise ValueError(f"unknown effective R4 experiment ID: {experiment_id}") from error


def source_kind(logical_experiment_id: str) -> str:
    return "r1_reused" if logical_experiment_id in R1_REUSED_AUDIT_IDS else "r4_executed"
...
def row_for_logical(
    ...
    try:
        ...
        raise ValueError(f"R4 manifest lacks logical slot {logical_experiment_id}") from error
    ...
    *,
    ...
    try:
        ...
        if bridge.get("reused_logical_ids") != [] or bridge.get("reused_runs") != {}:
            errors.append("R1 bridge must exclude all historical runs from R4 reuse")
        ...
        if bridge.get("historical_accepted_scientific_denominator_contribution") != 0:
            errors.append("R1 historical runs contribute to the R4 denominator")
...
def _validate_input_derivation(
    ...
    try:
        directory = _safe_project_path(project_root, row["input_directory"], "R4 input directory")
        ...
        for key in set(before) | set(after):
            expected = (
                (_expected_r4_suffix(r1_row["suffix"]),)
                if key == "suffix"
                else before.get(key)
            )
            if after.get(key) != expected:
                errors.append(f"{prefix} INPUT differs outside the registered R4 suffix: {key}")
        if row["suffix"] != _expected_r4_suffix(r1_row["suffix"]):
            errors.append(f"{prefix} manifest suffix differs from the exact R4 derivation")
...
def _validate_manifest_rows(
    ...
    if ids != R4_AUDIT_IDS:
        errors.append("R4 manifest rows must be the exact 041--080 physical IDs in order")
    if tuple(row.get("execution_index", "") for row in rows) != tuple(
        str(index) for index in range(1, 41)
    ):
        errors.append("R4 manifest execution indices differ from the exact execution order")
    r1_by_id = _r1_by_id(project_root)
    for index, physical_id in enumerate(R4_AUDIT_IDS, 1):
        ...
        if row.get("execution_phase") != ("P1" if physical_id in R4_PILOT_IDS else "P2"):
        ...
        if row.get("suffix") != _expected_r4_suffix(old["suffix"]):
    ...
    if config.get("registered_experiment_ids") != list(R4_AUDIT_IDS):
        errors.append("config registered physical IDs differ")
    if config.get("execution_order") != list(EXECUTION_ORDER):
        errors.append("config R4 execution order differs")
...
def _translated_energy_matrix(r1_config: dict) -> dict[str, object]:
    ...
    matrix["r4_executed_audit_run_count"] = 40
    return matrix
...
def _validate_logical_matrices(
    ...
    try:
        ...
        errors.append(f"cannot rebuild R4 logical/new matrices: {error}")
...
def _validate_config_contract(
    ...
    if set(config) != expected_keys:
        errors.append("R4 config top-level key set differs")
    if type(config.get("schema_version")) is not int or config.get("schema_version") != 2:
        errors.append("R4 config schema version differs")
    if config.get("protocol_revision") != PROTOCOL_REVISION:
        errors.append("R4 config protocol revision differs")
    if config.get("status") != "preregistered":
        errors.append("R4 config status must be preregistered")
    if config.get("preregistration_date") != "2026-08-07":
        errors.append("R4 preregistration date differs")
    if config.get("input_root") != str(INPUT_ROOT):
        errors.append("R4 input root differs")
    if config.get("scope") != "G1 third-smearing / dense-k thermodynamic-label R4 continuation only":
        errors.append("R4 scope statement differs")

    r1_config = read_json(project_root / R1_CONFIG_PATH)
    expected_execution = {
        "rank_count": 4,
        "pilot_ids": list(R4_PILOT_IDS),
        "k_gate_completion_ids": list(R4_PILOT_IDS),
    ...
    if config.get("execution") != expected_execution:
        errors.append("R4 execution/no-retry/supervisor contract differs")
    ...
    if isinstance(observed_execution, dict):
        for contract_name in (
            ...
            if not isinstance(observed_contract, dict) or type(
                ...
                errors.append(f"R4 {contract_name} schema version type differs")
        ...
        if (
            ...
            errors.append("R4 supervisor completion runner-exit type differs")
    expected_coverage = {
        ...
        "r3_historical_accepted_excluded_count": 1,
        "r3_rejected_archive_reused_count": 0,
        "selected_scalar_point_count": 42,
    }
    if config.get("coverage") != expected_coverage:
        errors.append("R4 all-new 40-run logical coverage accounting differs")
    ...
    if config.get("input_derivation") != expected_input_derivation:
        errors.append("R4 R1-input derivation contract differs")
    try:
        if config.get("r2_stopped_bridge") != registration.build_r2_stopped_bridge(
            project_root
        ):
            errors.append("R4 immutable R2 stopped bridge differs")
    except (FileNotFoundError, TypeError, ValueError, subprocess.CalledProcessError) as error:
        errors.append(f"R4 immutable R2 stopped bridge validation failed: {error}")
    try:
        if config.get("r3_stopped_bridge") != registration.build_r3_stopped_bridge(
            project_root
        ):
            errors.append("R4 immutable R3 stopped bridge differs")
    except (FileNotFoundError, TypeError, ValueError, subprocess.CalledProcessError) as error:
        errors.append(f"R4 immutable R3 stopped bridge validation failed: {error}")
    try:
        expected_runtime = registration.build_r4_runtime_contract(
            project_root, r1_config
        )
        if config.get("runtime") != expected_runtime:
            errors.append("R4 stop-confirmed runtime contract differs")
    except (FileNotFoundError, KeyError, TypeError, ValueError) as error:
        errors.append(f"R4 stop-confirmed runtime contract validation failed: {error}")
    for key in (
        "numerical_axes",
        "output_contract",
        "thermodynamic_semantics",
        ...
        if config.get(key) != r1_config.get(key):
            errors.append(f"R4 frozen R1 scientific/runtime contract changed: {key}")
    for key in (
        "base_evidence_commit",
        "upstream_evidence",
        "source_runs",
        ...
        if config.get(key) != r1_config.get(key):
            errors.append(f"R4 frozen R1 upstream/source contract changed: {key}")
    try:
        if config.get("energy_matrix") != _translated_energy_matrix(r1_config):
            errors.append("R4 translated 42-point energy matrix differs")
        if config.get("field_label_groups") != _translated_field_groups(r1_config):
            errors.append("R4 translated field-label groups differ")
    except (KeyError, TypeError, ValueError) as error:
        errors.append(f"R4 translated scientific matrix validation failed: {error}")
    _validate_logical_matrices(project_root, config, rows, errors)

    manifest = config.get("manifest")
    if not isinstance(manifest, dict):
        errors.append("R4 manifest registration is missing")
    else:
        if manifest.get("path") != str(MANIFEST_PATH):
            errors.append("R4 manifest path differs")
        if manifest.get("sha256") != sha256(project_root / MANIFEST_PATH):
            errors.append("R4 manifest SHA-256 differs")
        if manifest.get("row_count") != 40:
            errors.append("R4 manifest row count differs")
        if manifest.get("fields") != list(MANIFEST_FIELDS):
            errors.append("R4 manifest field registration differs")
    expected_scope = {
    ...
    if config.get("formal_preregistration_commit_scope") != expected_scope:
        errors.append("R4 formal preregistration commit-scope contract differs")
...
def _validate_implementation(
    ...
    *,
    ...
    if not isinstance(implementation, dict) or not implementation:
        errors.append("R4 implementation closure is missing")
        return
    if not isinstance(modes, dict) or set(modes) != set(implementation):
        errors.append("R4 implementation Git-mode closure differs")
        return
    required = {
        "docs/S1_G1_THERMODYNAMIC_LABEL_AUDIT_R4_PROTOCOL.md",
        "scripts/generate_s1_g1_thermodynamic_label_audit_r4.py",
        "scripts/parse_s1_g1_thermodynamic_labels_r4.py",
        "scripts/validate_s1_g1_thermodynamic_label_audit_r4.py",
        "scripts/analyze_s1_g1_thermodynamic_label_audit_r4.py",
        "scripts/run_s1_g1_thermodynamic_label_audit_r4.sh",
        "scripts/launch_s1_g1_thermodynamic_label_audit_r4.py",
        "tests/unit/test_s1_g1_thermodynamic_label_audit_r4_validator.py",
    }
    if not required.issubset(implementation):
        errors.append("R4 implementation closure omits a required path")
    for relative, digest in implementation.items():
        try:
            path = _safe_project_path(project_root, relative, "implementation path")
            if not path.is_file() or path.is_symlink():
                errors.append(f"R4 implementation is missing or symbolic: {relative}")
                continue
            if not isinstance(digest, str) or sha256(path) != digest:
                errors.append(f"R4 implementation SHA-256 differs: {relative}")
            if require_committed:
                failure = _tracked_head_failure(project_root, relative)
                if failure:
                    errors.append(f"R4 implementation {failure}")
                    continue
                entries = str(_git(project_root, "ls-files", "--stage", "--", relative)).splitlines()
                if len(entries) != 1:
                    errors.append(f"R4 implementation is not tracked exactly once: {relative}")
                    continue
                mode = entries[0].split(maxsplit=1)[0]
                if mode not in {"100644", "100755"} or modes.get(relative) != mode:
                    errors.append(f"R4 implementation Git mode differs: {relative}")
        except (FileNotFoundError, TypeError, ValueError) as error:
            errors.append(f"R4 implementation validation failed {relative}: {error}")
...
def _validate_preregistration(
    ...
    try:
        ...
        if _introduction_commit(project_root, str(MANIFEST_PATH)) != prereg:
            errors.append("R4 config and manifest were not introduced together")
        expected_inputs = {
        ...
        if _commit_changed_paths(project_root, prereg) != expected_scope:
            errors.append("R4 preregistration scope is not config+manifest+complete 40-run input tree")
        parent = str(_git(project_root, "rev-parse", f"{prereg}^"))
        if config.get("generated_from_commit") != parent:
            errors.append("R4 preregistration parent differs from generated_from_commit")
        for relative in (str(CONFIG_PATH), str(MANIFEST_PATH)):
            if _blob_at(project_root, prereg, relative) != (project_root / relative).read_bytes():
                errors.append(f"R4 preregistration blob differs: {relative}")
        implementation = config.get("implementation")
        if isinstance(implementation, dict):
            for relative in implementation:
                if _blob_at(project_root, parent, relative) != (project_root / relative).read_bytes():
                    errors.append(f"R4 implementation differs from preregistration parent: {relative}")
        for row in rows:
            experiment_id = row["experiment_id"]
            input_directory = row["input_directory"]
            if str(_git(project_root, "ls-tree", "-r", parent, "--", f"runs/{experiment_id}")):
                errors.append(f"R4 run existed before preregistration: {experiment_id}")
            if str(_git(project_root, "ls-tree", "-r", parent, "--", input_directory)):
                errors.append(f"R4 input existed before preregistration: {experiment_id}")
            if _tree_oid(project_root, prereg, input_directory) != _tree_oid(
                project_root, "HEAD", input_directory
            ):
                errors.append(f"R4 input changed after preregistration: {experiment_id}")
            for basename in ("INPUT", "STRU", "KPT", "metadata.json"):
                relative = f"{input_directory}/{basename}"
                if _introduction_commit(project_root, relative) != prereg:
                    errors.append(f"R4 input was not introduced by preregistration: {relative}")
                if _blob_at(project_root, prereg, relative) != (project_root / relative).read_bytes():
                    errors.append(f"R4 input differs from preregistration: {relative}")
        return prereg
    except (
        FileNotFoundError,
        KeyError,
        ...
        errors.append(f"R4 preregistration binding failed: {error}")
        return None
...
def _active_introduction(
    ...
    if not run.is_dir() or run.is_symlink():
        errors.append(f"R4 active run is invalid: {experiment_id}")
        return None
    ...
    if not marker.is_file() or marker.is_symlink():
        errors.append(f"R4 active run lacks metadata/status marker: {experiment_id}")
        return None
    try:
        return _latest_introduction_commit(project_root, _relative(project_root, marker))
    except ValueError as error:
        errors.append(f"cannot bind R4 active run {experiment_id}: {error}")
        return None
...
def _archive_events(
    ...
    if not root.is_dir() or root.is_symlink():
        errors.append(f"R4 failed-archive root is invalid: {experiment_id}")
        return []
    ...
    for attempt in sorted(root.iterdir(), key=lambda path: path.name):
        ...
        if not attempt.is_dir() or attempt.is_symlink() or match is None:
            errors.append(f"R4 failed archive is non-canonical: {experiment_id}/{attempt.name}")
            continue
        try:
            ...
            if str(_git(project_root, "rev-parse", f"{archive_commit}^")) != failure_commit:
                errors.append(f"R4 archive is not adjacent to failed commit: {experiment_id}")
            ...
            if not failure_tree == archive_tree == head_tree:
                errors.append(f"R4 failed tree changed during or after archival: {experiment_id}")
            ...
            if not failure_paths or any(not path.startswith(f"{run_relative}/") for path in failure_paths):
                errors.append(f"R4 failed run commit is not independently scoped: {experiment_id}")
            ...
            if not archive_paths or any(
                ...
                errors.append(f"R4 archive commit scope differs: {experiment_id}")
            if subprocess.run(
                ["git", "-C", str(project_root), "cat-file", "-e", f"{archive_commit}:{run_relative}"],
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL,
            ).returncode == 0:
                errors.append(f"R4 failed run was copied instead of moved: {experiment_id}")
            events.append((attempt.name, failure_commit))
        except (subprocess.CalledProcessError, ValueError) as error:
            errors.append(f"cannot bind R4 failed archive {experiment_id}: {error}")
    return events
...
def validate_detachment_attestation(
    ...
    *,
    ...
    if not isinstance(execution, dict):
        return payload, ["R4 execution contract is missing"]
    ...
    try:
        ...
        if require_committed:
            ...
            if str(_git(project_root, "rev-parse", f"{introduction}^")) != prereg:
                errors.append("detachment attestation commit is not adjacent to R4 preregistration")
            if payload.get("git_head") != prereg:
                errors.append("detachment attestation git_head differs from R4 preregistration")
...
def validate_precompletion_analysis(
    ...
    *,
    ...
    try:
        parsed = _external_object(path, "R4 analysis summary")
        payload = parsed
        execution = config.get("execution")
        contract = (
        ...
        if not isinstance(contract, dict):
            errors.append("R4 analysis supervisor-completion contract is missing")
        elif (
            ...
            errors.append("R4 analysis/pre-completion authority status differs")
        if payload.get("config_sha256") != sha256(project_root / CONFIG_PATH):
            errors.append("R4 analysis config SHA-256 differs")
        if payload.get("manifest_sha256") != sha256(project_root / MANIFEST_PATH):
            errors.append("R4 analysis manifest SHA-256 differs")
        if require_committed:
            failure = _tracked_head_failure(project_root, str(ANALYSIS_SUMMARY_PATH))
            if failure:
                errors.append(f"R4 analysis summary {failure}")
            ...
            if str(_git(project_root, "rev-parse", "HEAD")) != introduction:
                errors.append("R4 pre-completion analysis is not HEAD")
            ...
            if (
                ...
                errors.append("R4 pre-completion analysis commit scope differs")
            if _blob_at(
                project_root, introduction, ANALYSIS_SUMMARY_PATH.as_posix()
            ) != path.read_bytes():
                errors.append("R4 analysis summary differs from its introduction blob")
    except (
        FileNotFoundError,
        KeyError,
        TypeError,
        ...
        errors.append(f"R4 pre-completion analysis validation failed: {error}")
    return payload, errors
...
def recompute_final_analysis(
    ...
    errors: list[str] = []
    try:
        from analyze_s1_g1_thermodynamic_label_audit_r4 import analyze

        with tempfile.TemporaryDirectory(prefix="m-ofdft-g1-r4-final-replay-") as directory:
        ...
        errors.append(f"R4 final aggregate-science replay failed: {error}")
    return errors
...
def validate_supervisor_completion(
    ...
    *,
    ...
    try:
        ...
        analysis = _external_object(analysis_path, "R4 analysis summary")
        ...
        if require_committed:
            ...
            if (
                ...
                errors.append("completion parent is not the exact R4 analysis commit")
            ...
            first_marker_relative = (
                f"{ATTEMPT_LEDGER_ROOT.as_posix()}/{R4_AUDIT_IDS[0]}.json"
            )
            ...
            if first_marker_parent != go_head:
                errors.append(
                    "first R4 attempt marker parent is not the supervisor GO HEAD"
                )
    except (
        FileNotFoundError,
        KeyError,
...
def _barrier_spec(
    ...
    prefix = [
        str(python),
        "-s",
        str(project_root / "scripts/validate_s1_g1_thermodynamic_label_audit_r4.py"),
        str(project_root / MANIFEST_PATH),
        "--config",
        str(project_root / CONFIG_PATH),
        "--scientific-config",
    ...
    if barrier_name == "production-parser-registration-before-041":
        return None, None, prefix
    if barrier_name == "k-gate-after-052":
        return R4_PILOT_IDS[-1], R4_K_GATE_COMPLETION_LOGICAL_ID, [*prefix, "--require-k-gate"]
    if barrier_name == "final-all-after-080":
        return R4_AUDIT_IDS[-1], NEW_TO_LOGICAL[R4_AUDIT_IDS[-1]], [
            *prefix,
            "--require-all-runs",
        ]
    if barrier_name == "final-analysis":
        return R4_AUDIT_IDS[-1], NEW_TO_LOGICAL[R4_AUDIT_IDS[-1]], [
            str(python),
            "-s",
            str(project_root / "scripts/analyze_s1_g1_thermodynamic_label_audit_r4.py"),
    ...
    if per_run:
        ...
        if experiment_id not in R4_AUDIT_IDS:
    ...
    raise ValueError(f"unknown R4 barrier name: {barrier_name}")
...
def validate_barrier_failures(
    ...
    *,
    ...
    execution = config.get("execution")
    if not isinstance(execution, dict):
        return records, ["R4 execution contract is missing"]
    ...
    if require_committed:
        ...
        for introduction, relative in introductions:
            ...
            try:
                ...
                for experiment_id in R4_AUDIT_IDS:
                    marker_relative = (
...
def validate_attempt_marker(
    ...
    *,
    ...
    if experiment_id not in R4_AUDIT_IDS:
        return payload, commit, [f"attempt marker ID is outside R4: {experiment_id}"]
    execution = config.get("execution")
    if not isinstance(execution, dict):
        return payload, commit, ["R4 execution contract is missing"]
    ...
    try:
        ...
        if require_committed:
            ...
            if commit == prereg or not _is_ancestor(project_root, prereg, commit):
                errors.append(f"{experiment_id}: attempt marker is not after R4 preregistration")
...
def _validate_execution_history(
    ...
    active: dict[str, str] = {}
    events: dict[str, list[tuple[str, str]]] = {experiment_id: [] for experiment_id in R4_AUDIT_IDS}
    marker_commits: dict[str, str] = {}
    ledger = project_root / ATTEMPT_LEDGER_ROOT
    if ledger.exists():
        if not ledger.is_dir() or ledger.is_symlink():
            errors.append("R4 attempt ledger is not a regular directory")
        else:
            observed = {path.name for path in ledger.iterdir()}
            allowed = {f"{experiment_id}.json" for experiment_id in R4_AUDIT_IDS}
            unexpected = observed - allowed
            if unexpected:
                errors.append(f"R4 attempt ledger contains unexpected entries: {sorted(unexpected)}")
    for experiment_id in R4_AUDIT_IDS:
        ...
        if introduction:
            ...
            if not changed or any(not path.startswith(f"runs/{experiment_id}/") for path in changed):
                errors.append(f"R4 active run commit is not independently scoped: {experiment_id}")
        ...
        if run_commits and experiment_id not in marker_commits:
            errors.append(f"R4 run lacks its prior formal-attempt ledger commit: {experiment_id}")
        for run_commit in run_commits:
            if marker_commits.get(experiment_id) and str(
                ...
                errors.append(
                    f"R4 run introduction is not adjacent to its attempt marker: {experiment_id}"
                )
        if introduction and archives:
            errors.append(f"R4 forbids an active retry after archive: {experiment_id}")
        if len(archives) > 1:
            errors.append(f"R4 forbids multiple failed attempts for one ID: {experiment_id}")
        for label, commit in events[experiment_id]:
            if commit == prereg or not _is_ancestor(project_root, prereg, commit):
                errors.append(f"R4 execution event is not after preregistration: {experiment_id}/{label}")
        ...
        for label, commit in events[experiment_id]:
            for predecessor in EXECUTION_ORDER[:index]:
                ...
                if predecessor_commit is None:
                    errors.append(
                        f"R4 execution order violation: {experiment_id}/{label} precedes accepted {predecessor}"
                    )
                    continue
                if predecessor_commit == commit or not _is_ancestor(project_root, predecessor_commit, commit):
                    errors.append(
                        f"R4 execution ancestry violation: {predecessor} is not before {experiment_id}/{label}"
                    )
    for predecessor in sorted(required_accepted_predecessors):
        if predecessor not in active:
            continue
        status_failure = _status_failure_r4(
            project_root / "runs" / predecessor / STATUS_NAME,
            predecessor,
            accepted=True,
        )
        if status_failure:
            errors.append(f"R4 predecessor {predecessor}: {status_failure}")
        for name in (STATUS_NAME, EVIDENCE_NAME):
            relative = f"runs/{predecessor}/{name}"
            failure = _tracked_head_failure(project_root, relative)
            if failure:
                errors.append(f"R4 predecessor {predecessor}: {failure}")
        ...
        if evidence_path.is_file() and not evidence_path.is_symlink():
            try:
                ...
                if (
                    ...
                    errors.append(f"R4 predecessor accepted-evidence identity differs: {predecessor}")
            except (ValueError, json.JSONDecodeError) as error:
                errors.append(f"R4 predecessor evidence is invalid {predecessor}: {error}")
    if any(
        events[experiment_id]
        for experiment_id in EXECUTION_ORDER[len(R4_PILOT_IDS) :]
    ):
        try:
            gate = evaluate_k_gate(project_root, config, rows, require_committed=True)
            if gate.get("accepted") is not True:
                errors.append(
                    "R4 proceeded beyond the twelve-run P1 anchor set before the complete k gate passed"
                )
        except (FileNotFoundError, KeyError, TypeError, ValueError) as error:
            errors.append(f"R4 post-P1 k-gate barrier failed: {error}")
...
def validate_registration(
    ...
    *,
    ...
    if config_path.resolve() != (project_root / CONFIG_PATH).resolve():
        errors.append("R4 config must use its canonical project path")
    if manifest_path.resolve() != (project_root / MANIFEST_PATH).resolve():
        errors.append("R4 manifest must use its canonical project path")
    if not config_path.is_file() or config_path.is_symlink():
        raise ValueError("R4 thermodynamic-label config is missing or symbolic")
    if not manifest_path.is_file() or manifest_path.is_symlink():
        raise ValueError("R4 thermodynamic-label manifest is missing or symbolic")
    try:
    ...
    if require_committed:
        for relative in (str(CONFIG_PATH), str(MANIFEST_PATH)):
            failure = _tracked_head_failure(project_root, relative)
            if failure:
                errors.append(f"R4 preregistration {failure}")
        ...
        if (
            ...
            errors.append(
                "R4 revision is terminal after a preserved barrier failure"
            )
    ...
    if errors:
        raise ValueError(
            "S1 G1 thermodynamic-label R4 registration validation failed:\n- "
            + "\n- ".join(errors)
        )
...
def _status_failure_r4(path: Path, experiment_id: str, *, accepted: bool) -> str | None:
    if not path.is_file() or path.is_symlink():
    ...
    for key, value in common.items():
        if payload.get(key) != value:
            return f"R4 authoritative status {key} differs"
    if accepted:
        return None if payload == _status_payload(experiment_id, accepted=True) else "accepted R4 status differs"
    allowed = {
        *common,
        "status",
        "workflow_exit_code",
    ...
    if (
        ...
        return "failed R4 authoritative status model differs"
    ...
    return None if coherent else "failed R4 status stage/exit tuple differs"
...
def _read_json_for_r4_failure(path: Path) -> dict:
...
def _valid_pre_counterpart_runtime_failure(run: Path) -> bool:
    """Recognize the one fail-closed state before counterpart generation.

    The namespace launcher creates counterpart evidence only after its payload
    exits zero. A rejected runtime launcher exits earlier by construction, so
    requiring a counterpart file in this exact state is a false archive error.
    Every independent status layer and the absence of both counterpart files
    are checked before that one legacy error may be suppressed.
    """

    runtime_root = run / "mpi_runtime_audit"
    status = _regular_json_object(run / "run_status.json")
    audit = _regular_json_object(runtime_root / "audit.json")
    host = _regular_json_object(runtime_root / "namespace/host_status.json")
    payload = _regular_json_object(runtime_root / "namespace/payload_status.json")
    objects_path = runtime_root / "objects.tsv"
    counterpart_paths = (
        runtime_root / "counterpart_audit.json",
        runtime_root / "counterparts.tsv",
    )
    if not all(isinstance(value, dict) for value in (status, audit, host, payload)):
        return False
    if not objects_path.is_file() or objects_path.is_symlink():
        return False
    if any(path.exists() or path.is_symlink() for path in counterpart_paths):
        return False
    launcher_exit = status.get("launcher_exit_code")
    if (
        not isinstance(launcher_exit, int)
        or isinstance(launcher_exit, bool)
        or launcher_exit == 0
    ):
        return False
    ...
    return bool(
        ...
        and status.get("status") == "rejected"
        and status.get("failure_stage") == "runtime_invocation_or_parser"
        and status.get("workflow_exit_code") == launcher_exit
        and status.get("invocation_exit_code") == launcher_exit
        and status.get("runtime_audit_json_present") is True
        and status.get("runtime_audit_status") == "rejected"
        and status.get("namespace_host_status") == "rejected"
        and status.get("counterpart_audit_status") is None
        and audit.get("schema_version") == 2
        and audit.get("protocol") == "runtime_relocation_equivalence"
        and audit.get("status") == "rejected"
        and audit.get("launcher_exit_code") == launcher_exit
        and host.get("schema_version") == 1
        and host.get("status") == "rejected"
        and host.get("namespace_payload_exit_code") == launcher_exit
        and host.get("counterpart_audit") == {}
        and f"namespace_payload_exit_code:{launcher_exit}"
        in host.get("failure_reasons", [])
        and payload.get("schema_version") == 1
        and payload.get("status") == "audit_rejected"
        and payload.get("audit_launcher_exit_code") == launcher_exit
    )


def _stop_confirmation_proof_failures(run: Path, experiment_id: str) -> list[str]:
    """Require before/after stopped-state proof for every accepted MPI rank."""

    audit = _regular_json_object(run / "mpi_runtime_audit/audit.json")
    if not isinstance(audit, dict):
        return [f"{experiment_id}: R4 runtime audit is missing or invalid"]
    processes = audit.get("processes")
    if not isinstance(processes, list):
        return [f"{experiment_id}: R4 runtime process evidence is missing"]
    errors: list[str] = []
    observed_ranks: set[int] = set()
    for process in processes:
        if not isinstance(process, dict) or process.get("role") != "rank":
            continue
        rank = process.get("rank")
        if not isinstance(rank, int) or isinstance(rank, bool):
            errors.append(f"{experiment_id}: R4 runtime rank identity is invalid")
            continue
        if rank in observed_ranks:
            errors.append(f"{experiment_id}: duplicate R4 stop proof for rank {rank}")
            continue
        observed_ranks.add(rank)
        before = process.get("initial_map_capture_stop_state_before")
        after = process.get("initial_map_capture_stop_state_after")
        if (
            process.get("initial_map_capture_stop_confirmed") is not True
            or before not in {"T", "t"}
            or after not in {"T", "t"}
        ):
            errors.append(
                f"{experiment_id}: rank {rank} lacks bracketing SIGSTOP proof"
            )
    if observed_ranks != {0, 1, 2, 3}:
        errors.append(
            f"{experiment_id}: R4 stop-proof rank set differs: "
            f"{sorted(observed_ranks)}"
        )
    return errors


def validate_failed_r4_run(
    project_root: Path,
    config: dict,
    row: dict[str, str],
    *,
    ...
    if inventory_path.is_file() and not inventory_path.is_symlink():
        try:
            ...
            marker_errors.append(f"{row['experiment_id']}: invalid R4 failure inventory: {error}")
    with _r4_failure_policy():
    ...
    legacy_counterpart_error = (
        f"{row['experiment_id']}: runtime evidence claim lacks "
        "mpi_runtime_audit/counterpart_audit.json"
    )
    if (
        legacy_counterpart_error in failures
        and _valid_pre_counterpart_runtime_failure(run)
    ):
        failures = [failure for failure in failures if failure != legacy_counterpart_error]
...
def replay_evidence(
    ...
    *,
    ...
    errors.extend(
        _stop_confirmation_proof_failures(run, experiment_id)
    )
    payload: dict[str, object] = {}
    try:
        ...
        if labels != reparsed:
            errors.append(f"{experiment_id}: thermodynamic labels differ from R4 recomputation")
        ...
        if require_replay_status:
            status_error = _status_failure_r4(run / STATUS_NAME, experiment_id, accepted=True)
            if status_error:
        ...
        if evidence.exists() or evidence.is_symlink():
            if not evidence.is_file() or evidence.is_symlink():
                errors.append(f"{experiment_id}: R4 evidence is not a regular file")
            elif read_json(evidence) != payload:
                errors.append(f"{experiment_id}: R4 evidence differs from recomputation")
            elif require_committed:
        ...
        errors.append(f"{experiment_id}: R4 run evidence validation failed: {error}")
    return payload, errors
...
# A descriptive compatibility alias for early R4 callers.  The analyzer-facing
...
def _write_evidence(path: Path, payload: dict[str, object]) -> None:
    if path.exists() or path.is_symlink():
        raise ValueError(f"refusing to overwrite R4 evidence: {path}")
...
def _write_exclusive_json(path: Path, payload: dict[str, object]) -> None:
    if path.exists() or path.is_symlink():
        raise ValueError(f"refusing to overwrite R4 failure evidence: {path}")
...
def evaluate_pilot_gate(
    ...
    *,
    ...
    return {
        **result,
        "pilot_ids": list(R4_PILOT_IDS),
        "gate_semantics": "all_new_r4_complete_dense_k_anchor_set",
    }
...
def evaluate_half_quarter_pair(
    ...
    *,
    ...
    quarter_logical = (
        ...
        if quarter_experiment_id in R4_AUDIT_IDS
...
def main() -> int:
    ...
    if args.check_detachment_attestation or args.check_detachment_attestation_record:
        ...
        if failures:
            raise ValueError(
                "R4 detachment-attestation validation failed:\n- "
                + "\n- ".join(failures)
            )
    if args.check_attempt_marker:
        ...
        if failures:
            raise ValueError("R4 attempt-marker validation failed:\n- " + "\n- ".join(failures))
        if args.require_committed and marker_commit is None:
            raise ValueError(f"R4 attempt marker is not committed: {experiment_id}")
    ...
    if args.check_analysis_summary:
        ...
        if failures:
            raise ValueError(
                "R4 pre-completion analysis validation failed:\n- "
                + "\n- ".join(failures)
            )
    if args.require_supervisor_completion:
        ...
        if failures:
            raise ValueError(
                "R4 supervisor-completion validation failed:\n- "
                + "\n- ".join(failures)
            )
        ...
        if replay_failures:
            raise ValueError(
                "R4 final aggregate-science replay failed:\n- "
                + "\n- ".join(replay_failures)
            )
    if per_run:
        experiment_id = per_run[0]
        if experiment_id not in by_id:
            raise ValueError(f"requested run is outside the R4 manifest: {experiment_id}")
        ...
        if args.check_failure_archives:
            ...
            if failures:
                raise ValueError("R4 failed-archive validation failed:\n- " + "\n- ".join(failures))
        elif args.check_failure_run:
            failures = validate_failed_r4_run(
                project_root, config, row, require_committed=args.require_committed
            )
            if failures:
                raise ValueError("R4 failed-run validation failed:\n- " + "\n- ".join(failures))
        else:
            ...
            if failures:
                ...
                raise ValueError("R4 run validation failed:\n- " + "\n- ".join(failures))
    ...
    if args.require_half_quarter_pair:
        ...
        logical = logical_id(requested) if requested in R4_AUDIT_IDS else requested
        ...
        if half_quarter["accepted"] is not True:
            raise ValueError("R4 half-quarter field gate rejected: " + json.dumps(half_quarter, sort_keys=True))
    ...
    if args.require_adjacent_eos:
        ...
        if adjacent["accepted"] is not True:
            raise ValueError("R4 adjacent EOS gate rejected: " + json.dumps(adjacent, sort_keys=True))
    ...
    if args.require_pilot_gate:
        ...
        if pilot["accepted"] is not True:
            raise ValueError("R4 recovery pilot gate rejected: " + json.dumps(pilot, sort_keys=True))
    ...
    if args.require_k_gate:
        ...
        if k_gate["accepted"] is not True:
            raise ValueError("R4 complete low-smearing k gate rejected: " + json.dumps(k_gate, sort_keys=True))
    ...
    if args.require_all_runs:
        for logical in LOGICAL_IDS:
            ...
            if failures:
                raise ValueError(
                    f"R4 all-run validation failed for {logical}/{effective_id(logical)}:\n- "
                    + "\n- ".join(failures)
                )
