...
from s2_g2_al1_pilot_common_r1 import build_analysis, git, load_config, render_outputs, require
...
def main() -> int:
    ...
    require(git(root, "show", "-s", "--format=%P", head).split() == [config["implementation_commit"]], "analysis HEAD is not the config-only preregistration")
    ...
    print(json.dumps({
        ...
        "accepted_candidates": analysis["summary"]["accepted_candidates"],
