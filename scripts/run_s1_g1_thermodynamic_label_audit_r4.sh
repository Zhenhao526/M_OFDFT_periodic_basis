...
: "${M_OFDFT_G1_R4_SUPERVISOR_STATE_DIRECTORY:?missing supervisor state binding}"
: "${M_OFDFT_G1_R4_SUPERVISOR_PID:?missing supervisor PID binding}"
: "${M_OFDFT_G1_R4_SUPERVISOR_START_TIME_TICKS:?missing supervisor start-time binding}"
: "${M_OFDFT_G1_R4_BOOT_ID:?missing supervisor boot binding}"
: "${M_OFDFT_G1_R4_LAUNCH_SHA256:?missing supervisor launch binding}"
: "${M_OFDFT_G1_R4_GO_SHA256:?missing supervisor GO binding}"
...
PROTOCOL = "S1-G1-THERMODYNAMIC-LABEL-AUDIT-R4"
RUNNER_RELATIVE = "scripts/run_s1_g1_thermodynamic_label_audit_r4.sh"
MANIFEST_RELATIVE = "config/S1_g1_thermodynamic_label_audit_r4_manifest.tsv"
CONFIG_RELATIVE = "config/S1_g1_thermodynamic_label_audit_r4.json"
...
if git("status", "--porcelain=v1", "--untracked-files=all"):
    raise SystemExit("refusing thermodynamic-label R4 audit from a dirty worktree")
...
if [[ ${#registered_paths[@]} -ne 7 ]]; then
    echo "R4 execution path/GO registration is incomplete" >&2
    exit 2
fi
...
parser="$project_root/scripts/parse_s1_g1_thermodynamic_labels_r4.py"
analyzer="$project_root/scripts/analyze_s1_g1_thermodynamic_label_audit_r4.py"
analysis_directory="$project_root/analysis/s1/g1_thermodynamic_label_audit_r4_20260807"
...
logical_by_effective = {
    ...
    for row in matrix
    if row["evidence_origin"] == "r4_executed"
}
...
record_gate_failure() {
    ...
    if [[ ! "$gate_id" =~ ^[a-z0-9][a-z0-9-]*$ ]]; then
        echo "Invalid R4 gate identifier: $gate_id" >&2
        exit 98
    fi
    if [[ $gate_status -eq 0 ]]; then
        echo "Refusing zero exit code for failed R4 gate: $gate_id" >&2
        exit 98
    fi
...
PY
    ...
    git commit -m "record failed G1 thermodynamic-label R4 barrier $gate_id"
    assert_commit_scope "$failure_relative"
    update_external_state "$after_effective_id" "$after_logical_id" "barrier_failed:$gate_id"
    echo "STOP: R4 barrier failure committed: $gate_id" >&2
    exit 97
}
...
PY
    ...
    git commit -m "start G1 thermodynamic-label audit R4 $experiment_id"
...
path = Path(sys.argv[1]) / "thermodynamic_label_status_r4.json"
...
from validate_s1_g1_thermodynamic_label_audit_r4 import classify_noncore_failure

run = Path(sys.argv[1])
payload = classify_noncore_failure(
    run, sys.argv[2], sys.argv[3], sys.argv[4], int(sys.argv[5]), Path(sys.argv[6])
)
with (run / "thermodynamic_label_failure_classification_r4.json").open(
    "x", encoding="utf-8"
) as handle:
    json.dump(payload, handle, indent=2, sort_keys=True)
    handle.write("\n")
...
run = Path(sys.argv[1])
output = run / "thermodynamic_label_failure_artifact_inventory_r4.json"
files = []
...
commit_run() {
    ...
    git commit -m "record G1 thermodynamic-label audit R4 $experiment_id"
...
archive_and_stop() {
    ...
    git commit -m "archive failed G1 thermodynamic-label audit R4 $experiment_id"
    ...
    echo "STOP $experiment_id: failed formal R4 attempt committed and archived; retry forbidden" >&2
    exit 97
}
...
run_gate production-parser-registration-before-041 null null \
    "$python_tool" -s "$validator" "$canonical_manifest" --config "$canonical_config" \
    --scientific-config "$frozen_config" --scientific-manifest "$frozen_manifest" \
    --require-committed
...
for ((offset = 0; offset < ${#run_plan[@]}; offset += 6)); do
    ...
    if [[ -e "$run_directory" ]]; then
        echo "STOP $experiment_id: run path already exists; single-use R4 execution cannot resume" >&2
        exit 97
    fi
    ...
    if [[ $workflow_status -eq 0 && $label_parser_status -eq 0 && $core_status -eq 0 ]]; then
        ...
        failure_decision=$(
            "$python_tool" -s - "$run_directory/thermodynamic_label_failure_classification_r4.json" <<'PY'
import json
import sys
...
PY
    ...
    if [[ "$experiment_id" == S1-20260807-052 ]]; then
        run_gate k-gate-after-052 "$experiment_id" "$logical_id" \
            "$python_tool" -s "$validator" "$canonical_manifest" --config "$canonical_config" \
            --scientific-config "$frozen_config" --scientific-manifest "$frozen_manifest" \
...
run_gate final-all-after-080 S1-20260807-080 S1-20260806-033 \
    "$python_tool" -s "$validator" "$canonical_manifest" --config "$canonical_config" \
    --scientific-config "$frozen_config" --scientific-manifest "$frozen_manifest" \
...
if [[ -d "$analysis_directory" ]]; then
    ...
    git commit -m "record G1 thermodynamic-label audit R4 analysis"
    assert_commit_scope "analysis/s1/g1_thermodynamic_label_audit_r4_20260807/"
fi
if [[ $analysis_exit -ne 0 ]]; then
    record_gate_failure final-analysis S1-20260807-080 S1-20260806-033 \
        "$analysis_exit" "${analysis_argv[@]}"
fi
run_gate final-analysis-status S1-20260807-080 S1-20260806-033 \
    "$python_tool" -s "$validator" "$canonical_manifest" --config "$canonical_config" \
    --scientific-config "$frozen_config" --scientific-manifest "$frozen_manifest" \
