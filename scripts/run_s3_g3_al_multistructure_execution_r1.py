...
import numpy as np
from s3_g3_al_multistructure_common_r1 import CONFIG_REL,canonical,git,load,replay_result,require,resource_preflight,sha_bytes,sha_path,source_exact,validate_config,validate_runtime,write_exclusive
...
def command(root,c,row,run):return ["/usr/bin/taskset","-c",str(c["runtime"]["logical_cpu"]),c["runtime"]["python"],"-s","-B",str(root/"scripts/run_s3_g3_al_multistructure_worker_r1.py"),"--project-root",str(root),"--experiment-id",row["experiment_id"],"--run-directory",str(run)]
