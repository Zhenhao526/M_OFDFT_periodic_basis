...
from s3_g3_al_multistructure_common_r1 import CONFIG_REL,canonical,git,load,require,resource_preflight,sha_path,source_exact,validate_config,write_exclusive
...
def main():
 ...
 try: bootstrap.mkdir(parents=True,exist_ok=False);(bootstrap/"runs").mkdir();write_exclusive(bootstrap/"session.json",canonical(session));os.rename(bootstrap,state)
  ...
  raise
 session_sha=sha_path(state/"session.json");env={"HOME":os.environ["HOME"],"PATH":os.environ["PATH"],"PYTHONPATH":str(root/"scripts"),"OMP_NUM_THREADS":"1","MKL_NUM_THREADS":"1","OPENBLAS_NUM_THREADS":"1","PYTHONDONTWRITEBYTECODE":"1","PYTHONNOUSERSITE":"1"};accepted=[];result_sha={};returns={};attempts={};inventories={}
 def freeze(index,row,run,rc,reason):
  ...
  if not (run/"runner_return.json").exists():write_exclusive(run/"runner_return.json",canonical({"schema_version":1,"experiment_id":row["experiment_id"],"return_code":rc,"reason":reason}))
 ...
 for index,row in enumerate(c["formal_cases"],1):
  ...
  try:
   run.mkdir();write_exclusive(run/"attempt.json",canonical({"schema_version":1,"status":"formal_started","experiment_id":row["experiment_id"],"attempt_number":1,"started_unix_ns":time.time_ns()}));attempts[row["experiment_id"]]=sha_path(run/"attempt.json");cmd=command(root,c,row,run);write_exclusive(run/"command.json",canonical({"argv":cmd,"cwd":str(root),"environment":env}));write_exclusive(run/"resource_preflight.json",canonical(resource_preflight(c)));proc=subprocess.run(cmd,cwd=root,env=env,text=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE);write_exclusive(run/"stdout.txt",proc.stdout.encode());write_exclusive(run/"stderr.txt",proc.stderr.encode());write_exclusive(run/"runner_return.json",canonical({"schema_version":1,"experiment_id":row["experiment_id"],"return_code":proc.returncode}));returns[row["experiment_id"]]=sha_path(run/"runner_return.json")
   if proc.returncode:return freeze(index,row,run,proc.returncode,"worker_nonzero_return")
   result=json.loads(proc.stdout.strip().splitlines()[-1]);require(result["experiment_id"]==row["experiment_id"] and result["status"] in {"accepted","completed_scientific_rejected"},"worker result differs");write_exclusive(run/"result.json",canonical(result));accepted.append(row["experiment_id"]);result_sha[row["experiment_id"]]=sha_path(run/"result.json");inventories[row["experiment_id"]]=inventory(run);best_effort_progress({"completed":index,"total":total,"experiment_id":row["experiment_id"],"scientific_status":result["status"],"wall_seconds":result["wall_seconds"]})
  except BaseException as exc:
   if not (run/"stderr.txt").exists():run.mkdir(exist_ok=True);write_exclusive(run/"stderr.txt",traceback.format_exc().encode())
   return freeze(index,row,run,70,f"runner_exception:{type(exc).__name__}")
...
def command(root,c,row,run):return ["taskset","-c",str(c["runtime"]["logical_cpu"]),c["runtime"]["python"],"-s","-B",str(root/"scripts/run_s3_g3_al_multistructure_worker_r1.py"),"--project-root",str(root),"--experiment-id",row["experiment_id"],"--run-directory",str(run)]
def inventory(run):return {p.name:{"sha256":sha_path(p),"size_bytes":p.stat().st_size} for p in sorted(run.iterdir()) if p.is_file() and not p.is_symlink()}
