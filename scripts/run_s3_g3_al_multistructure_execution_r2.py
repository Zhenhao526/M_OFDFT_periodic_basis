...
import numpy as np
from s3_g3_al_multistructure_common_r2 import CONFIG_REL,canonical,git,load,replay_result,require,resource_preflight,sha_bytes,sha_path,source_exact,validate_config,validate_runtime,write_exclusive
...
def main():
 ...
 for index,row in enumerate(c["formal_cases"],1):
  ...
  try:
   ...
   result=json.loads(proc.stdout.strip().splitlines()[-1]);require(result["experiment_id"]==row["experiment_id"] and result["status"] in {"accepted","completed_scientific_rejected"},"worker result differs");require(result["cpu_affinity"]==[c["runtime"]["logical_cpu"]] and result["runtime"]==runtime,"worker runtime differs");density=np.load(run/"density.npy",allow_pickle=False);require(sha_path(run/"density.npy")==result["density_sha256"],"worker density SHA differs");replay_result(root,c,row,result,density);result_bytes=canonical(result);observed_inventory=inventory(run);observed_inventory["result.json"]={"sha256":sha_bytes(result_bytes),"size_bytes":len(result_bytes)};write_exclusive(run/"result.json",result_bytes);result_sha[row["experiment_id"]]=observed_inventory["result.json"]["sha256"];inventories[row["experiment_id"]]=observed_inventory;accepted.append(row["experiment_id"]);best_effort_progress({"completed":index,"total":total,"experiment_id":row["experiment_id"],"scientific_status":result["status"],"wall_seconds":result["wall_seconds"]})
...
def command(root,c,row,run):return ["/usr/bin/taskset","-c",str(c["runtime"]["logical_cpu"]),c["runtime"]["python"],"-s","-B",str(root/"scripts/run_s3_g3_al_multistructure_worker_r2.py"),"--project-root",str(root),"--experiment-id",row["experiment_id"],"--run-directory",str(run)]
