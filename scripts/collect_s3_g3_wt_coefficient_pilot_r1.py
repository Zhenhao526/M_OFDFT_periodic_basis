...
def failure_outputs(root,c,terminal,state,runtime):
    failure=json.loads((state/"failure.json").read_text()); require(sha_path(state/"failure.json")==terminal["failure_sha256"],"failure SHA differs"); require(failure["status"]=="failed_no_retry" and failure["experiment_id"]==terminal["attempted_ids"][-1] and failure["attempted_ids"]==terminal["attempted_ids"] and failure["accepted_ids"]==terminal["accepted_ids"] and failure["unattempted_ids"]==terminal["unattempted_ids"] and failure["return_code"]==terminal["runner_return_code"],"failure identity differs")
    ...
    for index,row in enumerate(c["formal_cases"][:terminal["attempted"]],1):
        ...
        if not accepted: require(ret["return_code"]==failure["return_code"]==terminal["runner_return_code"],"failed run return code differs")
