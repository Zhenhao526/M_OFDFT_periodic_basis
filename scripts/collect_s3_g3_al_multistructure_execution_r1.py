...
import numpy as np
from s3_g3_al_multistructure_common_r1 import CONFIG_REL,canonical,git,load,render,replay_result,require,resource_preflight,sha_path,source_exact,validate_config,validate_runtime,write_exclusive
from run_s3_g3_al_multistructure_execution_r1 import command,environment
...
def envelope(root,c,row,run,t):
 ...
 require(cmd=={"argv":command(root,c,row,run),"cwd":str(root),"environment":environment(root,c)},"command differs");require(pre in ({"status":"accepted","hostname":c["runtime"]["hostname"],"logical_cpu":c["runtime"]["logical_cpu"],"smt_domain":c["runtime"]["smt_domain"],"collisions":[]},{"status":"failed","error":"runner_exception"}),"preflight differs")
...
def load_rows(root,c,state,t,rows):
 ...
 for index,row in enumerate(rows,1):
  run=state/"runs"/f"{index:03d}_{row['experiment_id']}";ret=envelope(root,c,row,run,t);require(ret["return_code"]==0,"accepted return differs");result,density=replay_run_result(root,c,row,run,t,True);runs.append(result);densities[row["experiment_id"]]=density
