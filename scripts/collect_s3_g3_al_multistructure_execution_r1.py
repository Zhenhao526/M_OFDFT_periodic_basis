...
def envelope(root,c,row,run,t,accepted):
 ...
 require(ret["schema_version"]==1 and ret["experiment_id"]==eid and isinstance(ret["return_code"],int) and set(ret) in ({"schema_version","experiment_id","return_code"},{"schema_version","experiment_id","return_code","reason"}) and sha_path(run/"runner_return.json")==t["runner_return_sha256"][eid],"return differs");return ret
...
def load_rows(root,c,state,t,rows):
 ...
 for index,row in enumerate(rows,1):
  run=state/"runs"/f"{index:03d}_{row['experiment_id']}";ret=envelope(root,c,row,run,t,True);require(ret["return_code"]==0,"accepted return differs");result,density=replay_run_result(root,c,row,run,t,True);runs.append(result);densities[row["experiment_id"]]=density
...
def failure_output(root,c,state,t):
 ...
 for index,row in enumerate(c["formal_cases"][:t["attempted"]],1):
  accepted=row["experiment_id"] in t["accepted_ids"];run=state/"runs"/f"{index:03d}_{row['experiment_id']}";ret=envelope(root,c,row,run,t,accepted);require((run/"result.json").exists()==accepted,"failure result denominator differs")
