...
CONFIG_REL=Path("config/S3_g3_al_multistructure_execution_r2.json")
MATRIX_REL=Path("config/S3_g3_al_multistructure_matrix_r1.json")
PROTOCOL_REL=Path("docs/S3_G3_AL_MULTISTRUCTURE_EXECUTION_R2_PROTOCOL.md")
COMMON_REL=Path("scripts/s3_g3_al_multistructure_common_r2.py")
WORKER_REL=Path("scripts/run_s3_g3_al_multistructure_worker_r2.py")
RUNNER_REL=Path("scripts/run_s3_g3_al_multistructure_execution_r2.py")
COLLECTOR_REL=Path("scripts/collect_s3_g3_al_multistructure_execution_r2.py")
VALIDATOR_REL=Path("scripts/validate_s3_g3_al_multistructure_execution_r2.py")
TEST_REL=Path("tests/test_s3_g3_al_multistructure_execution_r2.py")
...
def validate_config(root,c):
 e=execution_config(root);m=matrix_config(root,e);require(e["schema_version"]==1 and e["protocol_revision"]=="S3-G3-AL-MULTISTRUCTURE-EXECUTION-20260812-R2","execution schema differs")
 require(e["base_commit"]=="21fb0db1aae023f6c53b0e7dd2b9205c51b85223","base differs")
 ...
 cases=formal_cases(m);require(len(cases)==80 and [x["experiment_id"] for x in cases]==[f"S3-20260812-{i:03d}" for i in range(201,281)],"case denominator differs")
...
def _registered_execution_process(comm,tokens):
 names={"run_s3_g3_al_multistructure_execution_r2.py","run_s3_g3_al_multistructure_worker_r2.py"}
...
def replay_result(root,c,row,result,density):
 ...
 target_cell=cell_for_row(root,c,row);original=legacy.load_source_density;legacy.load_source_density=lambda _root,_c:(target_cell,None)
