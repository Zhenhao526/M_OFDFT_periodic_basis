...
CONFIG_REL=Path("config/S3_g3_al_multistructure_execution_r3.json")
MATRIX_REL=Path("config/S3_g3_al_multistructure_matrix_r1.json")
PROTOCOL_REL=Path("docs/S3_G3_AL_MULTISTRUCTURE_EXECUTION_R3_PROTOCOL.md")
COMMON_REL=Path("scripts/s3_g3_al_multistructure_common_r3.py")
WORKER_REL=Path("scripts/run_s3_g3_al_multistructure_worker_r3.py")
RUNNER_REL=Path("scripts/run_s3_g3_al_multistructure_execution_r3.py")
COLLECTOR_REL=Path("scripts/collect_s3_g3_al_multistructure_execution_r3.py")
VALIDATOR_REL=Path("scripts/validate_s3_g3_al_multistructure_execution_r3.py")
TEST_REL=Path("tests/test_s3_g3_al_multistructure_execution_r3.py")
...
def formal_cases(matrix):
 ...
 for structure in matrix["structures"]:
  ...
  out.append({**base,"experiment_id":f"S3-20260813-{n:03d}","route":"full_grid_WT_reference","initialization":"uniform"});n+=1
  for init in matrix["coefficient_initializations"]:
   out.append({**base,"experiment_id":f"S3-20260813-{n:03d}","route":"coefficient_23_function","initialization":init});n+=1
 return out
...
def validate_config(root,c):
 e=execution_config(root);m=matrix_config(root,e);require(e["schema_version"]==1 and e["protocol_revision"]=="S3-G3-AL-MULTISTRUCTURE-EXECUTION-20260813-R3","execution schema differs")
 require(e["base_commit"]=="46f6505ff4602ee6153b11645d5a41bcb5a9a2d5","base differs")
 r2_identity={"closure_commit":"46f6505ff4602ee6153b11645d5a41bcb5a9a2d5","closure_path":"analysis/s3/g3_al_multistructure_execution_r2_20260812/operational_failure_closure.json","closure_sha256":"9107896f83cc68fbf4a910be6f5d425abd734c6804adf9712cd2d217a445c6b1","terminal_sha256":"2b96200e94604872b22b64a93385bcf69687bbb7e01b3e182107ac4cd9641fc9","failure_sha256":"dfeb87589bf2246f1643cbb07c8ca7d37d976d6b526cc2e29951cbf616e6f936","accepted_prefix_count":28,"failed_experiment_id":"S3-20260812-229","disposition":"operational_failure_closed_no_retry"}
 require(e["r2_operational_failure"]==r2_identity,"R2 failure identity differs");r2_path=root/e["r2_operational_failure"]["closure_path"];require(sha_path(r2_path)==e["r2_operational_failure"]["closure_sha256"] and _git_bytes(root,e["r2_operational_failure"]["closure_commit"],e["r2_operational_failure"]["closure_path"])==r2_path.read_bytes(),"R2 closure bytes differ");r2=json.loads(r2_path.read_text());require(r2["status"]==e["r2_operational_failure"]["disposition"] and r2["denominator"]["accepted_prefix"]==28 and r2["denominator"]["failed_experiment_id"]==e["r2_operational_failure"]["failed_experiment_id"] and r2["state_inventory"]["terminal_sha256"]==e["r2_operational_failure"]["terminal_sha256"] and r2["state_inventory"]["failure_sha256"]==e["r2_operational_failure"]["failure_sha256"],"R2 closure content differs")
 failure_identity={"evidence_commit":"21fb0db1aae023f6c53b0e7dd2b9205c51b85223","summary_path":"analysis/s3/g3_al_multistructure_execution_r1_20260812/summary.json","summary_sha256":"0916dae0fa42912751cc7294a66f47595fe24006f07c82aa50971d1797125b96","terminal_sha256":"a5be1e6935e828fbb7678939e963cbffbcfb26eb3c4bb165bb87ee2deec8784b","failure_sha256":"03da310f0b9bf7094a20c4ca62b13a16ac54838d89fea75d4a26249db7e3c4a9","disposition":"evidence_valid_operational_failure_no_retry","accepted_prefix_count":13,"failed_experiment_id":"S3-20260812-114","failure_reason":"monitor_shell_false_positive_collision"}
 require(e["r1_operational_failure"]==failure_identity,"R1 failure identity differs");failure_path=root/e["r1_operational_failure"]["summary_path"];require(sha_path(failure_path)==e["r1_operational_failure"]["summary_sha256"] and _git_bytes(root,e["r1_operational_failure"]["evidence_commit"],e["r1_operational_failure"]["summary_path"])==failure_path.read_bytes(),"R1 failure evidence bytes differ");failure=json.loads(failure_path.read_text());require(failure["status"]==e["r1_operational_failure"]["disposition"] and failure["accepted_prefix_count"]==13 and sha_bytes(canonical(failure["terminal"]))==e["r1_operational_failure"]["terminal_sha256"] and failure["terminal"]["failure_sha256"]==e["r1_operational_failure"]["failure_sha256"] and failure["terminal"]["failed"]==1 and failure["terminal"]["accepted"]==13 and failure["terminal"]["attempted"]==14 and failure["terminal"]["attempted_ids"][-1]==e["r1_operational_failure"]["failed_experiment_id"],"R1 failure evidence differs")
 require(e["matrix"]=={"commit":"f979447b596ecb30879fd72e2e1b1360be297aaf","path":"config/S3_g3_al_multistructure_matrix_r1.json","sha256":"66d36fca5f814374e8ecd83f3b589b8c42c2e405d441b695bfff6fd86b0df2b2"},"matrix identity differs")
 require(e["r1_scientific_source"]=={"preregistration_commit":"b0d596dbe22d5a66b0eb13721860444c12bb77c3","config_path":"config/S3_g3_wt_coefficient_pilot_r1.json","config_sha256":"1502418cdaf65c34d05c18b117125f9964d81f11d632deca0ab8817fe3c06069"},"R1 science identity differs")
 ...
 cases=formal_cases(m);require(len(cases)==80 and [x["experiment_id"] for x in cases]==[f"S3-20260813-{i:03d}" for i in range(301,381)],"case denominator differs")
...
def _registered_execution_process(comm,tokens):
 names={"run_s3_g3_al_multistructure_execution_r3.py","run_s3_g3_al_multistructure_worker_r3.py"}
...
def render(c,runs):
 ...
 readme=(f"# S3/G3 Al twenty-structure fixed-WT coefficient execution R2\n\nStatus: `{s['status']}`. Eighty fresh cases cover twenty registered geometries. Full-grid Euler residuals are diagnostic. G3 remains open; S4 is unauthorized.\n").encode();return {"README.md":readme,"runs.json":canonical({"schema_version":1,"protocol_revision":c["protocol_revision"],"runs":runs}),"gates.tsv":('\n'.join(lines)+'\n').encode(),"summary.json":canonical(s)}
