#!/usr/bin/env python3
"""Validate the fail-closed S1-G1 thermodynamic-label audit R2.

R2 is a continuation, not a reinterpretation of R1.  Ten immutable accepted
R1 runs are replayed with the frozen R1 validator.  Thirty new experiment IDs
complete the same forty logical scientific slots.  Every scientific gate in
this module resolves a logical slot explicitly; no gate infers a partner from
an R2 experiment-number offset.
"""
...
import generate_s1_g1_thermodynamic_label_audit_r2 as registration
...
from parse_s1_g1_thermodynamic_labels_r2 import parse_run as parse_label_run
...
R2_AUDIT_IDS = tuple(registration.R2_AUDIT_IDS)
...
EVIDENCE_NAME = "g1_thermodynamic_label_audit_r2.json"
STATUS_NAME = "thermodynamic_label_status_r2.json"
FAILURE_CLASS_NAME = "thermodynamic_label_failure_classification_r2.json"
FAILURE_INVENTORY_NAME = "thermodynamic_label_failure_artifact_inventory_r2.json"
LABEL_NAME = r1.LABEL_NAME
ANALYSIS_ROOT = Path("analysis/s1/g1_thermodynamic_label_audit_r2_20260806")
ANALYSIS_SUMMARY_PATH = ANALYSIS_ROOT / "summary.json"

EXPECTED_REUSED_LOGICAL_IDS = (
    "S1-20260806-024",
    "S1-20260806-036",
    "S1-20260806-031",
    "S1-20260806-039",
...
EXPECTED_NEW_TO_LOGICAL = {
    "S1-20260806-041": "S1-20260806-034",
    "S1-20260806-042": "S1-20260806-040",
    **{
        f"S1-20260806-{new:03d}": f"S1-20260806-{logical:03d}"
        for new, logical in zip(range(43, 49), range(1, 7))
    },
    **{
        f"S1-20260806-{new:03d}": f"S1-20260806-{logical:03d}"
        for new, logical in zip(range(49, 63), range(7, 21))
    },
    "S1-20260806-063": "S1-20260806-022",
    "S1-20260806-064": "S1-20260806-023",
    "S1-20260806-065": "S1-20260806-025",
    "S1-20260806-066": "S1-20260806-026",
    "S1-20260806-067": "S1-20260806-029",
    "S1-20260806-068": "S1-20260806-030",
    "S1-20260806-069": "S1-20260806-032",
...
R2_PILOT_IDS = ("S1-20260806-041", "S1-20260806-042")
R2_PILOT_LOGICAL_PAIR = ("S1-20260806-034", "S1-20260806-040")
...
def _assert_static_namespace() -> None:
    if R1_REUSED_AUDIT_IDS != EXPECTED_REUSED_LOGICAL_IDS:
        raise RuntimeError("generator changed the frozen ten-run R1 reuse set")
    if NEW_TO_LOGICAL != EXPECTED_NEW_TO_LOGICAL:
        raise RuntimeError("generator changed the frozen R2-to-logical mapping")
    if R2_AUDIT_IDS != tuple(EXPECTED_NEW_TO_LOGICAL):
        raise RuntimeError("R2 physical ID order differs from 041--070")
    expected_effective = {
        **{logical: logical for logical in EXPECTED_REUSED_LOGICAL_IDS},
    ...
    if EXECUTION_ORDER != R2_AUDIT_IDS:
        raise RuntimeError("R2 execution order must be the exact 041--070 sequence")
...
def logical_id(experiment_id: str) -> str:
    ...
    try:
        ...
        raise ValueError(f"unknown effective R2 experiment ID: {experiment_id}") from error


def source_kind(logical_experiment_id: str) -> str:
    return "r1_reused" if logical_experiment_id in R1_REUSED_AUDIT_IDS else "r2_executed"
...
def row_for_logical(
    ...
    try:
        ...
        raise ValueError(f"R2 manifest lacks logical slot {logical_experiment_id}") from error
...
def _validate_r1_bridge(
    ...
    *,
    ...
    try:
        ...
        if bridge.get("reused_logical_ids") != list(R1_REUSED_AUDIT_IDS):
            errors.append("R1 bridge reuse order differs from the accepted R1 prefix")
        reused = bridge.get("reused_runs")
        if not isinstance(reused, dict) or set(reused) != set(R1_REUSED_AUDIT_IDS):
            errors.append("R1 bridge reused-run denominator differs")
            reused = {}
        ...
        for experiment_id in R1_REUSED_AUDIT_IDS:
            ...
            if anchor.get("introduction_commit") != introduction:
                errors.append(f"R1 reuse introduction commit differs: {experiment_id}")
            if not _is_ancestor(project_root, R1_PREREGISTRATION_COMMIT, introduction):
                errors.append(f"R1 reuse predates R1 preregistration: {experiment_id}")
            if not _is_ancestor(project_root, introduction, R1_FAILURE_COMMIT) or introduction == R1_FAILURE_COMMIT:
                errors.append(f"R1 reuse is not before the frozen R1 failure: {experiment_id}")
            if anchor.get("tree_oid") != _tree_oid(project_root, "HEAD", f"runs/{experiment_id}"):
                errors.append(f"R1 reuse tree differs at HEAD: {experiment_id}")
            ...
            if not changed or any(not path.startswith(prefix) for path in changed):
                errors.append(f"R1 reused run commit is not independently scoped: {experiment_id}")
            artifacts = anchor.get("artifacts")
            if not isinstance(artifacts, dict) or not artifacts:
                errors.append(f"R1 reuse artifact anchor is missing: {experiment_id}")
            else:
                for artifact_name, artifact in artifacts.items():
                    if not isinstance(artifact_name, str) or not isinstance(artifact, dict):
                        errors.append(f"R1 reuse artifact anchor is malformed: {experiment_id}")
                        continue
                    relative = artifact.get("path")
                    if not isinstance(relative, str):
                        errors.append(f"R1 reuse artifact path is missing: {experiment_id}/{artifact_name}")
                        continue
                    failure = _tracked_head_failure(project_root, relative)
                    if failure:
                        errors.append(f"R1 reuse {experiment_id}: {failure}")
                    elif artifact.get("sha256") != sha256(project_root / relative):
                        errors.append(f"R1 reuse artifact SHA-256 differs: {experiment_id}/{artifact_name}")
        if len(set(introduction_commits)) != len(R1_REUSED_AUDIT_IDS):
            errors.append("R1 reused runs do not have ten distinct introduction commits")
        for earlier, later in zip(introduction_commits, introduction_commits[1:]):
            if not _is_ancestor(project_root, earlier, later) or earlier == later:
                errors.append("R1 reused-run commit ancestry differs from frozen execution order")
        ...
        forbidden = set(LOGICAL_IDS) - set(R1_REUSED_AUDIT_IDS) - {R1_FAILURE_ID}
        ...
        if require_committed:
            ...
            reused_details: dict[str, object] = {}
            for experiment_id in R1_REUSED_AUDIT_IDS:
                payload, failures = r1.replay_evidence(
                    project_root,
                    r1_config,
                    by_r1[experiment_id],
                    require_committed=True,
                ...
                if failures:
                    errors.extend(f"R1 reuse {experiment_id}: {failure}" for failure in failures)
                else:
                    reused_details[experiment_id] = payload
...
def _validate_input_derivation(
    ...
    try:
        directory = _safe_project_path(project_root, row["input_directory"], "R2 input directory")
        ...
        for key in set(before) | set(after):
            expected = (
                (_expected_r2_suffix(r1_row["suffix"]),)
                if key == "suffix"
                else before.get(key)
            )
            if after.get(key) != expected:
                errors.append(f"{prefix} INPUT differs outside the registered R2 suffix: {key}")
        if row["suffix"] != _expected_r2_suffix(r1_row["suffix"]):
            errors.append(f"{prefix} manifest suffix differs from the exact R2 derivation")
...
def _validate_manifest_rows(
    ...
    if ids != R2_AUDIT_IDS:
        errors.append("R2 manifest rows must be the exact 041--070 physical IDs in order")
    if tuple(row.get("execution_index", "") for row in rows) != tuple(
        str(index) for index in range(1, 31)
    ):
        errors.append("R2 manifest execution indices differ from the exact execution order")
    r1_by_id = _r1_by_id(project_root)
    for index, physical_id in enumerate(R2_AUDIT_IDS, 1):
        ...
        if row.get("execution_phase") != ("P1" if physical_id in R2_PILOT_IDS else "P2"):
        ...
        if row.get("suffix") != _expected_r2_suffix(old["suffix"]):
    ...
    if config.get("registered_experiment_ids") != list(R2_AUDIT_IDS):
        errors.append("config registered physical IDs differ")
    if config.get("execution_order") != list(EXECUTION_ORDER):
        errors.append("config R2 execution order differs")
...
def _translated_energy_matrix(r1_config: dict) -> dict[str, object]:
    ...
    matrix["r1_reused_audit_run_count"] = 10
    matrix["r2_executed_audit_run_count"] = 30
    return matrix
...
def _validate_logical_matrices(
    ...
    try:
        ...
        if config.get("new_run_matrix") != expected_new:
            errors.append("new run matrix differs from the frozen thirty-ID plan")
        ...
        errors.append(f"cannot rebuild R2 logical/new matrices: {error}")
...
def _validate_config_contract(
    ...
    if config.get("status") != "preregistered":
        errors.append("R2 config status must be preregistered")
    if config.get("preregistration_date") != "2026-08-06":
        errors.append("R2 preregistration date differs")
    if config.get("input_root") != str(INPUT_ROOT):
        errors.append("R2 input root differs")
    if config.get("scope") != "G1 third-smearing / dense-k thermodynamic-label R2 continuation only":
        errors.append("R2 scope statement differs")

    r1_config = read_json(project_root / R1_CONFIG_PATH)
    expected_execution = {
        "rank_count": 4,
        "pilot_ids": list(R2_PILOT_IDS),
        "k_gate_completion_ids": list(R2_PILOT_IDS),
    ...
    if config.get("coverage") != expected_coverage:
        errors.append("R2 10+30 logical coverage accounting differs")
    ...
    if config.get("input_derivation") != expected_input_derivation:
        errors.append("R2 R1-input derivation contract differs")
    for key in (
        "numerical_axes",
        "output_contract",
        "thermodynamic_semantics",
        ...
        if config.get(key) != r1_config.get(key):
            errors.append(f"R2 frozen R1 scientific/runtime contract changed: {key}")
    for key in (
        "base_evidence_commit",
        "upstream_evidence",
        "source_runs",
        ...
        if config.get(key) != r1_config.get(key):
            errors.append(f"R2 frozen R1 upstream/source contract changed: {key}")
    try:
        if config.get("energy_matrix") != _translated_energy_matrix(r1_config):
            errors.append("R2 translated 42-point energy matrix differs")
        if config.get("field_label_groups") != _translated_field_groups(r1_config):
            errors.append("R2 translated field-label groups differ")
    except (KeyError, TypeError, ValueError) as error:
        errors.append(f"R2 translated scientific matrix validation failed: {error}")
    _validate_logical_matrices(project_root, config, rows, errors)

    manifest = config.get("manifest")
    if not isinstance(manifest, dict):
        errors.append("R2 manifest registration is missing")
    else:
        if manifest.get("path") != str(MANIFEST_PATH):
            errors.append("R2 manifest path differs")
        if manifest.get("sha256") != sha256(project_root / MANIFEST_PATH):
            errors.append("R2 manifest SHA-256 differs")
        if manifest.get("row_count") != 30:
            errors.append("R2 manifest row count differs")
        if manifest.get("fields") != list(MANIFEST_FIELDS):
            errors.append("R2 manifest field registration differs")
    expected_scope = {
    ...
    if config.get("formal_preregistration_commit_scope") != expected_scope:
        errors.append("R2 formal preregistration commit-scope contract differs")
...
def _validate_implementation(
    ...
    *,
    ...
    if not isinstance(implementation, dict) or not implementation:
        errors.append("R2 implementation closure is missing")
        return
    if not isinstance(modes, dict) or set(modes) != set(implementation):
        errors.append("R2 implementation Git-mode closure differs")
        return
    required = {
        "docs/S1_G1_THERMODYNAMIC_LABEL_AUDIT_R2_PROTOCOL.md",
    ...
    if not required.issubset(implementation):
        errors.append("R2 implementation closure omits a required path")
    for relative, digest in implementation.items():
        try:
            path = _safe_project_path(project_root, relative, "implementation path")
            if not path.is_file() or path.is_symlink():
                errors.append(f"R2 implementation is missing or symbolic: {relative}")
                continue
            if not isinstance(digest, str) or sha256(path) != digest:
                errors.append(f"R2 implementation SHA-256 differs: {relative}")
            if require_committed:
                failure = _tracked_head_failure(project_root, relative)
                if failure:
                    errors.append(f"R2 implementation {failure}")
                    continue
                entries = str(_git(project_root, "ls-files", "--stage", "--", relative)).splitlines()
                if len(entries) != 1:
                    errors.append(f"R2 implementation is not tracked exactly once: {relative}")
                    continue
                mode = entries[0].split(maxsplit=1)[0]
                if mode not in {"100644", "100755"} or modes.get(relative) != mode:
                    errors.append(f"R2 implementation Git mode differs: {relative}")
        except (FileNotFoundError, TypeError, ValueError) as error:
            errors.append(f"R2 implementation validation failed {relative}: {error}")
...
def _validate_preregistration(
    ...
    try:
        ...
        if _introduction_commit(project_root, str(MANIFEST_PATH)) != prereg:
            errors.append("R2 config and manifest were not introduced together")
        expected_inputs = {
        ...
        if _commit_changed_paths(project_root, prereg) != expected_scope:
            errors.append("R2 preregistration scope is not config+manifest+complete 30-run input tree")
        ...
        for relative in (str(CONFIG_PATH), str(MANIFEST_PATH)):
            if _blob_at(project_root, prereg, relative) != (project_root / relative).read_bytes():
                errors.append(f"R2 preregistration blob differs: {relative}")
        implementation = config.get("implementation")
        if isinstance(implementation, dict):
            for relative in implementation:
                if _blob_at(project_root, parent, relative) != (project_root / relative).read_bytes():
                    errors.append(f"R2 implementation differs from preregistration parent: {relative}")
        for row in rows:
            experiment_id = row["experiment_id"]
            ...
            if str(_git(project_root, "ls-tree", "-r", parent, "--", input_directory)):
                errors.append(f"R2 input existed before preregistration: {experiment_id}")
            if _tree_oid(project_root, prereg, input_directory) != _tree_oid(
                project_root, "HEAD", input_directory
            ):
                errors.append(f"R2 input changed after preregistration: {experiment_id}")
            for basename in ("INPUT", "STRU", "KPT", "metadata.json"):
                relative = f"{input_directory}/{basename}"
                if _introduction_commit(project_root, relative) != prereg:
                    errors.append(f"R2 input was not introduced by preregistration: {relative}")
                if _blob_at(project_root, prereg, relative) != (project_root / relative).read_bytes():
                    errors.append(f"R2 input differs from preregistration: {relative}")
        return prereg
    except (
        FileNotFoundError,
        KeyError,
...
def _active_introduction(
    ...
    if not run.is_dir() or run.is_symlink():
        errors.append(f"R2 active run is invalid: {experiment_id}")
        return None
    ...
    if not marker.is_file() or marker.is_symlink():
        errors.append(f"R2 active run lacks metadata/status marker: {experiment_id}")
        return None
    try:
        return _latest_introduction_commit(project_root, _relative(project_root, marker))
    except ValueError as error:
        errors.append(f"cannot bind R2 active run {experiment_id}: {error}")
        return None
...
def _archive_events(
    ...
    for attempt in sorted(root.iterdir(), key=lambda path: path.name):
        ...
        if not attempt.is_dir() or attempt.is_symlink() or match is None:
            errors.append(f"R2 failed archive is non-canonical: {experiment_id}/{attempt.name}")
            continue
        try:
            ...
            if str(_git(project_root, "rev-parse", f"{archive_commit}^")) != failure_commit:
                errors.append(f"R2 archive is not adjacent to failed commit: {experiment_id}")
            ...
            if not failure_tree == archive_tree == head_tree:
                errors.append(f"R2 failed tree changed during or after archival: {experiment_id}")
            ...
            if not failure_paths or any(not path.startswith(f"{run_relative}/") for path in failure_paths):
                errors.append(f"R2 failed run commit is not independently scoped: {experiment_id}")
            ...
            if not archive_paths or any(
                ...
                errors.append(f"R2 archive commit scope differs: {experiment_id}")
            if subprocess.run(
                ["git", "-C", str(project_root), "cat-file", "-e", f"{archive_commit}:{run_relative}"],
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL,
            ).returncode == 0:
                errors.append(f"R2 failed run was copied instead of moved: {experiment_id}")
            events.append((attempt.name, failure_commit))
        except (subprocess.CalledProcessError, ValueError) as error:
            errors.append(f"cannot bind R2 failed archive {experiment_id}: {error}")
    return events
...
def validate_detachment_attestation(
    ...
    *,
    ...
    if not isinstance(execution, dict):
        return payload, ["R2 execution contract is missing"]
    ...
    try:
        ...
        if require_committed:
            ...
            if str(_git(project_root, "rev-parse", f"{introduction}^")) != prereg:
                errors.append("detachment attestation commit is not adjacent to R2 preregistration")
            if payload.get("git_head") != prereg:
                errors.append("detachment attestation git_head differs from R2 preregistration")
...
def validate_precompletion_analysis(
    ...
    *,
    ...
    try:
        parsed = _external_object(path, "R2 analysis summary")
        payload = parsed
        execution = config.get("execution")
        contract = (
        ...
        if not isinstance(contract, dict):
            errors.append("R2 analysis supervisor-completion contract is missing")
        elif (
            ...
            errors.append("R2 analysis/pre-completion authority status differs")
        if payload.get("config_sha256") != sha256(project_root / CONFIG_PATH):
            errors.append("R2 analysis config SHA-256 differs")
        if payload.get("manifest_sha256") != sha256(project_root / MANIFEST_PATH):
            errors.append("R2 analysis manifest SHA-256 differs")
        if require_committed:
            failure = _tracked_head_failure(project_root, str(ANALYSIS_SUMMARY_PATH))
            if failure:
                errors.append(f"R2 analysis summary {failure}")
            ...
            if str(_git(project_root, "rev-parse", "HEAD")) != introduction:
                errors.append("R2 pre-completion analysis is not HEAD")
            ...
            if (
                ...
                errors.append("R2 pre-completion analysis commit scope differs")
            if _blob_at(
                project_root, introduction, ANALYSIS_SUMMARY_PATH.as_posix()
            ) != path.read_bytes():
                errors.append("R2 analysis summary differs from its introduction blob")
    except (
        FileNotFoundError,
        KeyError,
        TypeError,
        ...
        errors.append(f"R2 pre-completion analysis validation failed: {error}")
    return payload, errors
...
def recompute_final_analysis(
    ...
    errors: list[str] = []
    try:
        from analyze_s1_g1_thermodynamic_label_audit_r2 import analyze

        with tempfile.TemporaryDirectory(prefix="m-ofdft-g1-r2-final-replay-") as directory:
        ...
        errors.append(f"R2 final aggregate-science replay failed: {error}")
    return errors
...
def validate_supervisor_completion(
    ...
    *,
    ...
    try:
        ...
        analysis = _external_object(analysis_path, "R2 analysis summary")
...
def _barrier_spec(
    ...
    prefix = [
        str(python),
        "-s",
        str(project_root / "scripts/validate_s1_g1_thermodynamic_label_audit_r2.py"),
        str(project_root / MANIFEST_PATH),
        "--config",
        str(project_root / CONFIG_PATH),
        "--require-committed",
    ...
    if barrier_name == "imported-p0-before-041":
        return None, None, [*prefix, "--require-pilot-gate"]
    if barrier_name == "k-gate-after-042":
        return R2_PILOT_IDS[1], R2_PILOT_LOGICAL_PAIR[1], [*prefix, "--require-k-gate"]
    if barrier_name == "final-all-after-070":
    ...
    if barrier_name == "final-analysis":
        return R2_AUDIT_IDS[-1], NEW_TO_LOGICAL[R2_AUDIT_IDS[-1]], [
            str(python),
            "-s",
            str(project_root / "scripts/analyze_s1_g1_thermodynamic_label_audit_r2.py"),
    ...
    if per_run:
        experiment_id = f"S1-20260806-{per_run.group(2)}"
        if experiment_id not in R2_AUDIT_IDS:
    ...
    raise ValueError(f"unknown R2 barrier name: {barrier_name}")
...
def validate_barrier_failures(
    ...
    *,
    ...
    execution = config.get("execution")
    if not isinstance(execution, dict):
        return records, ["R2 execution contract is missing"]
...
def validate_attempt_marker(
    ...
    *,
    ...
    if experiment_id not in R2_AUDIT_IDS:
        return payload, commit, [f"attempt marker ID is outside R2: {experiment_id}"]
    execution = config.get("execution")
    if not isinstance(execution, dict):
        return payload, commit, ["R2 execution contract is missing"]
    ...
    try:
        ...
        if require_committed:
            ...
            if commit == prereg or not _is_ancestor(project_root, prereg, commit):
                errors.append(f"{experiment_id}: attempt marker is not after R2 preregistration")
...
def _validate_execution_history(
    ...
    active: dict[str, str] = {}
    events: dict[str, list[tuple[str, str]]] = {experiment_id: [] for experiment_id in R2_AUDIT_IDS}
    marker_commits: dict[str, str] = {}
    ledger = project_root / ATTEMPT_LEDGER_ROOT
    if ledger.exists():
        if not ledger.is_dir() or ledger.is_symlink():
            errors.append("R2 attempt ledger is not a regular directory")
        else:
            observed = {path.name for path in ledger.iterdir()}
            allowed = {f"{experiment_id}.json" for experiment_id in R2_AUDIT_IDS}
            unexpected = observed - allowed
            if unexpected:
                errors.append(f"R2 attempt ledger contains unexpected entries: {sorted(unexpected)}")
    for experiment_id in R2_AUDIT_IDS:
        ...
        if introduction:
            ...
            if not changed or any(not path.startswith(f"runs/{experiment_id}/") for path in changed):
                errors.append(f"R2 active run commit is not independently scoped: {experiment_id}")
        ...
        if run_commits and experiment_id not in marker_commits:
            errors.append(f"R2 run lacks its prior formal-attempt ledger commit: {experiment_id}")
        for run_commit in run_commits:
            if marker_commits.get(experiment_id) and str(
                ...
                errors.append(
                    f"R2 run introduction is not adjacent to its attempt marker: {experiment_id}"
                )
        if introduction and archives:
            errors.append(f"R2 forbids an active retry after archive: {experiment_id}")
        if len(archives) > 1:
            errors.append(f"R2 forbids multiple failed attempts for one ID: {experiment_id}")
        for label, commit in events[experiment_id]:
            if commit == prereg or not _is_ancestor(project_root, prereg, commit):
                errors.append(f"R2 execution event is not after preregistration: {experiment_id}/{label}")
        ...
        for label, commit in events[experiment_id]:
            for predecessor in EXECUTION_ORDER[:index]:
                ...
                if predecessor_commit is None:
                    errors.append(
                        f"R2 execution order violation: {experiment_id}/{label} precedes accepted {predecessor}"
                    )
                    continue
                if predecessor_commit == commit or not _is_ancestor(project_root, predecessor_commit, commit):
                    errors.append(
                        f"R2 execution ancestry violation: {predecessor} is not before {experiment_id}/{label}"
                    )
    for predecessor in sorted(required_accepted_predecessors):
        if predecessor not in active:
            continue
        status_failure = _status_failure_r2(
            project_root / "runs" / predecessor / STATUS_NAME,
            predecessor,
            accepted=True,
        )
        if status_failure:
            errors.append(f"R2 predecessor {predecessor}: {status_failure}")
        for name in (STATUS_NAME, EVIDENCE_NAME):
            relative = f"runs/{predecessor}/{name}"
            failure = _tracked_head_failure(project_root, relative)
            if failure:
                errors.append(f"R2 predecessor {predecessor}: {failure}")
        ...
        if evidence_path.is_file() and not evidence_path.is_symlink():
            try:
                ...
                if (
                    ...
                    errors.append(f"R2 predecessor accepted-evidence identity differs: {predecessor}")
            except (ValueError, json.JSONDecodeError) as error:
                errors.append(f"R2 predecessor evidence is invalid {predecessor}: {error}")
    if any(events[experiment_id] for experiment_id in EXECUTION_ORDER[2:]):
        try:
            gate = evaluate_k_gate(project_root, config, rows, require_committed=True)
            if gate.get("accepted") is not True:
                errors.append("R2 proceeded beyond recovery pilots before the complete k gate passed")
        except (FileNotFoundError, KeyError, TypeError, ValueError) as error:
            errors.append(f"R2 post-pilot k-gate barrier failed: {error}")
...
def validate_registration(
    ...
    *,
    ...
    if not config_path.is_file() or config_path.is_symlink():
        raise ValueError("R2 thermodynamic-label config is missing or symbolic")
    if not manifest_path.is_file() or manifest_path.is_symlink():
        raise ValueError("R2 thermodynamic-label manifest is missing or symbolic")
    ...
    if require_committed:
        ...
        if (
            ...
            errors.append(
                "R2 revision is terminal after a preserved barrier failure"
            )
        ...
        try:
            imported_p0 = evaluate_imported_p0_gate(project_root, require_committed=True)
            if imported_p0.get("accepted") is not True:
                errors.append("the imported immutable R1 P0 gate is no longer accepted")
        except (FileNotFoundError, KeyError, TypeError, ValueError) as error:
            errors.append(f"imported R1 P0 gate revalidation failed: {error}")
    if errors:
        raise ValueError(
            "S1 G1 thermodynamic-label R2 registration validation failed:\n- "
            + "\n- ".join(errors)
        )
...
def _status_failure_r2(path: Path, experiment_id: str, *, accepted: bool) -> str | None:
    if not path.is_file() or path.is_symlink():
    ...
    for key, value in common.items():
        if payload.get(key) != value:
            return f"R2 authoritative status {key} differs"
    if accepted:
        return None if payload == _status_payload(experiment_id, accepted=True) else "accepted R2 status differs"
    allowed = {
        *common,
        "status",
        "workflow_exit_code",
    ...
    if (
        ...
        return "failed R2 authoritative status model differs"
    ...
    return None if coherent else "failed R2 status stage/exit tuple differs"
...
def _read_json_for_r2_failure(path: Path) -> dict:
    ...
    *,
    ...
    if inventory_path.is_file() and not inventory_path.is_symlink():
        try:
            ...
            marker_errors.append(f"{row['experiment_id']}: invalid R2 failure inventory: {error}")
    with _r2_failure_policy():
    ...
    *,
    ...
    try:
        ...
        if require_replay_status:
            status_error = _status_failure_r2(run / STATUS_NAME, experiment_id, accepted=True)
            if status_error:
        ...
        if evidence.exists() or evidence.is_symlink():
            if not evidence.is_file() or evidence.is_symlink():
                errors.append(f"{experiment_id}: R2 evidence is not a regular file")
            elif read_json(evidence) != payload:
                errors.append(f"{experiment_id}: R2 evidence differs from recomputation")
            elif require_committed:
        ...
        errors.append(f"{experiment_id}: R2 run evidence validation failed: {error}")
    return payload, errors
...
# A descriptive compatibility alias for early R2 callers.  The analyzer-facing
...
def _write_exclusive_json(path: Path, payload: dict[str, object]) -> None:
    if path.exists() or path.is_symlink():
        raise ValueError(f"refusing to overwrite R2 failure evidence: {path}")
...
def evaluate_imported_p0_gate(
    project_root: Path, *, require_committed: bool
) -> dict[str, object]:
    rows: list[dict[str, str]] = []
    pairs = (
        ("S1-20260806-024", "S1-20260806-036"),
    ...
    payloads = [
        ...
        for common, extra in pairs
    ]
    for payload in payloads:
        payload["accepted"] = (
            float(payload["absolute_energy_difference_mev_per_atom"]) < 2.0
            and payload["field_metrics"]["accepted"] is True  # type: ignore[index]
        )
    return {
        "pair_count": len(payloads),
        "pairs": payloads,
        "accepted": len(payloads) == 2 and all(item["accepted"] is True for item in payloads),
    }
...
def evaluate_pilot_gate(
    ...
    *,
    ...
    del config, rows
    # R2 starts only after revalidating the already completed immutable R1 P0.
    # New IDs 041/042 complete P1 and are then covered by the six-pair k gate.
    return evaluate_imported_p0_gate(
        project_root, require_committed=require_committed
    )
...
def evaluate_half_quarter_pair(
    ...
    *,
    ...
    quarter_logical = (
        ...
        if quarter_experiment_id in R2_AUDIT_IDS
...
def main() -> int:
    ...
    if args.check_detachment_attestation:
        ...
        if failures:
            raise ValueError(
                "R2 detachment-attestation validation failed:\n- "
                + "\n- ".join(failures)
            )
    if args.check_attempt_marker:
        ...
        if failures:
            raise ValueError("R2 attempt-marker validation failed:\n- " + "\n- ".join(failures))
        if args.require_committed and marker_commit is None:
            raise ValueError(f"R2 attempt marker is not committed: {experiment_id}")
    ...
    if args.check_analysis_summary:
        ...
        if failures:
            raise ValueError(
                "R2 pre-completion analysis validation failed:\n- "
                + "\n- ".join(failures)
            )
    if args.require_supervisor_completion:
        ...
        if failures:
            raise ValueError(
                "R2 supervisor-completion validation failed:\n- "
                + "\n- ".join(failures)
            )
        ...
        if replay_failures:
            raise ValueError(
                "R2 final aggregate-science replay failed:\n- "
                + "\n- ".join(replay_failures)
            )
    if per_run:
        experiment_id = per_run[0]
        if experiment_id not in by_id:
            raise ValueError(f"requested run is outside the R2 manifest: {experiment_id}")
        ...
        if args.check_failure_archives:
            ...
            failures = validate_failed_r2_run(
    ...
    if args.require_half_quarter_pair:
        ...
        logical = logical_id(requested) if requested in R2_AUDIT_IDS else requested
        ...
        if half_quarter["accepted"] is not True:
            raise ValueError("R2 half-quarter field gate rejected: " + json.dumps(half_quarter, sort_keys=True))
    ...
    if args.require_adjacent_eos:
        ...
        if adjacent["accepted"] is not True:
            raise ValueError("R2 adjacent EOS gate rejected: " + json.dumps(adjacent, sort_keys=True))
    ...
    if args.require_pilot_gate:
        ...
        if pilot["accepted"] is not True:
            raise ValueError("R2 recovery pilot gate rejected: " + json.dumps(pilot, sort_keys=True))
    ...
    if args.require_all_runs:
        for logical in LOGICAL_IDS:
            ...
            if failures:
                raise ValueError(
                    f"R2 all-run validation failed for {logical}/{effective_id(logical)}:\n- "
                    + "\n- ".join(failures)
                )
