#!/usr/bin/env python3
"""Parse one preregistered S1-G1 thermodynamic-label audit R3 run.

R3 reuses the frozen, tested R1 scientific helper functions, but owns its
...
from generate_s1_g1_thermodynamic_label_audit_r3 import (
    CONFIG_PATH,
    MANIFEST_PATH,
    PROTOCOL_REVISION,
    R3_AUDIT_IDS,
...
def _validate_r3_registration(
    config_path: Path, manifest_path: Path
) -> tuple[dict[str, object], list[dict[str, str]]]:
    """Validate the R3 namespace without changing the frozen R1 parser file."""
    ...
    require(
        config.get("registered_experiment_ids") == list(R3_AUDIT_IDS),
    ...
    require(
        len(rows) == len(R3_AUDIT_IDS),
        f"manifest must contain exactly {len(R3_AUDIT_IDS)} rows",
    )
    require(
        tuple(row["experiment_id"] for row in rows) == R3_AUDIT_IDS,
        "manifest row ID order differs",
    )
    execution_order = config.get("execution_order")
    require(
        execution_order == list(R3_AUDIT_IDS),
    ...
    require(
        ...
        == {
            experiment_id: index
            for index, experiment_id in enumerate(R3_AUDIT_IDS, 1)
        },
...
def _validate_r3_inputs(run: Path, row: dict[str, str]) -> dict[str, object]:
    """Validate generated R3 inputs without the R1 metadata namespace."""
...
def _materialized_registration(
    ...
    with tempfile.TemporaryDirectory(prefix="m-ofdft-g1-r3-registration-") as temporary:
...
def validate_registration_contract(
    *,
    ...
    with _materialized_registration(config_bytes, manifest_bytes) as materialized:
        config, rows = _validate_r3_registration(*materialized)
    return {
...
def parse_run(
    ...
    *,
    ...
    """Parse an R3 run with a local registration and frozen R1 helpers."""
    ...
    with _materialized_registration(
        ...
        config, rows = _validate_r3_registration(
    ...
    require(experiment_id in R3_AUDIT_IDS, "run directory basename is not a registered ID")
    row = next(item for item in rows if item["experiment_id"] == experiment_id)
    metadata = _validate_r3_inputs(run, row)
    ...
    payload: dict[str, object] = {
        "schema_revision": "S1-G1-THERMODYNAMIC-LABELS-R3",
        "protocol_revision": PROTOCOL_REVISION,
        "status": "accepted",
