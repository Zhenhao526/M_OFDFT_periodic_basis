#!/usr/bin/env python3
"""Launch and attest the detached G1 thermodynamic-label R3 supervisor.
...
DEFAULT_CONFIG = Path("config/S1_g1_thermodynamic_label_audit_r3.json")
DEFAULT_MANIFEST = Path("config/S1_g1_thermodynamic_label_audit_r3_manifest.tsv")
DEFAULT_RUNNER = Path("scripts/run_s1_g1_thermodynamic_label_audit_r3.sh")
    ...
    try:
        for name, fixed_fd in SEALED_EXECUTION_INPUT_FDS.items():
            temporary_fd = os.memfd_create(
                f"m_ofdft_g1_r3_{name}", allow_sealing | cloexec
            )
...
def _runner_environment(
    ...
    bindings = {
        "M_OFDFT_G1_R3_SUPERVISOR_STATE_DIRECTORY": str(state_directory),
        "M_OFDFT_G1_R3_SUPERVISOR_PID": str(supervisor_pid),
        "M_OFDFT_G1_R3_SUPERVISOR_START_TIME_TICKS": str(
            supervisor_start_time_ticks
        ),
        "M_OFDFT_G1_R3_BOOT_ID": boot_id,
        "M_OFDFT_G1_R3_LAUNCH_SHA256": launch_sha256,
        "M_OFDFT_G1_R3_GO_SHA256": go_sha256,
    }
...
def _supervise(arguments: argparse.Namespace) -> NoReturn:
    ...
    try:
        ...
        raise SystemExit("another R3 supervisor owns the state lock") from error
...
def finalize(arguments: argparse.Namespace) -> int:
    ...
    if (
        ...
        raise ValueError("completion analysis is not an accepted R3 summary")
    git_head = _git(project_root, "rev-parse", "HEAD")
