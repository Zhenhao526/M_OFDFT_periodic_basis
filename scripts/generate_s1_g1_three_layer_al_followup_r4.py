#!/usr/bin/env python3
"""Generate or byte-check the eight immutable Al-domain follow-up R4 inputs."""
...
def generate(project_root: Path, *, mode: str = "write") -> list[Path]:
    require(mode in {"write", "refresh-metadata", "check", "dry-run"}, "invalid generation mode")
    ...
    for row in rows:
        ...
        if mode == "write":
            ...
            require(directory.is_dir() and not directory.is_symlink(), f"registered input directory missing: {directory}")
            for name in ("INPUT", "STRU", "KPT"):
                path = directory / name
                require(path.is_file() and path.read_bytes() == rendered[name], f"non-metadata input changed during preregistration: {path}")
            metadata_path = directory / "metadata.json"
            require(metadata_path.is_file() and not metadata_path.is_symlink(), f"registered metadata missing: {metadata_path}")
            atomic_write(metadata_path, rendered["metadata.json"])  # type: ignore[arg-type]
            validate_case(directory, row, config)
        elif mode == "check":
            for name in ("INPUT", "STRU", "KPT", "metadata.json"):
                path = directory / name
...
def main() -> int:
    ...
    mode.add_argument("--refresh-metadata", action="store_true")
    args = parser.parse_args()
    project_root = find_project_root(args.project_root)
    selected = "check" if args.check_only else "dry-run" if args.dry_run else "refresh-metadata" if args.refresh_metadata else "write"
