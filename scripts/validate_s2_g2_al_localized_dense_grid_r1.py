...
import analyze_s2_g2_al_localized_dense_grid_r1 as analysis
import s2_g2_al_localized_common_r1 as common

CONFIG_REL = Path("config/S2_g2_al_localized_dense_grid_r1.json")
PROTOCOL_REL = Path("docs/S2_G2_AL_LOCALIZED_DENSE_GRID_R1_PROTOCOL.md")
ANALYZER_REL = Path("scripts/analyze_s2_g2_al_localized_dense_grid_r1.py")
VALIDATOR_REL = Path("scripts/validate_s2_g2_al_localized_dense_grid_r1.py")
TEST_REL = Path("tests/test_s2_g2_al_localized_dense_grid_r1.py")
...
def implementation_identity(root: Path, commit: str) -> dict:
    ...
    rows = diff_rows(root, analysis.BASE_COMMIT, commit)
    common.require(rows == [("A", path) for path in IMPLEMENTATION_PATHS], "implementation delta differs")
    config = json.loads(git(root, "show", f"{commit}:{CONFIG_REL}") or "{}")
...
def validate_implementation(root: Path) -> dict:
    ...
    return {"status": "accepted_implementation", "head": head, "new_solver_run_count": 0}
...
def validate_preregistered(root: Path) -> dict:
    ...
    common.require((root / CONFIG_REL).read_bytes() == git(root, "show", f"{head}:{CONFIG_REL}").encode() + b"\n" or json.loads((root / CONFIG_REL).read_text()) == config, "working config differs")
    validate_sources(root, config)
    return {"status": "accepted_preregistered", "head": head, "target_grids": config["analysis"]["target_grids"], "new_solver_run_count": 0}
...
def validate_committed(root: Path) -> dict:
    ...
    expected_paths = sorted(f"{config['execution']['analysis_root']}/{name}" for name in config["output_files"])
    common.require(diff_rows(root, prereg, head) == [("A", path) for path in expected_paths], "evidence delta differs")
    with tempfile.TemporaryDirectory(prefix="s2_dense_replay_") as temporary:
        temp = Path(temporary)
        summary, outputs = analysis.build_analysis(root, config)
        common.require(sorted(outputs) == sorted(config["output_files"]), "replay output denominator differs")
    ...
    return {"status": "accepted_committed_evidence", "scientific_status": summary["status"], "scientific_gate_accepted": summary["scientific_gate_accepted"], "failed_gates": summary["failed_gates"], "new_solver_run_count": 0}
