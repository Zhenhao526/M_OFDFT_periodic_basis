#!/usr/bin/env python3
"""Analyze the 40 newly executed logical slots of G1 R4."""
...
from validate_s1_g1_thermodynamic_label_audit_r4 import (
...
DEFAULT_OUTPUT = Path("analysis/s1/g1_thermodynamic_label_audit_r4_20260807")
...
def _readme(summary: dict[str, object]) -> str:
    ...
    lines = [
        "# S1 G1 thermodynamic-label audit R4",
        "",
        ...
        f"- New R4 runs accepted: `{summary['accepted_new_r4_count']}/40`",
        ...
        "",
        "The R4 denominator contains forty new runs in the current frozen runtime "
...
def analyze(
    ...
    *,
    ...
    if require_committed and not _git_clean(project_root):
        raise ValueError("refusing final R4 analysis from a dirty worktree")
    ...
    if replay_scientific_config is None:
        registration_snapshot = tempfile.TemporaryDirectory(
            prefix="m-ofdft-g1-r4-analysis-registration-"
        )
    ...
    if len(logical_to_effective) != 40 or len(set(logical_to_effective.values())) != 40:
        raise ValueError("R4 logical/effective mapping is not an exact 40-slot bijection")
    if logical_to_effective["S1-20260806-034"] == "S1-20260806-034":
        raise ValueError("archived R1-034 is forbidden from the R4 acceptance denominator")
    ...
    runtime_by_origin = {
        ...
        "r4_executed": {"accepted_run_count": 0, "rank_lifecycle_count": 0, "successful_syscall_count": 0},
    }
    ...
    expected_runtime = {
        "r1_reused": {"accepted_run_count": 0, "rank_lifecycle_count": 0, "successful_syscall_count": 0},
        "r4_executed": {"accepted_run_count": 40, "rank_lifecycle_count": 160, "successful_syscall_count": 480},
    }
    ...
    exact_counts = {
        ...
        "accepted_new_r4_runs": accepted_new,
    ...
    summary: dict[str, object] = {
        ...
        "accepted_new_r4_count": accepted_new,
