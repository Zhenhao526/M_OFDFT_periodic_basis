#!/usr/bin/env python3
"""Shared fail-closed helpers for the S1/G1 three-layer R1 pilot."""
...
CONFIG_PATH = Path("config/S1_g1_three_layer_r1.json")
MANIFEST_PATH = Path("config/S1_g1_three_layer_r1_manifest.tsv")
MANIFEST_FIELDS = (
    "execution_index",
    "experiment_id",
    "phase",
...
ID_PATTERN = re.compile(r"S1-20260810-(3(?:0[1-9]|1[0-8]))\Z")
...
def load_manifest(project_root: Path, path: Path = MANIFEST_PATH) -> list[dict[str, str]]:
    ...
    with io.StringIO(text, newline="") as handle:
        ...
        rows = list(reader)
    require(len(rows) == 18, "manifest must contain exactly 18 rows")
    ids: list[str] = []
    for expected_index, row in enumerate(rows, start=1):
        ...
        require(row["phase"] in {"p0", "al_eos", "mg_required", "mg_optional"}, "invalid phase")
    ...
    require(len(set(ids)) == 18, "duplicate experiment ID")
    require(ids == [f"S1-20260810-{number:03d}" for number in range(301, 319)], "ID sequence differs")
    return rows
...
def phase_rows(rows: list[dict[str, str]], phase: str) -> list[dict[str, str]]:
    ...
    expected = {"p0": 6, "al_eos": 6, "mg_required": 2, "mg_optional": 4}
