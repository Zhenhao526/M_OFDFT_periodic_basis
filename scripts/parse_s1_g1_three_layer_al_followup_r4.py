...
def validate_followup_metadata_schema(metadata: object, expected_protocol_revision: str) -> tuple[dict, dict]:
    """Map the registered role-only schema in memory, never in raw evidence.

    The inherited R1 parser predates the follow-up protocol and expects the
    redundant ``requirement`` key.  R4 accepts exactly one schema: ``role`` is
    present and ``requirement`` is absent.  The compatibility key exists only
    in the temporary object returned to the inherited parser and is deleted
    from the final result.
    """
    require(isinstance(metadata, dict), "follow-up metadata must be an object")
    require(metadata.get("schema_version") == 1, "follow-up metadata schema version differs")
    require(metadata.get("protocol_revision") == expected_protocol_revision, "follow-up metadata protocol differs")
    require("requirement" not in metadata, "legacy requirement key is forbidden in follow-up metadata")
    role = metadata.get("role")
    require(isinstance(role, str) and role in FOLLOWUP_ROLES, "follow-up metadata role is missing or invalid")
    phase = metadata.get("phase")
    require(phase in {"strain", "endpoint"}, "follow-up metadata phase is missing or invalid")
    require((phase == "strain") == role.startswith("al_"), "follow-up phase/role mapping differs")
    mapped = {**metadata, "requirement": role}
    proof = {
        "raw_schema": "role_only_requirement_absent",
        "compatibility_mapping": "in_memory_requirement_equals_role_for_inherited_parser_only",
        "raw_role": role,
        "raw_phase": phase,
        "legacy_requirement_was_present_in_raw": False,
        "legacy_requirement_removed_from_result": True,
        "accepted": True,
    }
    return mapped, proof


def parse_explicit_r4_affinity(run_dir: Path, config: dict) -> list[dict]:
    runtime = config["runtime"]
    rows: list[dict] = []
    for rank in range(int(runtime["rank_count"])):
        path = run_dir / "affinity" / f"rank_{rank:03d}.json"
        payload = read_json(path)
        require(isinstance(payload, dict), "R4 affinity payload must be object")
        require("physical_core_ids" not in payload and "expected_physical_core_id" not in payload, "ambiguous physical-core label is forbidden in R4 raw evidence")
        require(payload.get("rank") == rank and payload.get("local_rank") == rank, "R4 rank identity differs")
        require(payload.get("hostname") == runtime["required_hostname"], "R4 rank hostname differs")
        require(payload.get("os_logical_cpu_affinity") == runtime["thread_siblings_by_rank"][rank], "R4 OS logical affinity differs")
        require(payload.get("primary_os_logical_cpu_id") == runtime["primary_os_logical_cpu_ids_by_rank"][rank], "R4 primary OS CPU differs")
        require(payload.get("physical_package_id") == runtime["required_physical_package_id"], "R4 package differs")
        require(payload.get("sysfs_core_id") == runtime["sysfs_core_id_by_rank"][rank], "R4 /sys core_id differs")
        require(payload.get("thread_siblings") == runtime["thread_siblings_by_rank"][rank], "R4 thread siblings differ")
        require(payload.get("accepted") is True, "R4 rank affinity rejected")
    ...
    require([row["sysfs_core_id"] for row in rows] == runtime["sysfs_core_id_by_rank"], "R4 sysfs core_id denominator differs")
    return rows
...
def parse_run(
    ...
    config: dict,
    *,
    require_followup_orchestration: bool = True,
    rank_wrapper_path: Path | None = None,
) -> dict:
    if require_followup_orchestration:
        original_affinity_parser = base_parser.parse_affinity
        original_json_reader = base_parser.read_json
        raw_metadata_path = (run_dir / "metadata.json").resolve()
        raw_metadata = read_json(raw_metadata_path)
        mapped_metadata, schema_mapping = validate_followup_metadata_schema(
            raw_metadata, config["protocol_revision"]
        )

        def mapped_json_reader(path: Path) -> object:
            if Path(path).resolve() == raw_metadata_path:
                return mapped_metadata
            return original_json_reader(path)

        base_parser.parse_affinity = parse_explicit_r4_affinity
        base_parser.read_json = mapped_json_reader
        try:
            result = base_parser.parse_run(run_dir, config)
        finally:
            base_parser.parse_affinity = original_affinity_parser
            base_parser.read_json = original_json_reader
        require(result.pop("requirement") == raw_metadata["role"], "in-memory requirement/role mapping differs")
        result["metadata_schema_mapping"] = schema_mapping
        ...
        # The frozen R1/continuation source evidence predates R4 and retains its
        # registered legacy affinity schema. Replay it byte-exactly without
        # relabelling that read-only source as newly generated R4 evidence.
        result = base_parser.parse_run(run_dir, config)
    ...
    if require_followup_orchestration:
        ...
        wrapper = rank_wrapper_path or (Path(__file__).resolve().parent / "s1_g1_three_layer_al_followup_r4_rank_wrapper.py")
