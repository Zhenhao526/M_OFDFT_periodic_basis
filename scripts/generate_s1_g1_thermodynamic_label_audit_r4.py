#!/usr/bin/env python3
"""Generate the frozen S1-G1 thermodynamic-label R4 preregistration.

R4 is a clean rerun of the immutable R1 scientific matrix after R3 stopped
at a runtime-audit race. No R1, R2, or R3 run is reused in the R4 acceptance
denominator: all forty logical slots receive all-new
experiment IDs.  Every R4 input is derived mechanically from the corresponding
...
CONFIG_PATH = Path("config/S1_g1_thermodynamic_label_audit_r4.json")
MANIFEST_PATH = Path("config/S1_g1_thermodynamic_label_audit_r4_manifest.tsv")
INPUT_ROOT = Path("inputs/s1/g1_thermodynamic_label_audit_r4")
PROTOCOL_PATH = Path("docs/S1_G1_THERMODYNAMIC_LABEL_AUDIT_R4_PROTOCOL.md")
...
R3_IMPLEMENTATION_COMMIT = "e31f4567fa3d3f24c810110dffaad7accefc9211"
R3_HANDOFF_COMMIT = "11f2c6632ebc3800cf1c8dfdeb4d6325c2caa14d"
R3_PREREGISTRATION_COMMIT = "d6a09ba964bae82c8e85251ba55ea4ec60f131ff"
R3_DETACHMENT_COMMIT = "36e5070a56d1da912c1d2b70cb046727b4dff111"
R3_FIRST_MARKER_COMMIT = "b8f58f41705df92f9b17954172d1ebb2b42cd204"
R3_ACCEPTED_RUN_COMMIT = "2bf6450b0a2188bbb0935f1054303dd33fa08c88"
R3_SECOND_MARKER_COMMIT = "14d9c3bbabd216e20900ac5cfd61a03170dd9b46"
R3_FAILED_RUN_COMMIT = "56f2dcbd3d8ce0148b6de982a6380842bc0736f3"
R3_ARCHIVE_COMMIT = "20be191ee1811afcce20ca253c8b68bb80213a2b"
R3_TERMINAL_BARRIER_COMMIT = "0d984d0074bf1e74caf587dfebda1395e83b4581"
R3_CONFIG_PATH = Path("config/S1_g1_thermodynamic_label_audit_r3.json")
R3_MANIFEST_PATH = Path("config/S1_g1_thermodynamic_label_audit_r3_manifest.tsv")
R3_INPUT_ROOT = Path("inputs/s1/g1_thermodynamic_label_audit_r3")
R3_ORCHESTRATION_ROOT = Path(
    "orchestration/s1/g1_thermodynamic_label_audit_r3_20260807"
)
R3_ATTEMPT_LEDGER_ROOT = R3_ORCHESTRATION_ROOT / "attempts"
R3_DETACHMENT_PATH = R3_ORCHESTRATION_ROOT / "detachment.json"
R3_BARRIER_PATH = R3_ORCHESTRATION_ROOT / "barrier_failures/failure-archive-002.json"
R3_COMPLETION_PATH = R3_ORCHESTRATION_ROOT / "supervisor_completion.json"
R3_ANALYSIS_ROOT = Path("analysis/s1/g1_thermodynamic_label_audit_r3_20260807")
R3_ACCEPTED_ID = "S1-20260807-001"
R3_FAILED_ID = "S1-20260807-002"
R3_FAILED_ARCHIVE_PATH = Path(
    "failed_runs/runtime_relocation/S1-20260807-002/attempt-56f2dcbd3d8c"
)
R3_CONFIG_BLOB_OID = "1779ddf6b534aad277a1b60b51b959d7919d8128"
R3_CONFIG_SHA256 = "cad5a7c0638b5019ac3525dbbfd4e5783bf8848665f7691218b271b18865383f"
R3_MANIFEST_BLOB_OID = "870808328408d8cc3e6117220e05040a223422c4"
R3_MANIFEST_SHA256 = "90511215af832ff08619fd60e1f17e688c3daa491bde61d3d5ca9327a8211298"
R3_INPUT_ROOT_TREE_OID = "4969341e662022fd8155e4b33246ccc55d715d40"
R3_ACCEPTED_TREE_OID = "5c5f31d2ab8b9e1c9553c3d35ae028e2969fd7b7"
R3_FAILED_TREE_OID = "f6fa8896774dce116affc1905a8eb91153ceb05e"
R3_BARRIER_BLOB_OID = "54ac12392ed3a5a2807a8fc8523118e429ba9a2b"

R4_AUDIT_IDS = tuple(f"S1-20260807-{value:03d}" for value in range(41, 81))
...
PILOT_IDS = R4_AUDIT_IDS[:12]
EXECUTION_ORDER = R4_AUDIT_IDS
...
LOGICAL_IDS_FOR_R4 = tuple(
    f"S1-20260806-{value:03d}" for value in _LOGICAL_ID_SUFFIXES
)
require(
    len(R4_AUDIT_IDS) == len(LOGICAL_IDS_FOR_R4),
    "R4/logical ID mapping lengths differ",
)
NEW_TO_LOGICAL = dict(zip(R4_AUDIT_IDS, LOGICAL_IDS_FOR_R4))
...
# R4 keeps the exact R1 TSV schema so the scientific parser can reuse the same
# row contract.  Logical-slot and bridge provenance live in the JSON config.
R4_MANIFEST_FIELDS = MANIFEST_FIELDS
...
SUPERVISOR_COMPLETION_PATH = Path(
    "orchestration/s1/g1_thermodynamic_label_audit_r4_20260807/"
    "supervisor_completion.json"
)
BARRIER_FAILURE_ROOT = Path(
    "orchestration/s1/g1_thermodynamic_label_audit_r4_20260807/barrier_failures"
)
...
R4_IMPLEMENTATION_PATHS = (
    ...
    "scripts/runtime_relocation_audit_launcher_g1_r4.py",
    "tests/unit/test_runtime_relocation_audit_launcher_g1_r4.py",
    "tests/unit/test_s1_g1_thermodynamic_label_audit_r4_generator.py",
    "tests/unit/test_s1_g1_thermodynamic_label_audit_r4_parser.py",
    "tests/unit/test_s1_g1_thermodynamic_label_audit_r4_validator.py",
    "tests/unit/test_s1_g1_thermodynamic_label_audit_r4_launcher.py",
    "tests/unit/test_s1_g1_thermodynamic_label_audit_r4_runner.py",
    "tests/unit/test_s1_g1_thermodynamic_label_audit_r4_analysis.py",
)
R4_EXECUTABLE_IMPLEMENTATION_PATHS = frozenset(
    {"scripts/run_s1_g1_thermodynamic_label_audit_r4.sh"}
)
...
def build_r2_stopped_bridge(project_root: Path) -> dict[str, object]:
    ...
    return {
        ...
        "r4_reuse_forbidden": True,
    }


def build_r3_stopped_bridge(project_root: Path) -> dict[str, object]:
    """Bind the immutable R3 stop without importing its accepted point."""

    project_root = _project_root(project_root)
    chain = (
        R3_IMPLEMENTATION_COMMIT,
        R3_HANDOFF_COMMIT,
        R3_PREREGISTRATION_COMMIT,
        R3_DETACHMENT_COMMIT,
        R3_FIRST_MARKER_COMMIT,
        R3_ACCEPTED_RUN_COMMIT,
        R3_SECOND_MARKER_COMMIT,
        R3_FAILED_RUN_COMMIT,
        R3_ARCHIVE_COMMIT,
        R3_TERMINAL_BARRIER_COMMIT,
    )
    for parent, child in zip(chain, chain[1:]):
        require(
            str(_git(project_root, "rev-parse", f"{child}^")) == parent,
            f"R3 stopped commit chain is not adjacent: {parent}->{child}",
        )
    require(
        _is_ancestor(project_root, R3_TERMINAL_BARRIER_COMMIT, "HEAD"),
        "R3 terminal barrier is not an ancestor of HEAD",
    )
    for path, expected_oid, expected_sha256, label in (
        (R3_CONFIG_PATH, R3_CONFIG_BLOB_OID, R3_CONFIG_SHA256, "config"),
        (R3_MANIFEST_PATH, R3_MANIFEST_BLOB_OID, R3_MANIFEST_SHA256, "manifest"),
    ):
        relative = path.as_posix()
        require(
            _git_object_id(project_root, R3_PREREGISTRATION_COMMIT, relative, "blob")
            == expected_oid
            == _git_object_id(project_root, "HEAD", relative, "blob"),
            f"R3 {label} blob changed",
        )
        require(
            sha256_bytes(_git_blob(project_root, R3_PREREGISTRATION_COMMIT, relative))
            == expected_sha256,
            f"R3 {label} SHA-256 changed",
        )
    require(
        _git_object_id(
            project_root, R3_PREREGISTRATION_COMMIT, R3_INPUT_ROOT.as_posix(), "tree"
        )
        == R3_INPUT_ROOT_TREE_OID
        == _git_object_id(project_root, "HEAD", R3_INPUT_ROOT.as_posix(), "tree"),
        "R3 input tree changed",
    )
    require(
        _git_object_id(
            project_root, R3_ACCEPTED_RUN_COMMIT, f"runs/{R3_ACCEPTED_ID}", "tree"
        )
        == R3_ACCEPTED_TREE_OID
        == _git_object_id(project_root, "HEAD", f"runs/{R3_ACCEPTED_ID}", "tree"),
        "R3 accepted run tree changed",
    )
    require(
        _git_object_id(
            project_root, R3_FAILED_RUN_COMMIT, f"runs/{R3_FAILED_ID}", "tree"
        )
        == R3_FAILED_TREE_OID
        == _git_object_id(
            project_root, R3_ARCHIVE_COMMIT, R3_FAILED_ARCHIVE_PATH.as_posix(), "tree"
        )
        == _git_object_id(project_root, "HEAD", R3_FAILED_ARCHIVE_PATH.as_posix(), "tree"),
        "R3 failed/archive tree changed",
    )
    require(
        not str(
            _git(
    ...
    marker_paths = tuple(
        str(
            _git(
                ...
                "HEAD",
                "--",
                R3_ATTEMPT_LEDGER_ROOT.as_posix(),
            )
        ).splitlines()
    )
    expected_markers = tuple(
        (R3_ATTEMPT_LEDGER_ROOT / f"{experiment_id}.json").as_posix()
        for experiment_id in (R3_ACCEPTED_ID, R3_FAILED_ID)
    )
    require(marker_paths == expected_markers, "R3 attempted ID set changed")
    require(
        _changed_paths(project_root, R3_DETACHMENT_COMMIT)
        == (R3_DETACHMENT_PATH.as_posix(),),
        "R3 detachment commit scope differs",
    )
    require(
        _changed_paths(project_root, R3_FIRST_MARKER_COMMIT)
        == (expected_markers[0],),
        "R3 first marker commit scope differs",
    )
    require(
        _changed_paths(project_root, R3_SECOND_MARKER_COMMIT)
        == (expected_markers[1],),
        "R3 second marker commit scope differs",
    )
    require(
        _changed_paths(project_root, R3_TERMINAL_BARRIER_COMMIT)
        == (R3_BARRIER_PATH.as_posix(),),
        "R3 terminal barrier commit scope differs",
    )
    require(
        _git_object_id(
            project_root, R3_TERMINAL_BARRIER_COMMIT, R3_BARRIER_PATH.as_posix(), "blob"
        )
        == R3_BARRIER_BLOB_OID
        == _git_object_id(project_root, "HEAD", R3_BARRIER_PATH.as_posix(), "blob"),
        "R3 terminal barrier blob changed",
    )
    require(
        not str(
            _git(
                ...
                "HEAD",
                "--",
                R3_ANALYSIS_ROOT.as_posix(),
                R3_COMPLETION_PATH.as_posix(),
            )
        ),
        "R3 analysis/completion unexpectedly exists",
    )
    frozen_paths = (
        R3_CONFIG_PATH.as_posix(),
        R3_MANIFEST_PATH.as_posix(),
        R3_INPUT_ROOT.as_posix(),
        R3_ORCHESTRATION_ROOT.as_posix(),
        R3_ANALYSIS_ROOT.as_posix(),
        *(f"runs/S1-20260807-{value:03d}" for value in range(1, 41)),
        *(
            f"failed_runs/runtime_relocation/S1-20260807-{value:03d}"
    ...
    require(
        not str(
            _git(
                ...
                "diff",
                "--name-only",
                f"{R3_TERMINAL_BARRIER_COMMIT}..HEAD",
                "--",
                *frozen_paths,
            )
        ),
        "R3 namespace changed after the terminal barrier",
    )
    return {
        "protocol_revision": "S1-G1-THERMODYNAMIC-LABEL-AUDIT-R3",
        "terminal_status": "stopped",
        "commit_chain": list(chain),
        "config": _file_anchor_at_commit(
            project_root, R3_PREREGISTRATION_COMMIT, R3_CONFIG_PATH.as_posix()
        ),
        "manifest": _file_anchor_at_commit(
            project_root, R3_PREREGISTRATION_COMMIT, R3_MANIFEST_PATH.as_posix()
        ),
        "input_root": {
            "path": R3_INPUT_ROOT.as_posix(),
            "tree_oid": R3_INPUT_ROOT_TREE_OID,
        },
        "detachment": _file_anchor_at_commit(
            project_root, R3_DETACHMENT_COMMIT, R3_DETACHMENT_PATH.as_posix()
        ),
        "attempt_markers": [
            _file_anchor_at_commit(project_root, commit, path)
            for commit, path in zip(
                (R3_FIRST_MARKER_COMMIT, R3_SECOND_MARKER_COMMIT), expected_markers
            )
        ],
        "attempted_ids_exact": [R3_ACCEPTED_ID, R3_FAILED_ID],
        "registered_but_unattempted_ids_exact": [
            f"S1-20260807-{value:03d}" for value in range(3, 41)
        ],
        "accepted_run": {
            "experiment_id": R3_ACCEPTED_ID,
            "introduction_commit": R3_ACCEPTED_RUN_COMMIT,
            "tree_oid": R3_ACCEPTED_TREE_OID,
        },
        "failed_run": {
            "experiment_id": R3_FAILED_ID,
            "failure_commit": R3_FAILED_RUN_COMMIT,
            "archive_commit": R3_ARCHIVE_COMMIT,
            "archive_path": R3_FAILED_ARCHIVE_PATH.as_posix(),
            "tree_oid": R3_FAILED_TREE_OID,
        },
        "terminal_barrier": _file_anchor_at_commit(
            project_root, R3_TERMINAL_BARRIER_COMMIT, R3_BARRIER_PATH.as_posix()
        ),
        "analysis_present": False,
        "supervisor_completion_present": False,
        "accepted_scientific_denominator_contribution": 0,
...
def _require_clean(project_root: Path) -> None:
    ...
    require(status == "", "formal R4 preregistration requires a clean worktree")
...
def build_r1_bridge(
    ...
    """Rebuild the complete R1-to-R4 cryptographic bridge."""
...
def build_plan(
    ...
    """Build the 40-slot logical matrix and the 40-row R4 run matrix."""
        ...
        logical_matrix.append(
            {
                ...
                "evidence_origin": "r4_executed",
                "r1_execution_index": r1_row["execution_index"],
                "r1_execution_phase": r1_row["execution_phase"],
                "r4_execution_index": new_index[physical_id],
                "r4_execution_phase": "P1" if physical_id in PILOT_IDS else "P2",
    ...
    for execution_index, new_id in enumerate(EXECUTION_ORDER, 1):
        ...
        suffix = "g1tlr4_" + old_suffix.removeprefix("g1tlr1_")
        new_matrix.append(
            {
    ...
    require(len(logical_matrix) == 40 and len(new_matrix) == 40, "R4 matrix count differs")
    require(
        {row["physical_experiment_id"] for row in logical_matrix}
        == set(R4_AUDIT_IDS),
        "R4 physical ID partition differs",
    )
...
def _derive_input_blob(
    ...
    require(new_parsed.get("suffix") == (new_suffix,), "R4 parsed suffix differs")
    ...
    require(old_without_suffix == new_without_suffix, "R4 INPUT changed beyond suffix")
    return derived
...
def _derive_metadata_blob(
    ...
    require(
        ...
        f"R4 metadata changed unexpected keys: {sorted(changed)}",
    )
...
def _manifest_bytes(rows: Iterable[dict[str, object]]) -> bytes:
    ...
    count = 0
    for row in rows:
        require(set(row) == set(MANIFEST_FIELDS), "R4 manifest row key set differs")
        writer.writerow({field: row[field] for field in MANIFEST_FIELDS})
        count += 1
    require(count == len(R4_AUDIT_IDS), "R4 manifest row count differs")
...
def derive_registration_artifacts(
    ...
    for plan in new_matrix:
        ...
        require(set(row) == set(MANIFEST_FIELDS), f"R4 manifest schema differs: {new_id}")
        rows.append(row)

    require(tuple(row["experiment_id"] for row in rows) == EXECUTION_ORDER, "R4 manifest order differs")
    require(len(artifacts) == 4 * len(R4_AUDIT_IDS), "R4 input artifact count differs")
...
def _implementation_closure(
    ...
    *,
    ...
    modes: dict[str, str] = {}
    for relative in tuple(dict.fromkeys((*inherited_hashes.keys(), *R4_IMPLEMENTATION_PATHS))):
        path = project_root / relative
        ...
        if relative in R4_EXECUTABLE_IMPLEMENTATION_PATHS:
            require(
                mode == "100755" and os.access(path, os.X_OK),
                f"R4 executable implementation mode differs: {relative}",
            )
...
def build_r4_runtime_contract(
    project_root: Path, r1_config: dict[str, object]
) -> dict[str, object]:
    runtime = deepcopy(r1_config["runtime"])
    launcher = project_root / "scripts/runtime_relocation_audit_launcher_g1_r4.py"
    require(launcher.is_file() and not launcher.is_symlink(), "R4 audit shim is missing")
    runtime["wrappers"]["audit_launcher"] = {
        "path": str(launcher),
        "sha256": sha256_regular_file(launcher),
    }
    return runtime
...
def _map_energy_matrix(
    ...
    for point in points:
        ...
        if logical_id in LOGICAL_TO_EFFECTIVE_ID:
            ...
            point["source_kind"] = (
                "r1_reused" if logical_id in R1_REUSED_AUDIT_IDS else "r4_executed"
            )
    result["r1_reused_audit_run_count"] = len(R1_REUSED_AUDIT_IDS)
    result["r4_executed_audit_run_count"] = len(R4_AUDIT_IDS)
    return result
...
def _require_preregistration_targets_absent(
    ...
    for output in (
        ...
        require(
            ...
            f"refusing to overwrite R4 output: {output}",
        )
    for experiment_id in R4_AUDIT_IDS:
        for relative in (
            ...
            require(
                ...
                f"R4 execution/attempt path exists before preregistration: {relative}",
            )
    for relative in (
        ATTEMPT_LEDGER_ROOT,
        ...
        require(
            not target.exists() and not target.is_symlink(),
            f"R4 orchestration path exists before preregistration: {relative}",
        )
...
def prepare(
    ...
    *,
    ...
    input_root = Path(input_root)
    if write:
        require(output_config == CONFIG_PATH, "formal R4 config path must be canonical")
        require(output_manifest == MANIFEST_PATH, "formal R4 manifest path must be canonical")
        require(input_root == INPUT_ROOT, "formal R4 input root must be canonical")
    ...
    r3_stopped_bridge = build_r3_stopped_bridge(project_root)
    ...
    runtime = build_r4_runtime_contract(project_root, r1_config)
    ...
    config: dict[str, object] = {
        ...
        "scope": "G1 third-smearing / dense-k thermodynamic-label R4 continuation only",
        "registered_experiment_ids": list(R4_AUDIT_IDS),
        ...
        "r3_stopped_bridge": r3_stopped_bridge,
...
def main() -> int:
    ...
    summary = {
        ...
        "r4_execution_count": len(R4_AUDIT_IDS),
