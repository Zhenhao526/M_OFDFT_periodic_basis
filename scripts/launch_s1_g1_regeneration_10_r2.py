...
def is_compute_rank_command(command: list[str]) -> bool:
    ...
    if executable.startswith("abacus_pw_para") or "rank_wrapper" in executable:
        return True
    return executable.startswith("python") and any(
        "rank_wrapper" in Path(argument).name for argument in command[1:3]
    )
...
def runner_command(config: dict, row: dict[str, str]) -> list[str]:
    return [
        ...
        "-s",
        str(Path(config["project_root"]) / "scripts/run_s1_g1_regeneration_case_r2.py"),
        "--case-id",
        row["case_id"],
    ]
