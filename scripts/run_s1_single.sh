...
python3 - "$run_directory/experiment_metadata.json" "$experiment_id" "$git_commit" "$abacus" "$mpi_ranks" <<'PY'
import datetime
import hashlib
import json
import sys
...
sha256sum "$run_directory/INPUT" "$run_directory/STRU" "$run_directory/KPT" \
    "$run_directory/$pseudopotential" > "$run_directory/INPUT_SHA256SUMS"
