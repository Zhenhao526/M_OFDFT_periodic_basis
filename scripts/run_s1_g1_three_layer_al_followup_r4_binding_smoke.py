...
def validate_rank_payloads(root: Path, config: dict) -> list[dict]:
    ...
    for rank in range(runtime["rank_count"]):
        ...
        require(payload.get("rank_wrapper_sha256") == sha256_file(Path(__file__).resolve().parent / "s1_g1_three_layer_al_followup_r4_rank_wrapper.py"), f"rank {rank} wrapper SHA differs")
