...
from s2_g2_al1_basis_convergence_common_r2 import (
    BASE_COMMIT,
    CONFIG_REL,
...
def main() -> int:
    ...
    if args.implementation_only:
        ...
        require(analysis["summary"]["new_solver_run_count"] == 0, "analysis started a solver")
        ...
        require(actual_root.is_dir() and not actual_root.is_symlink(), "committed output root differs")
        require(sorted(path.name for path in actual_root.iterdir()) == sorted(config["output"]["files"]),
                "committed output denominator differs")
    ...
    print(json.dumps({
        ...
        "selected_candidate": summary["selected_candidate"]["candidate_id"] if summary and summary["selected_candidate"] else None,
