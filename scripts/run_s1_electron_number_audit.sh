...
manifest=${1:-$project_root/config/S1_electron_number_audit_manifest.tsv}
config=${2:-$project_root/config/S1_electron_number_audit.json}
manifest=$(realpath "$manifest")
config=$(realpath "$config")
cd "$project_root"

if [[ -n $(git status --porcelain) ]]; then
    echo "Refusing electron-number audit from a dirty worktree" >&2
    exit 2
fi
...
validator=scripts/validate_s1_electron_number_audit.py
...
if not isinstance(invocation_exit, int) or not isinstance(parser_exit, int):
    ...
    parser_exit = 97
try:
    code_commit = json.loads((run / "experiment_metadata.json").read_text(encoding="utf-8"))["code_commit"]
...
write_status(
    ...
    failure_stage=run_status.get("failure_stage") or "electron_number_outer_validation",
)
PY
}
...
archive_failed_attempt() {
    ...
    git commit -m "archive failed electron-number attempt $experiment_id"
    if [[ $(git rev-parse HEAD^) != "$failure_commit" ]]; then
...
exec 9< <("$python_tool" - "$manifest" <<'PY'
import csv
import sys
...
with open(sys.argv[1], encoding="utf-8", newline="") as handle:
    ...
    for row in sorted(rows, key=lambda item: item["audit_experiment_id"]):
        print("\t".join((
            row["audit_experiment_id"], row["source_experiment_id"],
            row["input_directory"], row["material"], row["series_id"],
        )))
PY
)
...
while IFS=$'\t' read -r audit_id source_id input_directory material series_id <&9; do
    ...
    if [[ "$audit_id" != S1-20260805-119 && "$audit_id" != S1-20260805-120 ]]; then
        "$python_tool" "$validator" "$manifest" --config "$config" \
            --require-committed --check-run S1-20260805-119 >/dev/null
        "$python_tool" "$validator" "$manifest" --config "$config" \
            --require-committed --check-run S1-20260805-120 >/dev/null
    fi
    ...
    git commit -m "record electron-number density replay $audit_id from $source_id"
...
exec 9<&-
if [[ $processed -ne 30 ]]; then
    echo "Electron-number runner processed $processed OF rows instead of 30" >&2
    exit 97
fi
"$python_tool" "$validator" "$manifest" --config "$config" \
