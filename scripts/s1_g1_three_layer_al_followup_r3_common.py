#!/usr/bin/env python3
"""Shared fail-closed helpers for the Al-domain follow-up R3."""
...
CONFIG_PATH = Path("config/S1_g1_three_layer_al_domain_followup_r3.json")
MANIFEST_PATH = Path("config/S1_g1_three_layer_al_domain_followup_r3_manifest.tsv")
PROTOCOL_PATH = Path("docs/S1_G1_THREE_LAYER_AL_DOMAIN_FOLLOWUP_R3_PROTOCOL.md")
ID_SEQUENCE = tuple(f"S1-20260810-{number:03d}" for number in range(343, 351))
...
def find_project_root(start: Path | None = None) -> Path:
    ...
    for path in (candidate, *candidate.parents):
        if (path / ".git").exists() and (path / CONFIG_PATH).is_file():
            return path
    raise ValueError("Al follow-up R3 project root not found")
