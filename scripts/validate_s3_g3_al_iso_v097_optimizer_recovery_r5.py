...
from s3_g3_al_iso_v097_optimizer_recovery_common_r5 import CONFIG_REL,IMPLEMENTATION_PATHS,canonical,git,load,require,source_exact,validate_config
from collect_s3_g3_al_iso_v097_optimizer_recovery_r5 import replay_outputs
BASE="4b1f2188567b5645e0a76d400879f9572f15d230"
...
def main():
 ...
 if a.implementation_only:c=implementation(root,head);resolved=load(root);validate_config(root,resolved);source_exact(root,resolved);require(not Path(resolved["execution"]["state_root"]).exists(),"state exists");out={"status":"accepted_implementation","case_count":4}
 elif a.preregistered_only:c=registered(root,head);resolved=load(root);validate_config(root,resolved);source_exact(root,resolved);require(not Path(resolved["execution"]["state_root"]).exists(),"state exists");out={"status":"accepted_preregistered","case_count":4}
 else:
  ...
  s=json.loads(rendered["summary.json"]);require(s["evidence_valid"] is True and s["g3_overall_closed"] is False and s["s4_authorized"] is False,"disposition differs");out={"status":"accepted_committed_evidence","scientific_gate_accepted":s["scientific_gate_accepted"],"case_count":4}
