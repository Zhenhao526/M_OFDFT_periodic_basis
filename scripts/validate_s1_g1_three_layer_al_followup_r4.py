#!/usr/bin/env python3
"""Validate follow-up R4 preregistration or replay committed evidence."""
...
from analyze_s1_g1_three_layer_al_followup_r4 import build_analysis, write_analysis
from generate_s1_g1_three_layer_al_followup_r4 import generate
...
def validate_preregistered(
    project_root: Path,
    config: dict,
    rows: list[dict[str, str]],
    *,
    preregistration_head: str | None = None,
) -> dict:
    current_head = require_clean_tree(project_root)
    head = preregistration_head or current_head
    require(config["external_state_root"] == "/home/shenwei01/.local/state/m_ofdft/s1_g1_three_layer_al_domain_followup_r4_20260810", "external state path differs")
    require(not Path(config["external_state_root"]).exists(), "fresh R4 external state must not exist at preregistration")
    ...
    require(runtime["ambiguous_physical_core_label_in_raw_evidence_forbidden"] is True and runtime["detached_four_rank_binding_smoke_before_state_required"] is True, "R4 topology/smoke hard gate differs")
    ...
    require_ancestor(project_root, config["r3_operational_failure_closure_commit"], implementation)
    ...
    require_tracked_matches_head(project_root, [Path(config["r3_operational_failure_closure_path"])])
    ...
    return {"status": "accepted", "mode": "preregistered", "head": head, "current_head": current_head, "implementation_commit": implementation, "registered_run_count": 8, "registered_file_count": len(registered_paths(config, rows)), "r2_operational_failure_closure": r2_closure, "r3_operational_failure_closure": r3_closure, "r3_parser_regression": parser_regression, "preregistration_identity": prereg_identity}
...
def validate_final(project_root: Path, config: dict, rows: list[dict[str, str]], require_committed: bool) -> dict:
    ...
    for experiment_id in config["formal_ids"]:
        ...
        require(isinstance(result, dict) and result.get("affinity", {}).get("sysfs_core_ids") == config["runtime"]["sysfs_core_id_by_rank"], "explicit R4 result sysfs core_id map differs")
        require("physical_core_ids" not in result["affinity"] and all("physical_core_ids" not in row for row in result["affinity"]["ranks"]), "ambiguous physical-core label entered R4 result")
        require("requirement" not in result and result.get("metadata_schema_mapping", {}).get("accepted") is True, "R4 role-only metadata mapping proof differs")
        ...
        require(case_preflight.get("reserved_os_logical_cpus") == config["runtime"]["reserved_os_logical_cpu_ids"], "per-case OS logical/SMT proof differs")
    ...
    require(terminal.get("status") == "accepted" and terminal.get("attempted_count") == terminal.get("accepted_count") == 8 and terminal.get("runner_return_code") == 0, "terminal differs")
    require(terminal.get("attempted_ids") == terminal.get("accepted_ids") == config["formal_ids"], "terminal ID denominator differs")
    ...
    terminal_identity_keys = ("attempt_marker_sha256", "accepted_marker_sha256", "runner_return_sha256", "result_sha256", "metadata_sha256")
    require(all(isinstance(terminal.get(key), dict) and list(terminal[key]) == config["formal_ids"] for key in terminal_identity_keys), "terminal orchestration SHA-map denominator differs")
    require(terminal.get("accepted_result_sha256") == terminal.get("result_sha256"), "terminal accepted/result SHA maps differ")
    require(session["core_reservation_ack"]["accepted"] is True, "core reservation ACK absent")
    require(session["binding_smoke_identity"].get("accepted") is True and session["binding_smoke_identity"].get("rank_count") == 4 and session["binding_smoke_identity"].get("abacus_exec_count") == 0, "binding-smoke identity missing")
    require(len(session["core_locks"]) == 8 and [row["logical_cpu_id"] for row in session["core_locks"]] == config["runtime"]["reserved_os_logical_cpu_ids"], "core-lock domain differs")
    ...
    require(session["live_preflight"].get("reserved_os_logical_cpus") == config["runtime"]["reserved_os_logical_cpu_ids"] and not session["live_preflight"].get("abacus_collisions"), "session sibling-aware collision proof differs")
    ...
    with tempfile.TemporaryDirectory(prefix="g1_al_followup_r4_replay_") as temporary:
...
def main() -> int:
    ...
    parser.add_argument("--require-binding-smoke-committed", action="store_true")
    parser.add_argument("--require-committed", action="store_true")
    args = parser.parse_args()
    project_root = find_project_root(args.project_root)
    config = load_config(project_root)
    rows = load_manifest(project_root)
    if args.preregistered_only or args.require_binding_smoke_committed:
        require(not args.require_committed and not (args.preregistered_only and args.require_binding_smoke_committed), "validator modes are mutually exclusive")
        preregistration_head = None
        if args.require_binding_smoke_committed:
            smoke_spec = config["binding_smoke"]
            smoke_aggregate = read_json(Path(smoke_spec["external_root"]) / smoke_spec["external_aggregate_relative_path"])
            require(isinstance(smoke_aggregate, dict), "binding-smoke aggregate missing")
            preregistration_head = smoke_aggregate.get("preregistration_commit")
            require(isinstance(preregistration_head, str) and len(preregistration_head) == 40, "binding-smoke preregistration identity missing")
        payload = validate_preregistered(project_root, config, rows, preregistration_head=preregistration_head)
        if args.require_binding_smoke_committed:
            smoke = verify_binding_smoke(config, project_root, payload["current_head"])
            payload = {**payload, "mode": "binding_smoke_require_committed", "binding_smoke": smoke}
    else:
