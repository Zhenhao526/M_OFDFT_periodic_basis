#!/usr/bin/env python3
"""Analyze the 40 newly executed logical slots of G1 R3."""
...
from validate_s1_g1_thermodynamic_label_audit_r3 import (
...
DEFAULT_OUTPUT = Path("analysis/s1/g1_thermodynamic_label_audit_r3_20260807")
...
def _readme(summary: dict[str, object]) -> str:
    ...
    lines = [
        "# S1 G1 thermodynamic-label audit R3",
        "",
        f"- Audit status: `{summary['audit_status']}`",
        f"- Effective accepted logical runs: `{summary['accepted_effective_run_count']}/40`",
        f"- Immutable R1 runs reused: `{summary['accepted_reused_r1_count']}/0`",
        f"- Historical R1 candidates excluded: `{summary['excluded_historical_r1_count']}/10`",
        f"- New R3 runs accepted: `{summary['accepted_new_r3_count']}/40`",
        ...
        "",
        "The R3 denominator contains forty new runs in the current frozen runtime "
        "environment. Ten historically accepted R1 candidates are retained only as "
        "immutable provenance and contribute zero because their current runtime "
        "fingerprints cannot all be replayed. The archived indeterminate R1-034 and "
        "stopped R2 attempt also contribute zero. All scalar and field labels retain "
        "their finite-temperature Mermin interpretation; no exact-zero-temperature "
        "field claim is made.",
        "",
        "## Failure ledger",
        "",
...
def analyze(
    ...
    *,
    ...
    if require_committed and not _git_clean(project_root):
        raise ValueError("refusing final R3 analysis from a dirty worktree")
    ...
    if replay_scientific_config is None:
        registration_snapshot = tempfile.TemporaryDirectory(
            prefix="m-ofdft-g1-r3-analysis-registration-"
        )
    ...
    if len(logical_to_effective) != 40 or len(set(logical_to_effective.values())) != 40:
        raise ValueError("R3 logical/effective mapping is not an exact 40-slot bijection")
    if logical_to_effective["S1-20260806-034"] == "S1-20260806-034":
        raise ValueError("archived R1-034 is forbidden from the R3 acceptance denominator")
    ...
    runtime_by_origin = {
        ...
        "r3_executed": {"accepted_run_count": 0, "rank_lifecycle_count": 0, "successful_syscall_count": 0},
    }
    ...
    expected_runtime = {
        "r1_reused": {"accepted_run_count": 0, "rank_lifecycle_count": 0, "successful_syscall_count": 0},
        "r3_executed": {"accepted_run_count": 40, "rank_lifecycle_count": 160, "successful_syscall_count": 480},
    }
    ...
    exact_counts = {
        ...
        "accepted_new_r3_runs": accepted_new,
    ...
    expected_counts = {
        ...
        "accepted_reused_r1_runs": 0,
        "accepted_new_r3_runs": 40,
    ...
    summary: dict[str, object] = {
        ...
        "excluded_historical_r1_count": 10,
        "accepted_new_r3_count": accepted_new,
        ...
        "excluded_r2_failure": {
            ...
            "logical_experiment_id": "S1-20260806-034",
