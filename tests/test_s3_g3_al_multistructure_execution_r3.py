...
import numpy as np
import s3_g3_al_multistructure_common_r3 as c
import collect_s3_g3_al_multistructure_execution_r3 as collector
import run_s3_g3_al_multistructure_execution_r3 as runner
...
class T(unittest.TestCase):
 ...
 def synthetic_runs(self,delta_ev=0.015):
  ...
  for row in self.cfg["formal_cases"]:
   ...
   runs.append(item)
  return runs
 def test_01_denominator(self):self.assertEqual((len(self.cfg["formal_cases"]),self.cfg["formal_cases"][0]["experiment_id"],self.cfg["formal_cases"][-1]["experiment_id"]),(80,"S3-20260813-301","S3-20260813-380"))
 ...
 def test_13_failure_replays_accepted_prefix(self):
  ...
  with tempfile.TemporaryDirectory() as td:
   ...
   for index,row in enumerate(rows,1):
    run=state/"runs"/f"{index:03d}_{row['experiment_id']}";run.mkdir();worker_rc=0 if index==1 else None;payloads={"attempt.json":{"schema_version":1,"status":"formal_started","experiment_id":row["experiment_id"],"attempt_number":1,"started_unix_ns":1},"command.json":{"argv":runner.command(ROOT,self.cfg,row,run),"cwd":str(ROOT),"environment":runner.environment(ROOT,self.cfg)},"resource_preflight.json":{"status":"accepted","hostname":self.cfg["runtime"]["hostname"],"logical_cpu":self.cfg["runtime"]["logical_cpu"],"smt_domain":self.cfg["runtime"]["smt_domain"],"collisions":[]},"worker_return.json":{"schema_version":1,"experiment_id":row["experiment_id"],"return_code":worker_rc},"runner_return.json":{"schema_version":1,"experiment_id":row["experiment_id"],"worker_return_code":worker_rc,"runner_return_code":0 if index==1 else 70}}
 ...
 def test_19_command_argv_cwd_environment_tamper_rejected(self):
  ...
  with tempfile.TemporaryDirectory() as td:
   run=Path(td);attempt={"schema_version":1,"status":"formal_started","experiment_id":eid,"attempt_number":1,"started_unix_ns":1};pre={"status":"accepted","hostname":self.cfg["runtime"]["hostname"],"logical_cpu":self.cfg["runtime"]["logical_cpu"],"smt_domain":self.cfg["runtime"]["smt_domain"],"collisions":[]};worker={"schema_version":1,"experiment_id":eid,"return_code":0};ret={"schema_version":1,"experiment_id":eid,"worker_return_code":0,"runner_return_code":0};registered={"argv":runner.command(ROOT,self.cfg,row,run),"cwd":str(ROOT),"environment":runner.environment(ROOT,self.cfg)}
   for name,obj in (("attempt.json",attempt),("resource_preflight.json",pre),("worker_return.json",worker),("runner_return.json",ret)):(run/name).write_bytes(c.canonical(obj))
 ...
 def test_21_monitor_shell_is_not_execution_process(self):
  tokens=["bash","-lc","monitor /home/shenwei01/.local/state/m_ofdft/s3_g3_al_multistructure_execution_r3_20260813"]
 ...
 def test_22_exact_runner_and_worker_tokens_are_execution_processes(self):
  for script in ("run_s3_g3_al_multistructure_execution_r3.py","run_s3_g3_al_multistructure_worker_r3.py"):
 ...
 def test_23_shell_substring_and_unregistered_python_are_not_collisions(self):
  self.assertFalse(c._registered_execution_process("bash",["bash","-c","python run_s3_g3_al_multistructure_execution_r3.py"]))
  self.assertFalse(c._registered_execution_process("python3",["python3","/tmp/run_s3_g3_al_multistructure_execution_r30.py"]))
 ...
 def test_25_r2_failure_closure_is_frozen(self):
  e=json.loads((ROOT/c.CONFIG_REL).read_text())["r2_operational_failure"];x=json.loads((ROOT/e["closure_path"]).read_text())
  self.assertEqual((x["status"],x["denominator"]["attempted"],x["denominator"]["accepted_prefix"],x["denominator"]["failed_experiment_id"]),("operational_failure_closed_no_retry",29,28,"S3-20260812-229"))
 def test_26_geometry_replay_uses_registered_basis(self):
  row=next(x for x in self.cfg["formal_cases"] if x["geometry_kind"]=="volume_conserving_tetragonal");observed={}
  def fake_replay(root,cfg,registered,result,density):observed["basis"]=c.legacy.build_basis;return {"accepted":True}
  with mock.patch.object(c.legacy,"replay_result",side_effect=fake_replay):c.replay_result(ROOT,self.cfg,row,c.row_identity(row),np.zeros(1))
  self.assertIs(observed["basis"],c.build_basis)
 def test_27_post_worker_failure_return_codes_are_distinct(self):
  row=self.cfg["formal_cases"][0];eid=row["experiment_id"]
  with tempfile.TemporaryDirectory() as td:
   run=Path(td);payloads={"attempt.json":{"schema_version":1,"status":"formal_started","experiment_id":eid,"attempt_number":1,"started_unix_ns":1},"command.json":{"argv":runner.command(ROOT,self.cfg,row,run),"cwd":str(ROOT),"environment":runner.environment(ROOT,self.cfg)},"resource_preflight.json":{"status":"accepted","hostname":self.cfg["runtime"]["hostname"],"logical_cpu":self.cfg["runtime"]["logical_cpu"],"smt_domain":self.cfg["runtime"]["smt_domain"],"collisions":[]},"worker_return.json":{"schema_version":1,"experiment_id":eid,"return_code":0},"runner_return.json":{"schema_version":1,"experiment_id":eid,"worker_return_code":0,"runner_return_code":70,"reason":"post_worker_replay_failed"}}
   for name,obj in payloads.items():(run/name).write_bytes(c.canonical(obj))
   t={"attempt_sha256":{eid:c.sha_path(run/"attempt.json")},"runner_return_sha256":{eid:c.sha_path(run/"runner_return.json")}}
   worker,ret=collector.envelope(ROOT,self.cfg,row,run,t,False);self.assertEqual((worker["return_code"],ret["runner_return_code"]),(0,70))
 def test_28_full_preflight_allows_generic_dftpy_but_rejects_registered_worker(self):
  def process(root,pid,name,argv,cpus="74"):
   p=root/str(pid);p.mkdir();(p/"status").write_text(f"Name:\t{name}\nPPid:\t1\nCpus_allowed_list:\t{cpus}\n");(p/"cmdline").write_bytes(b"\0".join(x.encode() for x in argv)+b"\0")
  with tempfile.TemporaryDirectory() as td:
   proc=Path(td);process(proc,7001,"python3",["python3","-c","import time; time.sleep(20)","dftpy_audit_probe"])
   kwargs={"proc_root":proc,"current_pid":9999,"topology_reader":lambda _:"74,150","hostname":self.cfg["runtime"]["hostname"]}
   out=c.resource_preflight(self.cfg,**kwargs)
   self.assertEqual((out["status"],out["collisions"]),("accepted",[]))
   process(proc,7002,"python3",["python3","/tmp/run_s3_g3_al_multistructure_worker_r3.py"])
   with self.assertRaises(ValueError):c.resource_preflight(self.cfg,**kwargs)
