...
import generate_s1_g1_thermodynamic_label_audit_r4 as GENERATOR  # noqa: E402
...
class S1G1ThermodynamicLabelAuditR4GeneratorTest(unittest.TestCase):
    @classmethod
    ...
    def test_namespace_is_exact_all_new_forty(self) -> None:
        ...
        self.assertEqual(
            tuple(GENERATOR.NEW_TO_LOGICAL), GENERATOR.R4_AUDIT_IDS
        )
        self.assertEqual(
            tuple(GENERATOR.NEW_TO_LOGICAL.values()),
            tuple(f"S1-20260806-{value:03d}" for value in expected_logical_suffixes),
        )
        self.assertEqual(GENERATOR.R1_REUSED_AUDIT_IDS, ())
        self.assertEqual(len(GENERATOR.R4_AUDIT_IDS), 40)
        ...
        self.assertEqual(
            ...
            set(GENERATOR.R4_AUDIT_IDS),
        )
        self.assertEqual(GENERATOR.EXECUTION_ORDER, GENERATOR.R4_AUDIT_IDS)
    ...
    def test_plan_preserves_logical_slots_and_uses_p1_then_p2(self) -> None:
        ...
        self.assertEqual(len(self.new_matrix), 40)
        by_logical = {
        ...
        self.assertEqual(
            by_logical["S1-20260806-024"]["evidence_origin"], "r4_executed"
        )
        self.assertEqual(
            by_logical["S1-20260806-034"]["evidence_origin"], "r4_executed"
        )
        self.assertEqual(
            [row["execution_phase"] for row in self.new_matrix[:12]], ["P1"] * 12
        )
        self.assertEqual(
            {row["execution_phase"] for row in self.new_matrix[12:]}, {"P2"}
        )
        self.assertEqual(
    ...
    def test_manifest_keeps_r1_schema_and_binds_all_derived_hashes(self) -> None:
        ...
        self.assertEqual(observed[11]["execution_phase"], "P1")
        self.assertEqual(observed[12]["execution_phase"], "P2")
        for row in observed:
    ...
    def test_r1_bridge_retains_ten_historical_anchors_but_reuses_none(self) -> None:
        ...
        self.assertEqual(bridge["reused_logical_ids"], [])
        self.assertEqual(bridge["reused_runs"], {})
        self.assertEqual(
            tuple(bridge["historical_accepted_runs"]),
            GENERATOR.R1_HISTORICAL_ACCEPTED_IDS,
        )
        self.assertEqual(
            bridge["historical_accepted_scientific_denominator_contribution"], 0
        )
        for experiment_id, anchor in bridge["historical_accepted_runs"].items():
            self.assertEqual(
    ...
    def test_r2_stopped_bridge_is_immutable_and_never_reused(self) -> None:
        bridge = GENERATOR.build_r2_stopped_bridge(PROJECT_ROOT)
        self.assertEqual(bridge["terminal_status"], "stopped")
        self.assertEqual(bridge["failed_experiment_id"], "S1-20260806-041")
        self.assertEqual(bridge["failed_logical_experiment_id"], "S1-20260806-034")
        self.assertEqual(bridge["attempted_ids_exact"], ["S1-20260806-041"])
        self.assertEqual(len(bridge["registered_but_unattempted_ids_exact"]), 29)
        self.assertEqual(bridge["accepted_scientific_denominator_contribution"], 0)
        self.assertTrue(bridge["r4_reuse_forbidden"])
        self.assertFalse(bridge["analysis_present"])
        self.assertFalse(bridge["supervisor_completion_present"])
        self.assertEqual(bridge["parser_exit_code"], 1)
        self.assertEqual(bridge["runner_exit_code"], 97)
        self.assertEqual(
            bridge["detachment_attestation"],
            {
                "path": GENERATOR.R2_DETACHMENT_PATH.as_posix(),
                "blob_oid": GENERATOR.R2_DETACHMENT_BLOB_OID,
                "sha256": GENERATOR.R2_DETACHMENT_SHA256,
            },
        )
        self.assertEqual(
            bridge["first_attempt_marker"],
            {
                "path": GENERATOR.R2_FIRST_MARKER_PATH.as_posix(),
                "blob_oid": GENERATOR.R2_FIRST_MARKER_BLOB_OID,
                "sha256": GENERATOR.R2_FIRST_MARKER_SHA256,
            },
        )

    def test_r2_stopped_bridge_rejects_head_marker_blob_tampering(self) -> None:
        real_object_id = GENERATOR._git_object_id

        def object_id(root: Path, commit: str, relative: str, expected_type: str) -> str:
            if commit == "HEAD" and relative == GENERATOR.R2_FIRST_MARKER_PATH.as_posix():
                return "0" * 40
            return real_object_id(root, commit, relative, expected_type)

        with mock.patch.object(GENERATOR, "_git_object_id", side_effect=object_id):
            with self.assertRaisesRegex(ValueError, "R2 first marker blob changed"):
                GENERATOR.build_r2_stopped_bridge(PROJECT_ROOT)
    ...
    def test_dry_run_is_reproducible_after_outputs_exist_but_write_is_single_use(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            output_config = root / "existing-config.json"
            output_manifest = root / "existing-manifest.tsv"
            input_root = root / "existing-inputs"
            output_config.write_text("occupied\n", encoding="utf-8")
            output_manifest.write_text("occupied\n", encoding="utf-8")
            input_root.mkdir()
            with mock.patch.object(
                GENERATOR, "_require_preregistration_targets_absent"
            ) as absence_gate:
            ...
            absence_gate.assert_not_called()
            self.assertEqual(len(prepared["manifest_rows"]), 40)
            self.assertEqual(len(prepared["artifacts"]), 160)

        with (
            mock.patch.object(GENERATOR, "_require_clean"),
            mock.patch.object(
                GENERATOR,
                "_require_preregistration_targets_absent",
                side_effect=ValueError("single-use namespace occupied"),
            ) as absence_gate,
        ):
            with self.assertRaisesRegex(ValueError, "single-use namespace occupied"):
                GENERATOR.prepare(PROJECT_ROOT, write=True)
        absence_gate.assert_called_once()
    ...
    def test_prepare_copies_all_scientific_and_runtime_gates_from_r1(self) -> None:
        ...
        self.assertEqual(config["registered_experiment_ids"], list(GENERATOR.R4_AUDIT_IDS))
        ...
        self.assertEqual(
            config["runtime"],
            GENERATOR.build_r4_runtime_contract(PROJECT_ROOT, self.r1_config),
        )
        self.assertEqual(config["manifest"]["row_count"], 40)
        self.assertEqual(config["input_derivation"]["derived_file_count"], 160)
        self.assertEqual(
            config["coverage"]["r2_rejected_archive_reused_count"], 0
        )
        self.assertEqual(
            config["coverage"]["r3_historical_accepted_excluded_count"], 1
        )
        self.assertEqual(
            config["coverage"]["r3_rejected_archive_reused_count"], 0
        )
        self.assertEqual(
            config["r2_stopped_bridge"],
            GENERATOR.build_r2_stopped_bridge(PROJECT_ROOT),
        )
        self.assertEqual(
            config["r3_stopped_bridge"],
            GENERATOR.build_r3_stopped_bridge(PROJECT_ROOT),
        )
        self.assertEqual(
        ...
        self.assertTrue(
            config["execution"][
                "production_parser_registration_required_before_solver"
            ]
        )
