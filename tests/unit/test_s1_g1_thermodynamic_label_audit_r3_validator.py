...
import validate_s1_g1_thermodynamic_label_audit_r3 as VALIDATOR  # noqa: E402
...
def exact_launch_record(
    ...
    launcher_path = (
        root / "scripts/launch_s1_g1_thermodynamic_label_audit_r3.py"
    )
    return {
        "schema_version": 1,
...
class S1G1ThermodynamicLabelAuditR3ValidatorTest(unittest.TestCase):
    def test_cli_detachment_mode_requires_live_sealed_inputs(self) -> None:
        config = logical_config()
        argv = [
            str(SCRIPTS / "validate_s1_g1_thermodynamic_label_audit_r3.py"),
    ...
    def test_cli_pilot_alias_uses_all_new_twelve_run_k_gate(self) -> None:
        ...
        accepted = {
            ...
            "pairs": [],
            "accepted": True,
            "pilot_ids": list(VALIDATOR.R3_PILOT_IDS),
        }
        argv = [
            str(SCRIPTS / "validate_s1_g1_thermodynamic_label_audit_r3.py"),
            "--require-pilot-gate",
    ...
    def test_final_science_replay_compares_regenerated_artifacts_byte_exactly(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch(
                "analyze_s1_g1_thermodynamic_label_audit_r3.analyze",
    ...
    def test_detachment_attestation_replays_exact_hup_and_process_contract(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for relative, data in (
                ...
                (
                    Path("scripts/run_s1_g1_thermodynamic_label_audit_r3.sh"),
                    b"#!/bin/sh\n",
                ),
                (
                    Path("scripts/launch_s1_g1_thermodynamic_label_audit_r3.py"),
                    b"# frozen launcher\n",
                ),
            ):
                path = root / relative
            ...
            registered = {
                ...
                "runner_sha256": digest(
                    root / "scripts/run_s1_g1_thermodynamic_label_audit_r3.sh"
                ),
            }
            process = {
                "pid": 1234,
            ...
            subprocess.check_call(
                ["git", "-C", str(root), "config", "user.email", "r3@test.invalid"]
            )
            subprocess.check_call(
                ["git", "-C", str(root), "config", "user.name", "R3 Test"]
            )
            subprocess.check_call(
                [
                    "git",
    ...
    def test_supervisor_completion_binds_external_receipts_and_exact_git_parent(
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            first_marker = write(
                ...
                / f"{VALIDATOR.R3_AUDIT_IDS[0]}.json",
                b"{}\n",
            )
            subprocess.check_call(
            ...
            analysis_relative = Path(
                "analysis/s1/g1_thermodynamic_label_audit_r3_20260807/summary.json"
            )
            analysis_path = write(
                analysis_relative,
                (
            ...
            with (
                ...
                self.assertTrue(
                    any(
                        "first R3 attempt marker parent" in failure
                        for failure in causal_errors
                    )
                )
    ...
    def test_barrier_failure_is_exact_scope_and_forbids_later_attempt_marker(
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            barrier_name = "production-parser-registration-before-001"
            python_path = "/registered/python"
            command = [
                python_path,
                "-s",
                str(root / "scripts/validate_s1_g1_thermodynamic_label_audit_r3.py"),
                str(root / VALIDATOR.MANIFEST_PATH),
                "--config",
                str(root / VALIDATOR.CONFIG_PATH),
                "--scientific-config",
    ...
    def test_namespace_is_exact_all_new_forty(self) -> None:
        ...
        self.assertEqual(len(VALIDATOR.R3_AUDIT_IDS), 40)
        ...
        self.assertEqual(VALIDATOR.EXECUTION_ORDER, VALIDATOR.R3_AUDIT_IDS)
    ...
    def test_barrier_spec_reconstructs_per_run_and_analysis_release_commands(self) -> None:
        ...
        for barrier_name, expected_identity, suffix in (
            ("production-parser-registration-before-001", (None, None), []),
            (
                "k-gate-after-012",
                ("S1-20260807-012", "S1-20260806-038"),
                ["--require-k-gate"],
            ),
            (
                "final-all-after-040",
                ("S1-20260807-040", "S1-20260806-033"),
                ["--require-all-runs"],
            ),
        ):
        ...
        self.assertEqual(
            (effective, logical),
            (VALIDATOR.R3_AUDIT_IDS[-1], VALIDATOR.NEW_TO_LOGICAL[VALIDATOR.R3_AUDIT_IDS[-1]]),
        )
        self.assertEqual(
            command,
            [
                python_path,
                "-s",
                str(root / "scripts/analyze_s1_g1_thermodynamic_label_audit_r3.py"),
    ...
    def test_fixed_resolver_does_not_use_new_id_arithmetic(self) -> None:
        self.assertEqual(
            VALIDATOR.effective_id("S1-20260806-034"), "S1-20260807-001"
        )
        self.assertEqual(
            VALIDATOR.effective_id("S1-20260806-040"), "S1-20260807-002"
        )
        self.assertEqual(
            VALIDATOR.effective_id("S1-20260806-001"), "S1-20260807-013"
        )
        self.assertEqual(
            VALIDATOR.logical_id("S1-20260807-033"), "S1-20260806-022"
        )
    ...
    def test_config_resolver_fails_closed_on_matrix_tampering(self) -> None:
        ...
        config["logical_run_matrix"][21]["physical_experiment_id"] = "S1-20260807-034"
    ...
    def test_half_quarter_gate_uses_logical_partner_table(self) -> None:
        ...
        self.assertEqual(result["half_experiment_id"], "S1-20260807-020")
        self.assertEqual(result["quarter_experiment_id"], "S1-20260807-033")
    ...
    def test_k_gate_contains_six_all_new_anchor_pairs(self) -> None:
        ...
        self.assertEqual(recovery["coarse_experiment_id"], "S1-20260807-001")
        self.assertEqual(recovery["reference_experiment_id"], "S1-20260807-002")
    ...
    def test_accepted_status_binds_physical_and_logical_ids(self) -> None:
        ...
        self.assertEqual(
            VALIDATOR._status_payload("S1-20260807-033", accepted=True), expected
        )
        with tempfile.TemporaryDirectory() as temporary:
            path = Path(temporary) / VALIDATOR.STATUS_NAME
            path.write_text(json.dumps(expected), encoding="utf-8")
            self.assertIsNone(
                VALIDATOR._status_failure_r3(
                    path, "S1-20260807-033", accepted=True
                )
            )
    ...
    def test_failure_policy_is_r3_namespaced_and_restores_r1_globals(self) -> None:
        original = {
    ...
    def test_manifest_validator_freezes_science_and_translates_only_partners(self) -> None:
        ...
        for index, physical in enumerate(VALIDATOR.R3_AUDIT_IDS, 1):
            ...
            row.update(
                {
                    ...
                    "execution_phase": "P1" if physical in VALIDATOR.R3_PILOT_IDS else "P2",
                    ...
                    "suffix": VALIDATOR._expected_r3_suffix(old["suffix"]),
                    "input_sha256": "0" * 64,
            ...
            rows.append(row)
        config = {
            "registered_experiment_ids": list(VALIDATOR.R3_AUDIT_IDS),
    ...
    def test_replay_effective_dispatches_historical_slots_to_new_r3_runs(self) -> None:
        config = logical_config()
        new_row = {"experiment_id": "S1-20260807-003"}
        with (
            mock.patch.object(
                R1,
                "replay_evidence",
            ...
            mock.patch.object(
                VALIDATOR, "replay_evidence", return_value=({"source": "r3"}, [])
            ) as replay_r3,
        ):
        ...
        self.assertEqual(payload, {"source": "r3"})
        self.assertEqual(failures, [])
        replay_r1.assert_not_called()
        replay_r3.assert_called_once_with(
            PROJECT_ROOT,
            config,
            new_row,
            require_committed=True,
