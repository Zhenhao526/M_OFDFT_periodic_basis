...
import generate_s1_g1_thermodynamic_label_audit_r3 as REGISTRATION  # noqa: E402
import launch_s1_g1_thermodynamic_label_audit_r3 as LAUNCHER  # noqa: E402


RUNNER = SCRIPTS / "run_s1_g1_thermodynamic_label_audit_r3.sh"


class S1G1ThermodynamicLabelAuditR3RunnerTest(unittest.TestCase):
    ...
    def test_attempt_is_committed_before_solver_and_no_retry_is_fail_closed(self) -> None:
        ...
        production_registration = text.index(
            "run_gate production-parser-registration-before-001"
        )
        ...
        self.assertLess(production_registration, loop)
        self.assertLess(production_registration, marker)
        self.assertLess(production_registration, solver)
        self.assertLess(marker, solver)
        self.assertLess(marker_validator, solver)
        self.assertIn("os.O_CREAT | os.O_EXCL", text)
        self.assertIn('git commit -m "start G1 thermodynamic-label audit R3 $experiment_id"', text)
    ...
    def test_barriers_and_state_namespace_are_frozen(self) -> None:
        text = RUNNER.read_text(encoding="utf-8")
        self.assertNotIn("--require-pilot-gate", text)
        self.assertIn("--require-k-gate", text)
        self.assertIn("run_gate k-gate-after-012", text)
        self.assertIn("run_gate final-all-after-040", text)
        ...
        self.assertEqual(len(REGISTRATION.R3_AUDIT_IDS), 40)
