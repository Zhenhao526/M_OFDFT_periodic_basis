...
import validate_s1_g1_thermodynamic_label_audit_r4 as VALIDATOR  # noqa: E402
...
def exact_launch_record(
    ...
    launcher_path = (
        root / "scripts/launch_s1_g1_thermodynamic_label_audit_r4.py"
    )
    return {
        "schema_version": 1,
...
class S1G1ThermodynamicLabelAuditR4ValidatorTest(unittest.TestCase):
    def test_cli_detachment_mode_requires_live_sealed_inputs(self) -> None:
        config = logical_config()
        argv = [
            str(SCRIPTS / "validate_s1_g1_thermodynamic_label_audit_r4.py"),
    ...
    def test_cli_pilot_alias_uses_all_new_twelve_run_k_gate(self) -> None:
        registration = {
            "preregistration_commit": "a" * 40,
            "r1_bridge": {"reused": {}},
        }
        accepted = {
            ...
            "pairs": [],
            "accepted": True,
            "pilot_ids": list(VALIDATOR.R4_PILOT_IDS),
        }
        argv = [
            str(SCRIPTS / "validate_s1_g1_thermodynamic_label_audit_r4.py"),
            "--require-pilot-gate",
        ...
        pilot.assert_called_once_with(
            PROJECT_ROOT, mock.ANY, [], require_committed=True
        )
    ...
    def test_final_science_replay_compares_regenerated_artifacts_byte_exactly(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch(
                "analyze_s1_g1_thermodynamic_label_audit_r4.analyze",
    ...
    def test_detachment_attestation_replays_exact_hup_and_process_contract(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for relative, data in (
                ...
                (
                    Path("scripts/run_s1_g1_thermodynamic_label_audit_r4.sh"),
                    b"#!/bin/sh\n",
                ),
                (
                    Path("scripts/launch_s1_g1_thermodynamic_label_audit_r4.py"),
                    b"# frozen launcher\n",
                ),
            ):
                path = root / relative
            ...
            registered = {
                ...
                "runner_sha256": digest(
                    root / "scripts/run_s1_g1_thermodynamic_label_audit_r4.sh"
                ),
            }
            process = {
                "pid": 1234,
            ...
            subprocess.check_call(
                ["git", "-C", str(root), "config", "user.email", "r4@test.invalid"]
            )
            subprocess.check_call(
                ["git", "-C", str(root), "config", "user.name", "R4 Test"]
            )
            subprocess.check_call(
                [
                    "git",
    ...
    def test_supervisor_completion_binds_external_receipts_and_exact_git_parent(
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            first_marker = write(
                ...
                / f"{VALIDATOR.R4_AUDIT_IDS[0]}.json",
                b"{}\n",
            )
            subprocess.check_call(
            ...
            with (
                ...
                self.assertTrue(
                    any(
                        "first R4 attempt marker parent" in failure
                        for failure in causal_errors
                    )
                )
    ...
    def test_barrier_failure_is_exact_scope_and_forbids_later_attempt_marker(
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            barrier_name = "production-parser-registration-before-041"
            python_path = "/registered/python"
            command = [
                python_path,
                "-s",
                str(root / "scripts/validate_s1_g1_thermodynamic_label_audit_r4.py"),
                str(root / VALIDATOR.MANIFEST_PATH),
                "--config",
                str(root / VALIDATOR.CONFIG_PATH),
                "--scientific-config",
    ...
    def test_namespace_is_exact_all_new_forty(self) -> None:
        self.assertEqual(
        ...
        self.assertEqual(VALIDATOR.R1_REUSED_AUDIT_IDS, ())
        self.assertEqual(len(VALIDATOR.R4_AUDIT_IDS), 40)
        ...
        self.assertEqual(VALIDATOR.EXECUTION_ORDER, VALIDATOR.R4_AUDIT_IDS)
    ...
    def test_barrier_spec_reconstructs_per_run_and_analysis_release_commands(self) -> None:
        ...
        for barrier_name, expected_identity, suffix in (
            ("production-parser-registration-before-041", (None, None), []),
            (
                "k-gate-after-052",
                ("S1-20260807-052", "S1-20260806-038"),
                ["--require-k-gate"],
            ),
            (
                "final-all-after-080",
                ("S1-20260807-080", "S1-20260806-033"),
                ["--require-all-runs"],
            ),
        ):
        ...
        self.assertEqual(
            (effective, logical),
            (VALIDATOR.R4_AUDIT_IDS[-1], VALIDATOR.NEW_TO_LOGICAL[VALIDATOR.R4_AUDIT_IDS[-1]]),
        )
        self.assertEqual(
            command,
            [
                python_path,
                "-s",
                str(root / "scripts/analyze_s1_g1_thermodynamic_label_audit_r4.py"),
    ...
    def test_fixed_resolver_does_not_use_new_id_arithmetic(self) -> None:
        self.assertEqual(
            VALIDATOR.effective_id("S1-20260806-034"), "S1-20260807-041"
        )
        self.assertEqual(
            VALIDATOR.effective_id("S1-20260806-040"), "S1-20260807-042"
        )
        self.assertEqual(
            VALIDATOR.effective_id("S1-20260806-001"), "S1-20260807-053"
        )
        self.assertEqual(
            VALIDATOR.logical_id("S1-20260807-073"), "S1-20260806-022"
        )
        for historical_or_unknown in (
            "S1-20260806-024",
            "S1-20260806-071",
        ):
            with self.subTest(experiment_id=historical_or_unknown):
                with self.assertRaisesRegex(ValueError, "unknown effective"):
                    VALIDATOR.logical_id(historical_or_unknown)
    ...
    def test_config_resolver_fails_closed_on_matrix_tampering(self) -> None:
        ...
        config["logical_run_matrix"][21]["physical_experiment_id"] = "S1-20260807-074"
    ...
    def test_half_quarter_gate_uses_logical_partner_table(self) -> None:
        ...
        self.assertEqual(result["half_experiment_id"], "S1-20260807-060")
        self.assertEqual(result["quarter_experiment_id"], "S1-20260807-073")
    ...
    def test_k_gate_contains_six_all_new_anchor_pairs(self) -> None:
        config = logical_config()
        ...
        self.assertEqual(recovery["coarse_experiment_id"], "S1-20260807-041")
        self.assertEqual(recovery["reference_experiment_id"], "S1-20260807-042")
    ...
    def test_accepted_status_binds_physical_and_logical_ids(self) -> None:
        ...
        self.assertEqual(
            VALIDATOR._status_payload("S1-20260807-073", accepted=True), expected
        )
        with tempfile.TemporaryDirectory() as temporary:
            path = Path(temporary) / VALIDATOR.STATUS_NAME
            path.write_text(json.dumps(expected), encoding="utf-8")
            self.assertIsNone(
                VALIDATOR._status_failure_r4(
                    path, "S1-20260807-073", accepted=True
                )
            )
    ...
    def test_failure_policy_is_r4_namespaced_and_restores_r1_globals(self) -> None:
        original = {
    ...
    def test_manifest_validator_freezes_science_and_translates_only_partners(self) -> None:
        ...
        for index, physical in enumerate(VALIDATOR.R4_AUDIT_IDS, 1):
            ...
            row.update(
                {
                    ...
                    "execution_phase": "P1" if physical in VALIDATOR.R4_PILOT_IDS else "P2",
                    ...
                    "suffix": VALIDATOR._expected_r4_suffix(old["suffix"]),
                    "input_sha256": "0" * 64,
            ...
            rows.append(row)
        config = {
            "registered_experiment_ids": list(VALIDATOR.R4_AUDIT_IDS),
    ...
    def test_replay_effective_dispatches_historical_slots_to_new_r4_runs(self) -> None:
        config = logical_config()
        new_row = {"experiment_id": "S1-20260807-043"}
        with (
            mock.patch.object(
                R1,
                "replay_evidence",
                side_effect=AssertionError("historical R1 evidence was dispatched"),
            ) as replay_r1,
            mock.patch.object(
                VALIDATOR, "replay_evidence", return_value=({"source": "r4"}, [])
            ) as replay_r4,
        ):
        ...
        self.assertEqual(payload, {"source": "r4"})
        self.assertEqual(failures, [])
        replay_r1.assert_not_called()
        replay_r4.assert_called_once_with(
            PROJECT_ROOT,
            config,
            new_row,
            require_committed=True,
            require_replay_status=True,
            scientific_config_path=None,
            scientific_manifest_path=None,
        )

    def test_replay_passes_sealed_scientific_paths_to_label_parser(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            experiment_id = "S1-20260807-043"
    ...
    def test_parser_registration_and_metadata_failures_are_not_misclassified_as_science(self) -> None:
        for diagnostic in (
            "ValueError: configuration execution order differs\n",
            "ValueError: metadata protocol_revision differs\n",
        ):
            with self.subTest(diagnostic=diagnostic), tempfile.TemporaryDirectory() as temporary:
                root = Path(temporary)
                path = root / "thermodynamic_label_parser.stderr.txt"
            ...
            self.assertEqual(
                payload["failure_class"], "parser_contract_or_evidence_failure"
            )
            self.assertEqual(
                payload["failure_categories"], ["parser_contract_or_evidence"]
            )

    def test_explicit_thermodynamic_identity_failure_remains_scientific_rejection(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            path = root / "thermodynamic_label_parser.stderr.txt"
            path.write_text("ValueError: thermodynamic identity failed\n", encoding="utf-8")
            payload = VALIDATOR.classify_noncore_failure(
                root,
                "S1-20260807-041",
                "S1-20260806-034",
        ...
        self.assertEqual(payload["failure_categories"], ["thermodynamic_identity"])
...
class PreCounterpartFailureContractTest(unittest.TestCase):
    def _fixture(self, root: Path, exit_code: int = 98) -> Path:
        run = root / "run"
        runtime = run / "mpi_runtime_audit"
            ...
            json.dumps(
                {
                    ...
                    "namespace_host_status": "rejected",
                    "counterpart_audit_status": None,
                }
            ),
            encoding="utf-8",
        )
        (runtime / "audit.json").write_text(
            json.dumps(
                {
                    "schema_version": 2,
        ...
        (runtime / "objects.tsv").write_text(
            "pid\trole\trank\tmapped_path\n", encoding="utf-8"
        )
        (namespace / "host_status.json").write_text(
            json.dumps(
                {
                    "schema_version": 1,
                    "status": "rejected",
                    ...
                    "counterpart_audit": {},
                    "failure_reasons": [
                        f"namespace_payload_exit_code:{exit_code}"
                    ],
                }
            ),
            encoding="utf-8",
            ...
            json.dumps(
                {
                    ...
                    "audit_launcher_exit_code": exit_code,
                }
            ),
            encoding="utf-8",
        )
    ...
    def test_consistent_nonzero_early_stop_needs_no_counterpart(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            run = self._fixture(Path(temporary))
            self.assertTrue(VALIDATOR._valid_pre_counterpart_runtime_failure(run))

    def test_zero_payload_exit_cannot_use_early_stop_exception(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            run = self._fixture(Path(temporary), exit_code=0)
            self.assertFalse(VALIDATOR._valid_pre_counterpart_runtime_failure(run))

    def test_claimed_counterpart_status_requires_counterpart_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            run = self._fixture(Path(temporary))
            path = run / "run_status.json"
            status = json.loads(path.read_text())
            status["counterpart_audit_status"] = "rejected"
            path.write_text(json.dumps(status), encoding="utf-8")
            self.assertFalse(VALIDATOR._valid_pre_counterpart_runtime_failure(run))

    def test_partial_or_symbolic_counterpart_evidence_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            run = self._fixture(Path(temporary))
            runtime = run / "mpi_runtime_audit"
            (runtime / "counterparts.tsv").write_text("partial\n", encoding="utf-8")
            self.assertFalse(VALIDATOR._valid_pre_counterpart_runtime_failure(run))
            (runtime / "counterparts.tsv").unlink()
            (runtime / "counterpart_audit.json").symlink_to("missing-target")
            self.assertFalse(VALIDATOR._valid_pre_counterpart_runtime_failure(run))


class StopConfirmationProofTest(unittest.TestCase):
    def _write_audit(self, run: Path, processes: list[dict[str, object]]) -> None:
        directory = run / "mpi_runtime_audit"
        directory.mkdir(parents=True)
        (directory / "audit.json").write_text(
            json.dumps({"processes": processes}), encoding="utf-8"
        )

    def test_all_four_bracketed_rank_proofs_are_required(self) -> None:
        processes = [
            {
                "role": "rank",
                "rank": rank,
                "initial_map_capture_stop_confirmed": True,
                "initial_map_capture_stop_state_before": "t",
                "initial_map_capture_stop_state_after": "t",
            }
            for rank in range(4)
        ]
        with tempfile.TemporaryDirectory() as temporary:
            run = Path(temporary) / "run"
            self._write_audit(run, processes)
            self.assertEqual(
                VALIDATOR._stop_confirmation_proof_failures(run, "S1-20260807-041"),
                [],
            )

    def test_running_or_missing_rank_proof_is_rejected(self) -> None:
        processes = [
            {
                "role": "rank",
                "rank": rank,
                "initial_map_capture_stop_confirmed": True,
                "initial_map_capture_stop_state_before": "T",
                "initial_map_capture_stop_state_after": "T" if rank != 2 else "R",
            }
            for rank in range(3)
        ]
        with tempfile.TemporaryDirectory() as temporary:
            run = Path(temporary) / "run"
            self._write_audit(run, processes)
            failures = VALIDATOR._stop_confirmation_proof_failures(
                run, "S1-20260807-041"
            )
        self.assertTrue(any("rank 2 lacks" in failure for failure in failures))
        self.assertTrue(any("rank set differs" in failure for failure in failures))
