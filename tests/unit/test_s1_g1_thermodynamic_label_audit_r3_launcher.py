...
LAUNCHER = PROJECT_ROOT / "scripts/launch_s1_g1_thermodynamic_label_audit_r3.py"
sys.path.insert(0, str(PROJECT_ROOT / "scripts"))
import launch_s1_g1_thermodynamic_label_audit_r3 as LAUNCHER_MODULE  # noqa: E402


class S1G1ThermodynamicLabelAuditR3LauncherTest(unittest.TestCase):
    ...
    def test_go_replacement_after_second_exact_cannot_reach_runner(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                environment["M_OFDFT_G1_R3_GO_SHA256"], validated_sha256
            )
    ...
    def _fixture_repository(self, temporary: Path) -> Path:
        ...
        config = {
            ...
            "execution": {
                ...
                "detachment_attestation_path": "orchestration/r3/detachment.json",
            ...
            "manifest": {
                "path": "config/S1_g1_thermodynamic_label_audit_r3_manifest.tsv",
                "sha256": manifest_sha256,
            },
        }
        (project / "config/S1_g1_thermodynamic_label_audit_r3.json").write_text(
            json.dumps(config) + "\n", encoding="utf-8"
        )
        (project / "config/S1_g1_thermodynamic_label_audit_r3_manifest.tsv").write_text(
            manifest_text, encoding="utf-8"
        )
        validator = project / "scripts/validate_s1_g1_thermodynamic_label_audit_r3.py"
        ...
        runner = project / "scripts/run_s1_g1_thermodynamic_label_audit_r3.sh"
        runner.write_text(
            "#!/usr/bin/env bash\n"
            "set -euo pipefail\n"
            "mkdir -p analysis/s1/g1_thermodynamic_label_audit_r3_20260807\n"
            "printf '%s\\n' '{\"protocol_revision\":\"S1-G1-THERMODYNAMIC-LABEL-AUDIT-R3\","
            ...
            "git add -- analysis/s1/g1_thermodynamic_label_audit_r3_20260807\n"
            "git commit -qm 'fixture accepted analysis'\n",
            encoding="utf-8",
        )
        validator.chmod(0o755)
        subprocess.check_call(["git", "init", "-q"], cwd=project)
        subprocess.check_call(["git", "config", "user.name", "R3 test"], cwd=project)
        subprocess.check_call(["git", "config", "user.email", "r3@example.invalid"], cwd=project)
    ...
    def test_registered_absolute_bash_builds_runner_command(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            config = json.loads(
                (
                    project / "config/S1_g1_thermodynamic_label_audit_r3.json"
            ...
            command = LAUNCHER_MODULE._runner_command(
                ...
                project,
                Path("config/S1_g1_thermodynamic_label_audit_r3_manifest.tsv"),
                Path("config/S1_g1_thermodynamic_label_audit_r3.json"),
            )
            ...
            self.assertEqual(
                ...
                [
                    ...
                    str(project / "config/S1_g1_thermodynamic_label_audit_r3.json"),
                    "/proc/self/fd/201",
                    "/proc/self/fd/202",
                ],
            )
    ...
    def test_detached_launch_hup_attestation_and_go(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            project = self._fixture_repository(temporary)
            state = temporary / "single-use-state"
            attestation = Path("orchestration/r3/detachment.json")
            self._run(
                "start",
                "--project-root",
                project,
            ...
            self.assertFalse(
                os.access(
                    project / "scripts/run_s1_g1_thermodynamic_label_audit_r3.sh",
                    os.X_OK,
                )
            )
            pid = int(launch["process"]["pid"])
