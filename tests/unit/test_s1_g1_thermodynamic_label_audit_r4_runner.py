...
import generate_s1_g1_thermodynamic_label_audit_r4 as REGISTRATION  # noqa: E402
import launch_s1_g1_thermodynamic_label_audit_r4 as LAUNCHER  # noqa: E402


RUNNER = SCRIPTS / "run_s1_g1_thermodynamic_label_audit_r4.sh"


class S1G1ThermodynamicLabelAuditR4RunnerTest(unittest.TestCase):
    ...
    def test_attempt_is_committed_before_solver_and_no_retry_is_fail_closed(self) -> None:
        ...
        production_registration = text.index(
            "run_gate production-parser-registration-before-041"
        )
        ...
        self.assertIn('git commit -m "start G1 thermodynamic-label audit R4 $experiment_id"', text)
    ...
    def test_barriers_and_state_namespace_are_frozen(self) -> None:
        ...
        self.assertIn("run_gate k-gate-after-052", text)
        self.assertIn("run_gate final-all-after-080", text)
        ...
        self.assertEqual(len(REGISTRATION.R4_AUDIT_IDS), 40)
