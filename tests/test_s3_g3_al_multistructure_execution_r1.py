...
import collect_s3_g3_al_multistructure_execution_r1 as collector
import run_s3_g3_al_multistructure_execution_r1 as runner
...
class T(unittest.TestCase):
 ...
 def test_13_failure_replays_accepted_prefix(self):
  ...
  with tempfile.TemporaryDirectory() as td:
   ...
   failure={"experiment_id":rows[-1]["experiment_id"],"return_code":70};(state/"failure.json").write_bytes(c.canonical(failure));t={"attempted":2,"accepted":1,"attempted_ids":ids[:2],"accepted_ids":ids[:1],"runner_return_code":70,"failure_sha256":c.sha_path(state/"failure.json"),"attempt_sha256":attempt_sha,"runner_return_sha256":return_sha,"result_sha256":{ids[0]:c.sha_path(state/"runs"/f"001_{ids[0]}"/"result.json")}}
