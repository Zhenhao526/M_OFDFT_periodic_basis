...
from validate_s3_g3_wt_coefficient_pilot_r1 import normalized_config
...
def test_28_prereg_normalization_is_format_independent():
    implementation={"status":"implementation_pending_preregistration","implementation_commit":"__FREEZE_IMPLEMENTATION_COMMIT__","threshold":1e-6}
    prereg={"threshold":1e-6,"implementation_commit":"a"*40,"status":"preregistered_no_execution"}
    assert normalized_config(json.dumps(implementation,indent=1).encode())==normalized_config(json.dumps(prereg,separators=(",",":")).encode())
    prereg["threshold"]=1e-5
    assert normalized_config(json.dumps(implementation).encode())!=normalized_config(json.dumps(prereg).encode())
