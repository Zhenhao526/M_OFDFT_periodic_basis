...
import numpy as np
import s3_g3_al_iso_v097_optimizer_recovery_common_r5 as c
import collect_s3_g3_al_iso_v097_optimizer_recovery_r5 as collector
import run_s3_g3_al_iso_v097_optimizer_recovery_r5 as runner
import run_s3_g3_al_iso_v097_optimizer_recovery_worker_r5 as worker
...
class T(unittest.TestCase):
 ...
 def synthetic(self,delta=.015):
  ...
  for row in self.cfg["formal_cases"]:
   ...
   out.append(r)
  return out
 def test_01_denominator(self):self.assertEqual(([x["experiment_id"] for x in self.cfg["formal_cases"]],len(self.cfg["formal_cases"])),([f"S3-20260813-{x}" for x in range(391,395)],4))
 ...
 def test_15_process_tokens(self):
  self.assertFalse(c._registered_execution_process("bash",["bash","monitor run_s3_g3_al_iso_v097_optimizer_recovery_r5.py"]));self.assertTrue(c._registered_execution_process("python3",["python3","/tmp/run_s3_g3_al_iso_v097_optimizer_recovery_worker_r5.py"]));self.assertFalse(c._registered_execution_process("python3",["python3","/tmp/run_s3_g3_al_iso_v097_optimizer_recovery_worker_r50.py"]))
 ...
 def test_22_nested_replay_receives_exact_target_cell(self):
  row=self.cfg["formal_cases"][1];result={**c.row_identity(row),"registered_initial_density_sha256":self.cfg["recovery_policy"]["registered_initial_density_sha256"]["uniform"],"recovery_algorithm":self.cfg["recovery_policy"]["algorithm"],"recovery_events":[],"accepted_step_trust_radii":[.05],"accepted_step_coefficients":[[0.]*22],"iterations":1};expected=c.cell_for_row(ROOT,self.cfg,row);seen={}
  def capture(root,cfg,r,res,density,cell):seen["cell"]=cell
  with mock.patch.object(c.legacy,"replay_result",return_value={"ok":True}),mock.patch.object(c,"replay_recovery_algorithm",side_effect=capture):c.replay_result(ROOT,self.cfg,row,result,np.zeros(1))
  np.testing.assert_allclose(seen["cell"],expected,rtol=0,atol=0)
