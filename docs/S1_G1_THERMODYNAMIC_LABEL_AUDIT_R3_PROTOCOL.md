# S1 G1 第三 smearing / 稠密 k 点热力学标签审计 R3 协议

状态：`protocol_frozen`（正式执行状态以已提交的 R3 config、逐点证据和
summary 为准）
协议日期：`2026-08-07`
范围：仅继续 G1 “第三 smearing / 稠密 k 点热力学标签”子项。

R3 不追溯改判 R1 或 R2，也不会将有限温度量冒充为严格零温标签。
它完整保留 R1 的 10 个历史 accepted run、R1-034 与 R2-041 的原始
证据链，但这些历史运行对 R3 验收分母一律贡献 0。R3 用 40 个全新 ID
在同一个当前冻结环境中重算全部 40 个逻辑槽位。

## 1. R1/R2 事实边界与 R3 修订原因
...
common-quarter / extra-dense 端点。R3 设计复核期间在正式远端重新执行
...
候选点不能作为同一当前 runtime 合同下的完整分母。R3 不放宽 runtime 门、
...
`c0b7d1cbdfea594c130f086b29f898102d441383`。该尝试对 R3 验收分母贡献为
...
`ce89b513a01964dbd34e24d6f570700bf478210a`。它对 R3 验收分母贡献为 0，
不允许作为 R3 的 scalar、field、runtime 或 logical-run 证据。R3 config
必须通过 `r2_stopped_bridge` 绑定该终止链。

## 2. 科学问题、热力学语义与数值设置

R3 完全继承 R1 的科学问题、物理输入、数值轴和所有阈值。不得修改
...
R3 config 中的 `numerical_axes`、`output_contract`、
...
R3 最终分析仍使用 R1 定义的 40 个逻辑槽位，但每个槽位的有效证据 ID
都来自 R3。R1 的 10 个历史 accepted ID 只出现在 provenance bridge 中，
...
| R3 新 ID | R1 逻辑槽位 | 用途 |
|---|---|---|
| 001 | 034 | Mg 1.10 common-dense quarter，替代 R1 中断槽位 |
| 002 | 040 | Mg 1.10 extra-dense quarter，与 001 完成 k gate |
| 003, 004 | 024, 036 | Al 1.00 common/extra-dense quarter |
| 005, 006 | 031, 039 | Mg 1.00 common/extra-dense quarter |
| 007, 008 | 021, 035 | Al 0.90 common/extra-dense quarter |
| 009, 010 | 027, 037 | Al 1.10 common/extra-dense quarter |
| 011, 012 | 028, 038 | Mg 0.90 common/extra-dense quarter |
| 013–018 | 001–006 | Al/Mg 三端点 dense-standard field replay |
| 019–025 | 007–013 | Al 七点 common-dense half EOS |
| 026–032 | 014–020 | Mg 七点 common-dense half EOS |
| 033, 034 | 022, 023 | Al 0.94/0.97 common-dense quarter |
| 035, 036 | 025, 026 | Al 1.03/1.06 common-dense quarter |
| 037, 038 | 029, 030 | Mg 0.94/0.97 common-dense quarter |
| 039, 040 | 032, 033 | Mg 1.03/1.06 common-dense quarter |

R3 新 ID 的固定执行顺序是 `S1-20260807-001, ..., 040`。配置中必须同时保留
40 行 `logical_run_matrix` 和 40 行 `new_run_matrix`，且两者都只能解析为
R3 新 ID。分析器不得将历史 R1/R2 run tree 拷贝、改名或计入本轮证据。

## 4. 输入的机械派生和预注册边界

R3 generator 必须从 R1 preregistration commit 中直接读取每个逻辑槽位的
`INPUT`、`STRU`、`KPT`、`metadata.json` Git blob，而不是从工作树或历史
run 重建科学输入。机械派生规则是：

1. `STRU` 和 `KPT` 必须逐字节相同；
2. `INPUT` 只允许 `suffix` 从 `g1tlr1_*` 改为 `g1tlr3_*`；
3. `metadata.json` 只允许改动
   `protocol_revision`、`experiment_id`、`suffix` 三个 key；
4. metadata 中的 R1 `dataset_kind` 保留为输入来源证据，执行协议归属由
   R3 config 和新 protocol revision 确定；
5. manifest 恰有 40 行，表头与 R1 `MANIFEST_FIELDS` 完全相同；
6. 逻辑参照 ID 仍保留在 TSV 中，它们的有效证据 ID 由 config 的逻辑矩阵
   fail-closed 解析。

正式 preregistration 之前，R3 config、manifest、input root、40 个 active run
prefix、40 个 failed archive prefix 以及 40 个 attempt-ledger marker 必须全部不存在。
实现必须先在父提交冻结；随后的 preregistration commit 只能新增：

- `config/S1_g1_thermodynamic_label_audit_r3.json`；
- `config/S1_g1_thermodynamic_label_audit_r3_manifest.tsv`；
- `inputs/s1/g1_thermodynamic_label_audit_r3/` 下的 160 个注册 blob。

预注册提交中不得出现 run、failure archive、detachment attestation 或 attempt
ledger 证据。

在实现冻结前，生成器产出的真实 40 行 config/manifest 必须通过 R3
...
R3 首先且只能执行 12 个 common/extra-dense 锚点：

```text
S1-20260807-001  logical 034  Mg 1.10 common-dense quarter
S1-20260807-002  logical 040  Mg 1.10 extra-dense quarter
S1-20260807-003  logical 024  Al 1.00 common-dense quarter
S1-20260807-004  logical 036  Al 1.00 extra-dense quarter
S1-20260807-005  logical 031  Mg 1.00 common-dense quarter
S1-20260807-006  logical 039  Mg 1.00 extra-dense quarter
S1-20260807-007  logical 021  Al 0.90 common-dense quarter
S1-20260807-008  logical 035  Al 0.90 extra-dense quarter
S1-20260807-009  logical 027  Al 1.10 common-dense quarter
S1-20260807-010  logical 037  Al 1.10 extra-dense quarter
S1-20260807-011  logical 028  Mg 0.90 common-dense quarter
S1-20260807-012  logical 038  Mg 0.90 extra-dense quarter
```

这 12 点组成完整的六组 common/extra field pair 和 Al/Mg 两条三端点 k 曲线。
它们必须逐点 accepted，并在 `k-gate-after-012` 屏障中全部通过。在此之前，
013–040 的 run prefix、archive prefix 和
attempt marker 全部禁止出现。

### P2：完成热力学标签矩阵

k gate 通过后，严格按 013–040 顺序执行。每个点必须完成“正式尝试
...
R1-034 暴露的是 host orchestration 生命周期问题，因此 R3 的外层监督进程是正式
...
/usr/bin/env -i HOME=/home/shenwei01 LC_ALL=C LOGNAME=shenwei01 \
  ...
  /usr/bin/python3 -s scripts/launch_s1_g1_thermodynamic_label_audit_r3.py ...
```

- 外部单次 state directory 固定为
  `/home/shenwei01/.local/state/m_ofdft/g1_thermodynamic_label_audit_r3_20260807`；
...
```text
orchestration/s1/g1_thermodynamic_label_audit_r3_20260807/attempts/<ID>.json
```
...
监督进程正常结束也不能只靠外部 `terminal.json` 宣布整个 R3 协议 accepted。runner
...
```text
orchestration/s1/g1_thermodynamic_label_audit_r3_20260807/supervisor_completion.json
```
...
所有不等式均为严格不等式，等于阈值即失败。R3 不新增事后阈值，也不降低
R1 标准。

### 7.1 完整性、标量与 EOS

- 逻辑注册点 accepted `40/40`：R1 reused `0/0` + R3 executed `40/40`；
- R1 历史 accepted 候选排除 `10/10`，对 R3 分母贡献 `0`；
...
每个 accepted R3 run 独立提交。首个失败必须先以独立 commit 保留 active run
及机器可读状态，再用相邻 commit 移入唯一
`failed_runs/runtime_relocation/<ID>/attempt-<failure_commit[:12]>`，然后停止。
分类和 inventory 必须使用 R3 专属名，不得覆盖 R1 证据。
...
exact-scope commit 存在、内容严格通过且 committed validator 再次重验接受时，R3 整体才可
标记最终 `accepted`。
