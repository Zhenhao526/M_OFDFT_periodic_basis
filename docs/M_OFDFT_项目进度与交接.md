...
| 最近可用提交 | `c53e030`（S1-R7 双展宽 EOS 矩阵、固定清单与自动验收器） |
...
| 下一项唯一动作 | 提交并执行 `config/S1_eos_run_manifest.tsv` 中 S1-029 至 S1-070 的 42 次核心 EOS 计算 |
...
- [x] 单元测试扩展至 22/22，通过输入、解析、k 点尾部稳定、固定清单、BM3 拟合与双展宽严格门槛检查；
- [ ] 选择项目对外发布许可证；
...
1. 阅读 `docs/S1_BASELINE_PROTOCOL.md` 的 S1-R1 至 S1-R7 及全部收敛摘要；
2. 执行 `scripts/run_s1_manifest.sh config/S1_eos_run_manifest.tsv`；脚本可跳过已提交点，失败点会先提交再停止；
3. 生成十四个结构点的 OFDFT/标准展宽 KSDFT EOS，并为 KSDFT 生成对应半展宽 EOS；
4. 用两套 KS `E_KS(sigma->0)` EOS 比较平衡体积与以 v100 为锚的相对能量，再决定展宽参数并复核非平衡截断/k 网格；
...
- 当前唯一动作：执行 S1-029 至 S1-070 的 42 次核心 EOS 计算矩阵。
...
- 最近可用状态：提交 `c53e030`；包含 S1-R7 固定参数、42 次双展宽 EOS 输入和实验 ID、逐点提交运行器与自动 BM3/展宽验收器。
- 对应环境：`environment/`，ABACUS v3.11.0-beta.5 CPU + OpenMPI 5.0.10 + LibXC 7.0.0。
- 已通过测试：22/22 单元测试；42 份输入独立重生与 14 对展宽结构哈希一致；`S0-20260805-001/002/003` 双重复 SCF 均收敛；S1-010 至 028 的 KSDFT 数值运行全部收敛。
- 已知失败：`S1-20260805-004` 的初始严格阈值失败已复核；Al 20³→24³ 的零温门槛失败已由 28³ 确认解决；G1 非平衡收敛、积分电子数、第二程序和发布限制仍未解决。
