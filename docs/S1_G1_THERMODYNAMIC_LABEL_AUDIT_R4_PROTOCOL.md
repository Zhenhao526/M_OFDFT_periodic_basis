# S1 G1 第三 smearing / 稠密 k 点热力学标签审计 R4 协议

状态：`protocol_frozen`（正式执行状态以已提交的 R4 config、逐点证据和
summary 为准）
协议日期：`2026-08-07`
范围：仅继续 G1 “第三 smearing / 稠密 k 点热力学标签”子项。

R4 不追溯改判 R1、R2 或 R3，也不会将有限温度量冒充为严格零温标签。
它完整保留 R1 的 10 个历史 accepted run、R1-034、R2-041、R3-001
accepted run 与 R3-002 失败归档的原始证据链，但这些历史运行对 R4
验收分母一律贡献 0。R4 用 40 个全新 ID 在同一个当前冻结环境中重算
全部 40 个逻辑槽位。

## 1. R1/R2/R3 事实边界与 R4 修订原因
...
R1 历史上已接受 10 个点，且每个 run tree 自引入提交后保持不变：
...
common-quarter / extra-dense 端点。R4 设计复核期间在正式远端重新执行
冻结的历史运行验证，发现 `021/035/037` 仍可重放，而
`024/036/031/039/027/028/038` 因当前 `/etc/ld.so.cache` 的 SHA-256
`5050081a5f73e989a66004b99f2770a2ddcb744dd990646e40884e561ae1861f`
不同于 R1 记录的
`f55b9dd80d0a18cfeed7e89500b16cf161bbdc753ffc6df620220559445cce50`
而 fail closed。七点的直接错误均为 mapped regular-file SHA-256 mismatch，
随后重算证据也因此不同；未观察到科学数值越阈。该变化不是科学阈值失败，但意味着 10 个
候选点不能作为同一当前 runtime 合同下的完整分母。R4 不放宽 runtime 门、
不选择性复用剩余 3 点，而是把 10 点全部降为只读历史来源，并在 config 中
锁定每个点的 introduction commit、run-tree OID、R1 input-tree OID 与关键
证据 blob；`historical_accepted_scientific_denominator_contribution` 固定为 0。
...
`c0b7d1cbdfea594c130f086b29f898102d441383`。该尝试对 R4 验收分母贡献为
0，不得事后改判为 accepted，也不得在 R1 ID 034 上重试。

R2 已在首点 `S1-20260806-041` 的 SCF 和 runtime audit 成功后终止。
失败发生在科学输出解析之前的注册适配层：R2 向冻结的 R1 parser
注入了 30 个 ID，但 R1 registration 仍以字面量 `40` 检查
`execution_order`，因此拒绝了正确的 30 行 R2 注册。R2 的失败 run
提交为 `ff26667f881e067f11ae0be088ea3659fea8a61a`，归档提交为
`f91a3006cd99fd3d11845da2fc8e9c88f662d951`，归档 tree OID 为
`ce89b513a01964dbd34e24d6f570700bf478210a`。它对 R4 验收分母贡献为 0，
不允许作为 R4 的 scalar、field、runtime 或 logical-run 证据。R4 config
必须通过 `r2_stopped_bridge` 绑定该终止链。

R3 使用 40 个新 ID 开始重算。`S1-20260807-001` 已完成 SCF、科学标签和
runtime 审计并被接受；`S1-20260807-002` 则在进入 SCF 前由 runtime 审计器
停止。复核确认基础 launcher 在发送 SIGSTOP 后检查完整的 `State:` 文本是否
包含 `T/t`；字段名 `State:` 自身含小写 `t`，导致代码没有真正等待进程停止，
便读取 `/proc/<pid>/maps`，从而产生瞬态 `initial_map_capture_failed`。R3-002
的 namespace payload 因此以 98 退出，按原契约不会生成只在 payload 成功后
才产生的 counterpart 文件；旧失败归档验证器又把该合法早停误报为 counterpart
证据缺失。R3 随即以 terminal barrier 停止，001/002 两个 ID 均已消耗，
003–040 从未尝试。

R4 不修改已冻结的基础 launcher 或 R2 KMP shim，而是注册独立 R4 shim：
精确解析 `/proc/<pid>/status` 的单字符状态码，只有 map capture 前后均为
`T` 或 `t` 才允许接受，并在 audit process row 中保存双侧停止证明。R4 的
失败归档仅在 run/audit/host/payload 四层一致记录非零早停、counterpart 状态
为 null 且两个 counterpart 路径均不存在时，允许没有 counterpart；accepted
路径和 payload 成功后的 counterpart 门完全不放宽。R4 config 必须通过
`r3_stopped_bridge` 绑定 R3 的十提交停止链，且 R3-001 对 R4 分母贡献固定为 0。

## 2. 科学问题、热力学语义与数值设置

R4 完全继承 R1 的科学问题、物理输入、数值轴和所有阈值。不得修改
...
R4 config 中的 `numerical_axes`、`output_contract`、
`thermodynamic_semantics`、`acceptance`、`runtime_audit`、`kmp_contract`
和 `rank_count` 必须与冻结 R1 config 逐字节等价。`runtime` 除
`wrappers.audit_launcher` 改为已哈希的 R4 stop-confirmed shim 外必须相同。

## 3. 40 点逻辑矩阵与 40 个新 ID

R4 最终分析仍使用 R1 定义的 40 个逻辑槽位，但每个槽位的有效证据 ID
都来自 R4。R1 的 10 个历史 accepted ID 只出现在 provenance bridge 中，
不得成为 `logical_to_effective_id` 的值。
...
| 043, 044 | 024, 036 | Al 1.00 common/extra-dense quarter |
| 045, 046 | 031, 039 | Mg 1.00 common/extra-dense quarter |
| 047, 048 | 021, 035 | Al 0.90 common/extra-dense quarter |
| 049, 050 | 027, 037 | Al 1.10 common/extra-dense quarter |
| 051, 052 | 028, 038 | Mg 0.90 common/extra-dense quarter |
| 053–058 | 001–006 | Al/Mg 三端点 dense-standard field replay |
| 059–065 | 007–013 | Al 七点 common-dense half EOS |
| 066–072 | 014–020 | Mg 七点 common-dense half EOS |
| 073, 074 | 022, 023 | Al 0.94/0.97 common-dense quarter |
| 075, 076 | 025, 026 | Al 1.03/1.06 common-dense quarter |
| 077, 078 | 029, 030 | Mg 0.94/0.97 common-dense quarter |
| 079, 080 | 032, 033 | Mg 1.03/1.06 common-dense quarter |

R4 新 ID 的固定执行顺序是 `S1-20260807-041, ..., S1-20260807-080`。配置中必须同时保留
40 行 `logical_run_matrix` 和 40 行 `new_run_matrix`，且两者都只能解析为
R4 新 ID。分析器不得将历史 R1/R2/R3 run tree 拷贝、改名或计入本轮证据。

## 4. 输入的机械派生和预注册边界

R4 generator 必须从 R1 preregistration commit 中直接读取每个逻辑槽位的
`INPUT`、`STRU`、`KPT`、`metadata.json` Git blob，而不是从工作树或历史
run 重建科学输入。机械派生规则是：

1. `STRU` 和 `KPT` 必须逐字节相同；
2. `INPUT` 只允许 `suffix` 从 `g1tlr1_*` 改为 `g1tlr4_*`；
3. `metadata.json` 只允许改动
   `protocol_revision`、`experiment_id`、`suffix` 三个 key；
4. metadata 中的 R1 `dataset_kind` 保留为输入来源证据，执行协议归属由
   R4 config 和新 protocol revision 确定；
5. manifest 恰有 40 行，表头与 R1 `MANIFEST_FIELDS` 完全相同；
6. 逻辑参照 ID 仍保留在 TSV 中，它们的有效证据 ID 由 config 的逻辑矩阵
   fail-closed 解析。

正式 preregistration 之前，R4 config、manifest、input root、40 个 active run
prefix、40 个 failed archive prefix 以及 40 个 attempt-ledger marker 必须全部不存在。
实现必须先在父提交冻结；随后的 preregistration commit 只能新增：

- `config/S1_g1_thermodynamic_label_audit_r4.json`；
- `config/S1_g1_thermodynamic_label_audit_r4_manifest.tsv`；
- `inputs/s1/g1_thermodynamic_label_audit_r4/` 下的 160 个注册 blob。

预注册提交中不得出现 run、failure archive、detachment attestation 或 attempt
ledger 证据。

在实现冻结前，生成器产出的真实 40 行 config/manifest 必须通过 R4
生产 parser 的 registration 入口，不得调用或改写 R1 registration，也不得
mock 整个 parser。至少同时验证 39/41 项 order、重复 ID、相同 ID 交换顺序、错误 execution index 和
canonical/sealed 字节不一致全部被拒绝。预注册提交后，launcher 的
committed validator 必须对正式文件重做同一检查；runner 在首个 attempt
marker 之前还必须通过 sealed FD 重做该检查。任一检查失败都不得
创建 marker 或启动 solver。

## 5. 执行阶段与屏障

### P1：在当前环境重建完整 k gate

R4 首先且只能执行 12 个 common/extra-dense 锚点：

```text
S1-20260807-041  logical 034  Mg 1.10 common-dense quarter
S1-20260807-042  logical 040  Mg 1.10 extra-dense quarter
S1-20260807-043  logical 024  Al 1.00 common-dense quarter
S1-20260807-044  logical 036  Al 1.00 extra-dense quarter
S1-20260807-045  logical 031  Mg 1.00 common-dense quarter
S1-20260807-046  logical 039  Mg 1.00 extra-dense quarter
S1-20260807-047  logical 021  Al 0.90 common-dense quarter
S1-20260807-048  logical 035  Al 0.90 extra-dense quarter
S1-20260807-049  logical 027  Al 1.10 common-dense quarter
S1-20260807-050  logical 037  Al 1.10 extra-dense quarter
S1-20260807-051  logical 028  Mg 0.90 common-dense quarter
S1-20260807-052  logical 038  Mg 0.90 extra-dense quarter
```

这 12 点组成完整的六组 common/extra field pair 和 Al/Mg 两条三端点 k 曲线。
它们必须逐点 accepted，并在 `k-gate-after-052` 屏障中全部通过。在此之前，
053–080 的 run prefix、archive prefix 和
attempt marker 全部禁止出现。

### P2：完成热力学标签矩阵

k gate 通过后，严格按 053–080 顺序执行。每个点必须完成“正式尝试
...
R1-034 暴露的是 host orchestration 生命周期问题，因此 R4 的外层监督进程是正式
...
/usr/bin/env -i HOME=/home/shenwei01 LC_ALL=C LOGNAME=shenwei01 \
  ...
  /usr/bin/python3 -s scripts/launch_s1_g1_thermodynamic_label_audit_r4.py ...
```
...
```text
orchestration/s1/g1_thermodynamic_label_audit_r4_20260807/attempts/<ID>.json
```
...
监督进程正常结束也不能只靠外部 `terminal.json` 宣布整个 R4 协议 accepted。runner
...
所有不等式均为严格不等式，等于阈值即失败。R4 不新增事后阈值，也不降低
R1 标准。

### 7.1 完整性、标量与 EOS

- 逻辑注册点 accepted `40/40`：R1 reused `0/0` + R4 executed `40/40`；
- R1 历史 accepted 候选排除 `10/10`，对 R4 分母贡献 `0`；
...
每个 accepted R4 run 独立提交。首个失败必须先以独立 commit 保留 active run
及机器可读状态，再用相邻 commit 移入唯一
`failed_runs/runtime_relocation/<ID>/attempt-<failure_commit[:12]>`，然后停止。
分类和 inventory 必须使用 R4 专属名，不得覆盖 R1 证据。

最终 analyzer 必须显式报告 `R1 reused=0`、`R1 historical excluded=10`、
`R4 executed=40`、
`R1-034 failed archive contribution=0` 和 `R2-041 rejected archive contribution=0`，
并为 completion、source/input integrity、SCF、
...
exact-scope commit 存在、内容严格通过且 committed validator 再次重验接受时，R4 整体才可
标记最终 `accepted`。
